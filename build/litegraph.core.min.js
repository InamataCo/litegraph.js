...
return!0}this.options=b=b||{};var d=this;b.parentMenu&&(b.parentMenu.constructor!==this.constructor?(console.error("parentMenu must be of class ContextMenu, ignoring it"),b.parentMenu=null):(this.parentMenu=b.parentMenu,this.parentMenu.lock=!0,this.parentMenu.current_submenu=this));var e=null;b.event&&(e=b.event.constructor.name);"MouseEvent"!==e&&"CustomEvent"!==e&&"PointerEvent"!==e&&(console.error("Event passed to ContextMenu is not of type MouseEvent or CustomEvent. Ignoring it. ("+e+")"),b.event=
...
[];for(var a=0;a<this._nodes_in_order.length;++a)this._nodes_in_order[a].onExecute&&this._nodes_executable.push(this._nodes_in_order[a])};v.prototype.computeExecutionOrder=function(a,b){for(var c=[],d=[],e={},f={},h={},k=0,p=this._nodes.length;k<p;++k){var n=this._nodes[k];if(!a||n.onExecute){e[n.id]=n;var q=0;if(n.inputs)for(var t=0,m=n.inputs.length;t<m;t++)n.inputs[t]&&null!=n.inputs[t].link&&(q+=1);0==q?(d.push(n),b&&(n._level=1)):(b&&(n._level=0),h[n.id]=q)}}for(;0!=d.length;)if(n=d.shift(),
c.push(n),delete e[n.id],n.outputs)for(k=0;k<n.outputs.length;k++)if(a=n.outputs[k],null!=a&&null!=a.links&&0!=a.links.length)for(t=0;t<a.links.length;t++)(p=this.links[a.links[t]])&&!f[p.id]&&(q=this.getNodeById(p.target_id),null==q?f[p.id]=!0:(b&&(!q._level||q._level<=n._level)&&(q._level=n._level+1),f[p.id]=!0,--h[q.id],0==h[q.id]&&d.push(q)));for(k in e)c.push(e[k]);c.length!=this._nodes.length&&g.debug&&console.warn("something went wrong, nodes missing");p=c.length;for(k=0;k<p;++k)c[k].order=
...
a||100;for(var c=this.computeExecutionOrder(!1,!0),d=[],e=0;e<c.length;++e){var f=c[e],h=f._level||1;d[h]||(d[h]=[]);d[h].push(f)}c=a;for(e=0;e<d.length;++e)if(f=d[e]){h=100;for(var k=a+g.NODE_TITLE_HEIGHT,p=0;p<f.length;++p){var n=f[p];n.pos[0]=b==g.VERTICAL_LAYOUT?k:c;n.pos[1]=b==g.VERTICAL_LAYOUT?c:k;var q=b==g.VERTICAL_LAYOUT?1:0;n.size[q]>h&&(h=n.size[q]);k+=n.size[b==g.VERTICAL_LAYOUT?0:1]+a+g.NODE_TITLE_HEIGHT}c+=h+a}this.setDirtyCanvas(!0,!0)};v.prototype.getTime=function(){return this.globaltime};
...
n.name||"";n=b(n);f<n&&(f=n)}if(this.outputs)for(k=0,p=this.outputs.length;k<p;++k)n=this.outputs[k],n=n.label||n.name||"",n=b(n),h<n&&(h=n);a[0]=Math.max(f+h+10,e);a[0]=Math.max(a[0],g.NODE_WIDTH);this.widgets&&this.widgets.length&&(a[0]=Math.max(a[0],1.5*g.NODE_WIDTH));a[1]=(this.constructor.slot_start_y||0)+c*g.NODE_SLOT_HEIGHT;c=0;if(this.widgets&&this.widgets.length){k=0;for(p=this.widgets.length;k<p;++k)c=this.widgets[k].computeSize?c+(this.widgets[k].computeSize(a[0])[1]+4):c+(g.NODE_WIDGET_HEIGHT+
...
if(""==b||"*"==b)b=0;for(var f=0,h=a.length;f<h;++f){var k=(b+"").toLowerCase().split(","),p="0"==a[f].type||"*"==a[f].type?"0":a[f].type;p=(p+"").toLowerCase().split(",");for(sI=0;sI<k.length;sI++)for(dI=0;dI<p.length;dI++)if("_event_"==k[sI]&&(k[sI]=g.EVENT),"_event_"==p[sI]&&(p[sI]=g.EVENT),"*"==k[sI]&&(k[sI]=0),"*"==p[sI]&&(p[sI]=0),!(k[sI]!=p[dI]||d&&a[f].links&&null!==a[f].links))return c?a[f]:f}if(d&&!e)for(f=0,h=a.length;f<h;++f)for(k=(b+"").toLowerCase().split(","),p="0"==a[f].type||"*"==
...
1!=this.onMouse(a)){if(1!=a.which||this.pointer_is_double)if(2==a.which){if(g.middle_click_slot_add_default_node&&f&&this.allow_interaction&&!d&&!this.read_only&&!this.connecting_node&&!f.flags.collapsed&&!this.live_mode){e=d=h=!1;if(f.outputs)for(q=0,k=f.outputs.length;q<k;++q)if(p=f.outputs[q],n=f.getConnectionPos(!1,q),A(a.canvasX,a.canvasY,n[0]-15,n[1]-10,30,20)){h=p;d=q;e=!0;break}if(f.inputs)for(q=0,k=f.inputs.length;q<k;++q)if(p=f.inputs[q],n=f.getConnectionPos(!0,q),A(a.canvasX,a.canvasY,
n[0]-15,n[1]-10,30,20)){h=p;d=q;e=!1;break}h&&!1!==d&&(q=.5-(d+1)/(e?f.outputs.length:f.inputs.length),h=f.getBounding(),this.createDefaultNodeForSlot({nodeFrom:e?f:null,slotFrom:e?d:null,nodeTo:e?null:f,slotTo:e?null:d,position:[e?h[0]+h[2]:h[0],a.canvasY-80],nodeType:"AUTO",posAdd:[e?30:-30,130*-q],posSizeFix:[e?0:-1,0]}))}}else 3!=a.which&&!this.pointer_is_double||!this.allow_interaction||d||this.read_only||(f&&(Object.keys(this.selected_nodes).length&&(this.selected_nodes[f.id]||a.shiftKey||a.ctrlKey||
...
d=!0,q||(this.allow_dragnodes&&(this.graph.beforeChange(),this.node_dragged=f),this.selected_nodes[f.id]||this.processNodeSelected(f,a)));h=!1;if(f&&this.allow_interaction&&!d&&!this.read_only){this.live_mode||f.flags.pinned||this.bringToFront(f);if(!this.connecting_node&&!f.flags.collapsed&&!this.live_mode)if(!d&&!1!==f.resizable&&A(a.canvasX,a.canvasY,f.pos[0]+f.size[0]-5,f.pos[1]+f.size[1]-5,10,10))this.graph.beforeChange(),this.resizing_node=f,this.canvas.style.cursor="se-resize",d=!0;else{if(f.outputs){q=
0;for(var k=f.outputs.length;q<k;++q){var p=f.outputs[q],n=f.getConnectionPos(!1,q);if(A(a.canvasX,a.canvasY,n[0]-15,n[1]-10,30,20)){this.connecting_node=f;this.connecting_output=p;this.connecting_output.slot_index=q;this.connecting_pos=f.getConnectionPos(!1,q);this.connecting_slot=q;g.shift_click_do_break_link_from&&a.shiftKey&&f.disconnectOutput(q);if(e){if(f.onOutputDblClick)f.onOutputDblClick(q,a)}else if(f.onOutputClick)f.onOutputClick(q,a);d=!0;break}}}if(f.inputs)for(q=0,k=f.inputs.length;q<
k;++q)if(p=f.inputs[q],n=f.getConnectionPos(!0,q),A(a.canvasX,a.canvasY,n[0]-15,n[1]-10,30,20)){if(e){if(f.onInputDblClick)f.onInputDblClick(q,a)}else if(f.onInputClick)f.onInputClick(q,a);null!==p.link&&(n=this.graph.links[p.link],g.click_do_break_link_to&&(f.disconnectInput(q),d=this.dirty_bgcanvas=!0),this.allow_reconnect_links||a.shiftKey)&&(g.click_do_break_link_to||f.disconnectInput(q),this.connecting_node=this.graph._nodes_by_id[n.origin_id],this.connecting_slot=n.origin_slot,this.connecting_output=
this.connecting_node.outputs[this.connecting_slot],this.connecting_pos=this.connecting_node.getConnectionPos(!1,this.connecting_slot),d=this.dirty_bgcanvas=!0);d||(this.connecting_node=f,this.connecting_input=p,this.connecting_input.slot_index=q,this.connecting_pos=f.getConnectionPos(!0,q),this.connecting_slot=q,d=this.dirty_bgcanvas=!0)}}if(!d){var q=!1;k=[a.canvasX-f.pos[0],a.canvasY-f.pos[1]];if(n=this.processNodeWidgets(f,this.graph_mouse,a))q=!0,this.node_widget=[f,n];if(e&&this.selected_nodes[f.id]){if(f.onDblClick)f.onDblClick(a,
k,this);this.processNodeDblClicked(f);q=!0}f.onMouseDown&&f.onMouseDown(a,k,this)?q=!0:(f.subgraph&&!f.skip_subgraph_button&&!f.flags.collapsed&&k[0]>f.size[0]-g.NODE_TITLE_HEIGHT&&0>k[1]&&(c=this,setTimeout(function(){c.openSubgraph(f.subgraph)},10)),this.live_mode&&(q=h=!0));q?f.is_selected||this.processNodeSelected(f,a):(this.allow_dragnodes&&(this.graph.beforeChange(),this.node_dragged=f),this.processNodeSelected(f,a));this.dirty_canvas=!0}}else if(!d){if(!this.read_only)for(q=0;q<this.visible_links.length;++q)if(h=
...
if(a.inputs)for(var b=0;b<a.inputs.length;++b)delete this.highlighted_links[a.inputs[b].link];if(a.outputs)for(b=0;b<a.outputs.length;++b){var c=a.outputs[b];if(c.links)for(var d=0;d<c.links.length;++d)delete this.highlighted_links[c.links[d]]}}};m.prototype.deselectAllNodes=function(){if(this.graph){for(var a=this.graph._nodes,b=0,c=a.length;b<c;++b){var d=a[b];if(d.is_selected){if(d.onDeselected)d.onDeselected();d.is_selected=!1;if(this.onNodeDeselected)this.onNodeDeselected(d)}}this.selected_nodes=
...
!1,p.setProperty("name",h.name),p.setProperty("type",h.type),this.node_dragged.pos[0]=this.graph_mouse[0]-5,this.node_dragged.pos[1]=this.graph_mouse[1]-5,this.graph.afterChange()):console.error("graph input node not found:",k)}c.fillStyle="#9C9";c.beginPath();c.arc(184,d+.5*e,5,0,2*Math.PI);c.fill();c.fillStyle="#AAA";c.fillText(h.name,30,d+.75*e);c.fillStyle="#777";c.fillText(h.type,130,d+.75*e);d+=e}}this.drawButton(20,d+2,180,e-2,"+","#151515","#222")&&this.showSubgraphPropertiesDialog(b)}};m.prototype.drawSubgraphPanelRight=
...
b.outputs[h];if(!k.not_subgraph_input){if(this.drawButton(e-200,d+2,180,f-2)){var p=b.constructor.output_node_type||"graph/output";this.graph.beforeChange();var n=g.createNode(p);n?(a.add(n),this.block_click=!1,this.last_click_position=null,this.selectNodes([n]),this.node_dragged=n,this.dragging_canvas=!1,n.setProperty("name",k.name),n.setProperty("type",k.type),this.node_dragged.pos[0]=this.graph_mouse[0]-5,this.node_dragged.pos[1]=this.graph_mouse[1]-5,this.graph.afterChange()):console.error("graph input node not found:",
p)}c.fillStyle="#9C9";c.beginPath();c.arc(e-200+16,d+.5*f,5,0,2*Math.PI);c.fill();c.fillStyle="#AAA";c.fillText(k.name,e-200+30,d+.75*f);c.fillStyle="#777";c.fillText(k.type,e-200+130,d+.75*f);d+=f}}this.drawButton(e-200,d+2,180,f-2,"+","#151515","#222")&&this.showSubgraphPropertiesDialogRight(b)}};m.prototype.drawButton=function(a,b,c,d,e,f,h,k){var p=this.ctx;f=f||g.NODE_DEFAULT_COLOR;h=h||"#555";k=k||g.NODE_TEXT_COLOR;var n=b+g.NODE_TITLE_HEIGHT+2,q=this.mouse,t=g.isInsideRectangle(q[0],q[1],a,
n,c,d);n=(q=this.last_click_position)&&g.isInsideRectangle(q[0],q[1],a,n,c,d);p.fillStyle=t?h:f;n&&(p.fillStyle="#AAA");p.beginPath();p.roundRect(a,b,c,d,[4]);p.fill();null!=e&&e.constructor==String&&(p.fillStyle=k,p.textAlign="center",p.font=(.65*d|0)+"px Arial",p.fillText(e,a+.5*c,b+.75*d),p.textAlign="left");a=n&&!this.block_click;n&&this.blockClick();return a};m.prototype.isAreaClicked=function(a,b,c,d,e){var f=this.mouse;g.isInsideRectangle(f[0],f[1],a,b,c,d);b=(a=(f=this.last_click_position)&&
...
(d="red");this.drawNodeShape(a,b,B,c,d,a.is_selected,a.mouseOver);b.shadowColor="transparent";if(a.onDrawForeground)a.onDrawForeground(b,this,this.canvas);b.textAlign=k?"center":"left";b.font=this.inner_text_font;d=!e;var n=this.connecting_output;h=this.connecting_input;b.lineWidth=1;p=0;var q=new Float32Array(2);if(!a.flags.collapsed){if(a.inputs)for(c=0;c<a.inputs.length;c++){var t=a.inputs[c],m=t.type,l=t.shape;b.globalAlpha=f;this.connecting_output&&!g.isValidConnection(t.type,n.type)&&(b.globalAlpha=
.4*f);b.fillStyle=null!=t.link?t.color_on||this.default_connection_color_byType[m]||this.default_connection_color.input_on:t.color_off||this.default_connection_color_byTypeOff[m]||this.default_connection_color_byType[m]||this.default_connection_color.input_off;var u=a.getConnectionPos(!0,c,q);u[0]-=a.pos[0];u[1]-=a.pos[1];p<u[1]+.5*g.NODE_SLOT_HEIGHT&&(p=u[1]+.5*g.NODE_SLOT_HEIGHT);b.beginPath();"array"==m&&(l=g.GRID_SHAPE);t.type===g.EVENT||t.shape===g.BOX_SHAPE?k?b.rect(u[0]-5+.5,u[1]-8+.5,10,14):
b.rect(u[0]-6+.5,u[1]-5+.5,14,10):l===g.ARROW_SHAPE?(b.moveTo(u[0]+8,u[1]+.5),b.lineTo(u[0]-4,u[1]+6+.5),b.lineTo(u[0]-4,u[1]-6+.5),b.closePath()):l===g.GRID_SHAPE?(b.rect(u[0]-4,u[1]-4,2,2),b.rect(u[0]-1,u[1]-4,2,2),b.rect(u[0]+2,u[1]-4,2,2),b.rect(u[0]-4,u[1]-1,2,2),b.rect(u[0]-1,u[1]-1,2,2),b.rect(u[0]+2,u[1]-1,2,2),b.rect(u[0]-4,u[1]+2,2,2),b.rect(u[0]-1,u[1]+2,2,2),b.rect(u[0]+2,u[1]+2,2,2)):e?b.rect(u[0]-4,u[1]-4,8,8):b.arc(u[0],u[1],4,0,2*Math.PI);b.fill();d&&(m=null!=t.label?t.label:t.name)&&
(b.fillStyle=g.NODE_TEXT_COLOR,k||t.dir==g.UP?b.fillText(m,u[0],u[1]-10):b.fillText(m,u[0]+10,u[1]+5))}b.textAlign=k?"center":"right";b.strokeStyle="black";if(a.outputs)for(c=0;c<a.outputs.length;c++)if(t=a.outputs[c],m=t.type,l=t.shape,this.connecting_input&&!g.isValidConnection(m,h.type)&&(b.globalAlpha=.4*f),u=a.getConnectionPos(!1,c,q),u[0]-=a.pos[0],u[1]-=a.pos[1],p<u[1]+.5*g.NODE_SLOT_HEIGHT&&(p=u[1]+.5*g.NODE_SLOT_HEIGHT),b.fillStyle=t.links&&t.links.length?t.color_on||this.default_connection_color_byType[m]||
this.default_connection_color.output_on:t.color_off||this.default_connection_color_byTypeOff[m]||this.default_connection_color_byType[m]||this.default_connection_color.output_off,b.beginPath(),"array"==m&&(l=g.GRID_SHAPE),n=!0,m===g.EVENT||l===g.BOX_SHAPE?k?b.rect(u[0]-5+.5,u[1]-8+.5,10,14):b.rect(u[0]-6+.5,u[1]-5+.5,14,10):l===g.ARROW_SHAPE?(b.moveTo(u[0]+8,u[1]+.5),b.lineTo(u[0]-4,u[1]+6+.5),b.lineTo(u[0]-4,u[1]-6+.5),b.closePath()):l===g.GRID_SHAPE?(b.rect(u[0]-4,u[1]-4,2,2),b.rect(u[0]-1,u[1]-
4,2,2),b.rect(u[0]+2,u[1]-4,2,2),b.rect(u[0]-4,u[1]-1,2,2),b.rect(u[0]-1,u[1]-1,2,2),b.rect(u[0]+2,u[1]-1,2,2),b.rect(u[0]-4,u[1]+2,2,2),b.rect(u[0]-1,u[1]+2,2,2),b.rect(u[0]+2,u[1]+2,2,2),n=!1):e?b.rect(u[0]-4,u[1]-4,8,8):b.arc(u[0],u[1],4,0,2*Math.PI),b.fill(),!e&&n&&b.stroke(),d&&(m=null!=t.label?t.label:t.name))b.fillStyle=g.NODE_TEXT_COLOR,k||t.dir==g.DOWN?b.fillText(m,u[0],u[1]-8):b.fillText(m,u[0]-10,u[1]+5);b.textAlign="left";b.globalAlpha=1;if(a.widgets){t=p;if(k||a.widgets_up)t=2;null!=
a.widgets_start_y&&(t=a.widgets_start_y);this.drawNodeWidgets(a,t,b,this.node_widget&&this.node_widget[0]==a?this.node_widget[1]:null)}}else if(this.render_collapsed_slots){e=f=null;if(a.inputs)for(c=0;c<a.inputs.length;c++)if(t=a.inputs[c],null!=t.link){f=t;break}if(a.outputs)for(c=0;c<a.outputs.length;c++)t=a.outputs[c],t.links&&t.links.length&&(e=t);f&&(f=0,c=-.5*g.NODE_TITLE_HEIGHT,k&&(f=.5*a._collapsed_width,c=-g.NODE_TITLE_HEIGHT),b.fillStyle="#686",b.beginPath(),t.type===g.EVENT||t.shape===
...
"black";a.shadowOffsetX=2;a.shadowOffsetY=2;a.shadowBlur=3;a.fillStyle="#454";a.beginPath();a.roundRect(c[0]-.5*d,c[1]-15-24,d,24,[3]);a.moveTo(c[0]-10,c[1]-15);a.lineTo(c[0]+10,c[1]-15);a.lineTo(c[0],c[1]-5);a.fill();a.shadowColor="transparent";a.textAlign="center";a.fillStyle="#CEC";a.fillText(b,c[0],c[1]-15-24*.3)}};var x=new Float32Array(4);m.prototype.drawNodeShape=function(a,b,c,d,e,f,h){b.strokeStyle=d;b.fillStyle=e;e=g.NODE_TITLE_HEIGHT;var k=.5>this.ds.scale,p=a._shape||a.constructor.shape||
g.ROUND_SHAPE,n=a.constructor.title_mode,q=!0;n==g.TRANSPARENT_TITLE||n==g.NO_TITLE?q=!1:n==g.AUTOHIDE_TITLE&&h&&(q=!0);x[0]=0;x[1]=q?-e:0;x[2]=c[0]+1;x[3]=q?c[1]+e:c[1];h=b.globalAlpha;b.beginPath();p==g.BOX_SHAPE||k?b.fillRect(x[0],x[1],x[2],x[3]):p==g.ROUND_SHAPE||p==g.CARD_SHAPE?b.roundRect(x[0],x[1],x[2],x[3],p==g.CARD_SHAPE?[this.round_radius,this.round_radius,0,0]:[this.round_radius]):p==g.CIRCLE_SHAPE&&b.arc(.5*c[0],.5*c[1],.5*c[0],0,2*Math.PI);b.fill();!a.flags.collapsed&&q&&(b.shadowColor=
"transparent",b.fillStyle="rgba(0,0,0,0.2)",b.fillRect(0,-1,x[2],2));b.shadowColor="transparent";if(a.onDrawBackground)a.onDrawBackground(b,this,this.canvas,this.graph_mouse);if(q||n==g.TRANSPARENT_TITLE){if(a.onDrawTitleBar)a.onDrawTitleBar(b,e,c,this.ds.scale,d);else if(n!=g.TRANSPARENT_TITLE&&(a.constructor.title_color||this.render_title_colored)){q=a.constructor.title_color||d;a.flags.collapsed&&(b.shadowColor=g.DEFAULT_SHADOW_COLOR);if(this.use_gradients){var t=m.gradients[q];t||(t=m.gradients[q]=
b.createLinearGradient(0,0,400,0),t.addColorStop(0,q),t.addColorStop(1,"#000"));b.fillStyle=t}else b.fillStyle=q;b.beginPath();p==g.BOX_SHAPE||k?b.rect(0,-e,c[0]+1,e):(p==g.ROUND_SHAPE||p==g.CARD_SHAPE)&&b.roundRect(0,-e,c[0]+1,e,a.flags.collapsed?[this.round_radius]:[this.round_radius,this.round_radius,0,0]);b.fill();b.shadowColor="transparent"}q=!1;g.node_box_coloured_by_mode&&g.NODE_MODES_COLORS[a.mode]&&(q=g.NODE_MODES_COLORS[a.mode]);g.node_box_coloured_when_on&&(q=a.action_triggered?"#FFF":
a.execute_triggered?"#AAA":q);if(a.onDrawTitleBox)a.onDrawTitleBox(b,e,c,this.ds.scale);else p==g.ROUND_SHAPE||p==g.CIRCLE_SHAPE||p==g.CARD_SHAPE?(k&&(b.fillStyle="black",b.beginPath(),b.arc(.5*e,-.5*e,6,0,2*Math.PI),b.fill()),b.fillStyle=a.boxcolor||q||g.NODE_DEFAULT_BOXCOLOR,k?b.fillRect(.5*e-5,-.5*e-5,10,10):(b.beginPath(),b.arc(.5*e,-.5*e,5,0,2*Math.PI),b.fill())):(k&&(b.fillStyle="black",b.fillRect(.5*(e-10)-1,-.5*(e+10)-1,12,12)),b.fillStyle=a.boxcolor||q||g.NODE_DEFAULT_BOXCOLOR,b.fillRect(.5*
...
(h=g.NODE_TITLE_HEIGHT,q=a.size[0]-h,t=g.isInsideRectangle(this.graph_mouse[0]-a.pos[0],this.graph_mouse[1]-a.pos[1],q+2,-h+2,h-4,h-4),b.fillStyle=t?"#888":"#555",p==g.BOX_SHAPE||k?b.fillRect(q+2,-h+2,h-4,h-4):(b.beginPath(),b.roundRect(q+2,-h+2,h-4,h-4,[4]),b.fill()),b.fillStyle="#333",b.beginPath(),b.moveTo(q+.2*h,.6*-h),b.lineTo(q+.8*h,.6*-h),b.lineTo(q+.5*h,.3*-h),b.fill());if(a.onDrawTitle)a.onDrawTitle(b)}if(f){if(a.onBounding)a.onBounding(x);n==g.TRANSPARENT_TITLE&&(x[1]-=e,x[3]+=e);b.lineWidth=
...
f.inputs[h];if(k&&null!=k.link&&(k=this.graph.links[k.link])){var p=this.graph.getNodeById(k.origin_id);if(null!=p){var n=k.origin_slot;var q=-1==n?[p.pos[0]+10,p.pos[1]+10]:p.getConnectionPos(!1,n,J);var t=f.getConnectionPos(!0,h,K);C[0]=q[0];C[1]=q[1];C[2]=t[0]-q[0];C[3]=t[1]-q[1];0>C[2]&&(C[0]+=C[2],C[2]=Math.abs(C[2]));0>C[3]&&(C[1]+=C[3],C[3]=Math.abs(C[3]));if(F(C,G)){var m=p.outputs[n];n=f.inputs[h];if(m&&n&&(p=m.dir||(p.horizontal?g.DOWN:g.RIGHT),n=n.dir||(f.horizontal?g.UP:g.LEFT),this.renderLink(a,
q,t,k,!1,0,null,p,n),k&&k._last_time&&1E3>b-k._last_time)){m=2-.002*(b-k._last_time);var l=a.globalAlpha;a.globalAlpha=l*m;this.renderLink(a,q,t,k,!0,m,"white",p,n);a.globalAlpha=l}}}}}}a.globalAlpha=1};m.prototype.renderLink=function(a,b,c,d,e,f,h,k,p,n){d&&this.visible_links.push(d);!h&&d&&(h=d.color||m.link_type_colors[d.type]);h||(h=this.default_link_color);null!=d&&this.highlighted_links[d.id]&&(h="#FFF");k=k||g.RIGHT;p=p||g.LEFT;var q=H(b,c);this.render_connections_border&&.6<this.ds.scale&&
(a.lineWidth=this.connections_width+4);a.lineJoin="round";n=n||1;1<n&&(a.lineWidth=.5);a.beginPath();for(var t=0;t<n;t+=1){var l=5*(t-.5*(n-1));if(this.links_render_mode==g.SPLINE_LINK){a.moveTo(b[0],b[1]+l);var r=0,u=0,v=0,w=0;switch(k){case g.LEFT:r=-.25*q;break;case g.RIGHT:r=.25*q;break;case g.UP:u=-.25*q;break;case g.DOWN:u=.25*q}switch(p){case g.LEFT:v=-.25*q;break;case g.RIGHT:v=.25*q;break;case g.UP:w=-.25*q;break;case g.DOWN:w=.25*q}a.bezierCurveTo(b[0]+r,b[1]+u+l,c[0]+v,c[1]+w+l,c[0],c[1]+
l)}else if(this.links_render_mode==g.LINEAR_LINK){a.moveTo(b[0],b[1]+l);w=v=u=r=0;switch(k){case g.LEFT:r=-1;break;case g.RIGHT:r=1;break;case g.UP:u=-1;break;case g.DOWN:u=1}switch(p){case g.LEFT:v=-1;break;case g.RIGHT:v=1;break;case g.UP:w=-1;break;case g.DOWN:w=1}a.lineTo(b[0]+15*r,b[1]+15*u+l);a.lineTo(c[0]+15*v,c[1]+15*w+l);a.lineTo(c[0],c[1]+l)}else if(this.links_render_mode==g.STRAIGHT_LINK)a.moveTo(b[0],b[1]),l=b[0],r=b[1],u=c[0],v=c[1],k==g.RIGHT?l+=10:r+=10,p==g.LEFT?u-=10:v-=10,a.lineTo(l,
r),a.lineTo(.5*(l+u),r),a.lineTo(.5*(l+u),v),a.lineTo(u,v),a.lineTo(c[0],c[1]);else return}this.render_connections_border&&.6<this.ds.scale&&!e&&(a.strokeStyle="rgba(0,0,0,0.5)",a.stroke());a.lineWidth=this.connections_width;a.fillStyle=a.strokeStyle=h;a.stroke();e=this.computeConnectionPoint(b,c,.5,k,p);d&&d._pos&&(d._pos[0]=e[0],d._pos[1]=e[1]);.6<=this.ds.scale&&this.highquality_render&&p!=g.CENTER&&(this.render_connection_arrows&&(t=this.computeConnectionPoint(b,c,.25,k,p),q=this.computeConnectionPoint(b,
c,.26,k,p),d=this.computeConnectionPoint(b,c,.75,k,p),n=this.computeConnectionPoint(b,c,.76,k,p),this.render_curved_connections?(q=-Math.atan2(q[0]-t[0],q[1]-t[1]),n=-Math.atan2(n[0]-d[0],n[1]-d[1])):n=q=c[1]>b[1]?0:Math.PI,a.save(),a.translate(t[0],t[1]),a.rotate(q),a.beginPath(),a.moveTo(-5,-3),a.lineTo(0,7),a.lineTo(5,-3),a.fill(),a.restore(),a.save(),a.translate(d[0],d[1]),a.rotate(n),a.beginPath(),a.moveTo(-5,-3),a.lineTo(0,7),a.lineTo(5,-3),a.fill(),a.restore()),a.beginPath(),a.arc(e[0],e[1],
5,0,2*Math.PI),a.fill());if(f)for(a.fillStyle=h,t=0;5>t;++t)f=(.001*g.getTime()+.2*t)%1,e=this.computeConnectionPoint(b,c,f,k,p),a.beginPath(),a.arc(e[0],e[1],5,0,2*Math.PI),a.fill()};m.prototype.computeConnectionPoint=function(a,b,c,d,e){d=d||g.RIGHT;e=e||g.LEFT;var f=H(a,b),h=[a[0],a[1]],k=[b[0],b[1]];switch(d){case g.LEFT:h[0]+=-.25*f;break;case g.RIGHT:h[0]+=.25*f;break;case g.UP:h[1]+=-.25*f;break;case g.DOWN:h[1]+=.25*f}switch(e){case g.LEFT:k[0]+=-.25*f;break;case g.RIGHT:k[0]+=.25*f;break;
...
0==d.order&&a.strokeRect(d.pos[0]-g.NODE_TITLE_HEIGHT+.5,d.pos[1]-g.NODE_TITLE_HEIGHT+.5,g.NODE_TITLE_HEIGHT,g.NODE_TITLE_HEIGHT);a.fillStyle="#FFF";a.fillText(d.order,d.pos[0]+-.5*g.NODE_TITLE_HEIGHT,d.pos[1]-6)}a.globalAlpha=1};m.prototype.drawNodeWidgets=function(a,b,c,d){if(!a.widgets||!a.widgets.length)return 0;var e=a.size[0],f=a.widgets;b+=2;var h=g.NODE_WIDGET_HEIGHT,k=.5<this.ds.scale;c.save();c.globalAlpha=this.editor_alpha;for(var p=g.WIDGET_OUTLINE_COLOR,n=g.WIDGET_BGCOLOR,q=g.WIDGET_TEXT_COLOR,
t=g.WIDGET_SECONDARY_TEXT_COLOR,m=0;m<f.length;++m){var l=f[m],u=b;l.y&&(u=l.y);l.last_y=u;c.strokeStyle=p;c.fillStyle="#222";c.textAlign="left";l.disabled&&(c.globalAlpha*=.5);var r=l.width||e;switch(l.type){case "button":l.clicked&&(c.fillStyle="#AAA",l.clicked=!1,this.dirty_canvas=!0);c.fillRect(15,u,r-30,h);k&&!l.disabled&&c.strokeRect(15,u,r-30,h);k&&(c.textAlign="center",c.fillStyle=q,c.fillText(l.name,.5*r,u+.7*h));break;case "toggle":c.textAlign="left";c.strokeStyle=p;c.fillStyle=n;c.beginPath();
k?c.roundRect(15,u,r-30,h,[.5*h]):c.rect(15,u,r-30,h);c.fill();k&&!l.disabled&&c.stroke();c.fillStyle=l.value?"#89A":"#333";c.beginPath();c.arc(r-30,u+.5*h,.36*h,0,2*Math.PI);c.fill();k&&(c.fillStyle=t,null!=l.name&&c.fillText(l.name,30,u+.7*h),c.fillStyle=l.value?q:t,c.textAlign="right",c.fillText(l.value?l.options.on||"true":l.options.off||"false",r-40,u+.7*h));break;case "slider":c.fillStyle=n;c.fillRect(15,u,r-30,h);var v=l.options.max-l.options.min,w=(l.value-l.options.min)/v;c.fillStyle=d==
l?"#89A":"#678";c.fillRect(15,u,w*(r-30),h);k&&!l.disabled&&c.strokeRect(15,u,r-30,h);l.marker&&(v=(l.marker-l.options.min)/v,c.fillStyle="#AA9",c.fillRect(15+v*(r-30),u,2,h));k&&(c.textAlign="center",c.fillStyle=q,c.fillText(l.name+"  "+Number(l.value).toFixed(3),.5*r,u+.7*h));break;case "number":case "combo":c.textAlign="left";c.strokeStyle=p;c.fillStyle=n;c.beginPath();k?c.roundRect(15,u,r-30,h,[.5*h]):c.rect(15,u,r-30,h);c.fill();k&&(l.disabled||c.stroke(),c.fillStyle=q,l.disabled||(c.beginPath(),
c.moveTo(31,u+5),c.lineTo(21,u+.5*h),c.lineTo(31,u+h-5),c.fill(),c.beginPath(),c.moveTo(r-15-16,u+5),c.lineTo(r-15-6,u+.5*h),c.lineTo(r-15-16,u+h-5),c.fill()),c.fillStyle=t,c.fillText(l.name,35,u+.7*h),c.fillStyle=q,c.textAlign="right","number"==l.type?c.fillText(Number(l.value).toFixed(void 0!==l.options.precision?l.options.precision:3),r-30-20,u+.7*h):(v=l.value,l.options.values&&(w=l.options.values,w.constructor===Function&&(w=w()),w&&w.constructor!==Array&&(v=w[l.value])),c.fillText(v,r-30-20,
u+.7*h)));break;case "string":case "text":c.textAlign="left";c.strokeStyle=p;c.fillStyle=n;c.beginPath();k?c.roundRect(15,u,r-30,h,[.5*h]):c.rect(15,u,r-30,h);c.fill();k&&(l.disabled||c.stroke(),c.save(),c.beginPath(),c.rect(15,u,r-30,h),c.clip(),c.fillStyle=t,null!=l.name&&c.fillText(l.name,30,u+.7*h),c.fillStyle=q,c.textAlign="right",c.fillText(String(l.value).substr(0,30),r-30,u+.7*h),c.restore());break;default:l.draw&&l.draw(c,a,r,u,h)}b+=(l.computeSize?l.computeSize(r)[1]:h)+4;c.globalAlpha=
this.editor_alpha}c.restore();c.textAlign="left"};m.prototype.processNodeWidgets=function(a,b,c,d){function e(d,e){d.value=e;d.options&&d.options.property&&void 0!==a.properties[d.options.property]&&a.setProperty(d.options.property,e);d.callback&&d.callback(d.value,p,a,b,c)}if(!a.widgets||!a.widgets.length)return null;for(var f=b[0]-a.pos[0],h=b[1]-a.pos[1],k=a.size[0],p=this,n=this.getCanvasWindow(),q=0;q<a.widgets.length;++q){var t=a.widgets[q];if(t&&!t.disabled){var m=t.computeSize?t.computeSize(k)[1]:
g.NODE_WIDGET_HEIGHT,l=t.width||k;if(t==d||!(6>f||f>l-12||h<t.last_y||h>t.last_y+m||void 0===t.last_y)){d=t.value;switch(t.type){case "button":c.type===g.pointerevents_method+"down"&&(t.callback&&setTimeout(function(){t.callback(t,p,a,b,c)},20),this.dirty_canvas=t.clicked=!0);break;case "slider":d=t.value;n=Math.clamp((f-15)/(l-30),0,1);t.value=t.options.min+(t.options.max-t.options.min)*n;d!=t.value&&setTimeout(function(){e(t,t.value)},20);this.dirty_canvas=!0;break;case "number":case "combo":d=
t.value;if(c.type==g.pointerevents_method+"move"&&"number"==t.type)c.deltaX&&(t.value+=.1*c.deltaX*(t.options.step||1)),null!=t.options.min&&t.value<t.options.min&&(t.value=t.options.min),null!=t.options.max&&t.value>t.options.max&&(t.value=t.options.max);else if(c.type==g.pointerevents_method+"down"){var u=t.options.values;u&&u.constructor===Function&&(u=t.options.values(t,a));var r=null;"number"!=t.type&&(r=u.constructor===Array?u:Object.keys(u));f=40>f?-1:f>l-40?1:0;if("number"==t.type)t.value+=
.1*f*(t.options.step||1),null!=t.options.min&&t.value<t.options.min&&(t.value=t.options.min),null!=t.options.max&&t.value>t.options.max&&(t.value=t.options.max);else if(f)n=-1,this.last_mouseclick=0,n=u.constructor===Object?r.indexOf(String(t.value))+f:r.indexOf(t.value)+f,n>=r.length&&(n=r.length-1),0>n&&(n=0),t.value=u.constructor===Array?u[n]:n;else{var v=u!=r?Object.values(u):u;new g.ContextMenu(v,{scale:Math.max(1,this.ds.scale),event:c,className:"dark",callback:function(a,b,c){u!=r&&(a=v.indexOf(a));
this.value=a;e(this,a);p.dirty_canvas=!0;return!1}.bind(t)},n)}}else c.type==g.pointerevents_method+"up"&&"number"==t.type&&(f=40>f?-1:f>l-40?1:0,200>c.click_time&&0==f&&this.prompt("Value",t.value,function(a){this.value=Number(a);e(this,this.value)}.bind(t),c));d!=t.value&&setTimeout(function(){e(this,this.value)}.bind(t),20);this.dirty_canvas=!0;break;case "toggle":c.type==g.pointerevents_method+"down"&&(t.value=!t.value,setTimeout(function(){e(t,t.value)},20));break;case "string":case "text":c.type==
g.pointerevents_method+"down"&&this.prompt("Value",t.value,function(a){this.value=a;e(this,a)}.bind(t),c,t.options?t.options.multiline:!1);break;default:t.mouse&&(this.dirty_canvas=t.mouse(c,[f,h],a))}if(d!=t.value){if(a.onWidgetChanged)a.onWidgetChanged(t.name,t.value,d,t);a.graph._version++}return t}}}return null};m.prototype.drawGroups=function(a,b){if(this.graph){a=this.graph._groups;b.save();b.globalAlpha=.5*this.editor_alpha;for(var c=0;c<a.length;++c){var d=a[c];if(F(this.visible_area,d._bounding)){b.fillStyle=
d.color||"#335";b.strokeStyle=d.color||"#335";var e=d._pos,f=d._size;b.globalAlpha=.25*this.editor_alpha;b.beginPath();b.rect(e[0]+.5,e[1]+.5,f[0],f[1]);b.fill();b.globalAlpha=this.editor_alpha;b.stroke();b.beginPath();b.moveTo(e[0]+f[0],e[1]+f[1]);b.lineTo(e[0]+f[0]-10,e[1]+f[1]);b.lineTo(e[0]+f[0],e[1]+f[1]-10);b.fill();f=d.font_size||g.DEFAULT_GROUP_FONT_SIZE;b.font=f+"px Arial";b.textAlign="left";b.fillText(d.title,e[0]+4,e[1]+f)}}b.restore()}};m.prototype.adjustNodesSize=function(){for(var a=
...
.1);var d=setInterval(function(){b.editor_alpha*=c;b.dirty_canvas=!0;b.dirty_bgcanvas=!0;1>c&&.01>b.editor_alpha&&(clearInterval(d),1>c&&(b.live_mode=!0));1<c&&.99<b.editor_alpha&&(clearInterval(d),b.editor_alpha=1)},1)}else this.live_mode=!this.live_mode,this.dirty_bgcanvas=this.dirty_canvas=!0};m.prototype.onNodeSelectionChange=function(a){};m.onGroupAdd=function(a,b,c){a=m.active_canvas;a.getCanvasWindow();b=new g.LGraphGroup;b.pos=a.convertEventToCanvasOffset(c);a.graph.add(b)};m.onMenuAdd=function(a,
b,c,d,e){function f(a,b){var d=[];g.getNodeTypesCategories(h.filter||p.filter).filter(function(b){return b.startsWith(a)}).map(function(b){if(b){b=b.replace(new RegExp("^("+a+")"),"").split("/")[0];var c=""===a?b+"/":a+b+"/";-1!=b.indexOf("::")&&(b=b.split("::")[1]);-1===d.findIndex(function(a){return a.value===c})&&d.push({value:c,content:b,has_submenu:!0,callback:function(a,b,c,d){f(a.value,d)}})}});g.getNodeTypesInCategory(a.slice(0,-1),h.filter||p.filter).map(function(a){a.skip_list||d.push({value:a.type,
content:a.title,has_submenu:!1,callback:function(a,b,c,d){b=d.getFirstEvent();h.graph.beforeChange();if(a=g.createNode(a.value))a.pos=h.convertEventToCanvasOffset(b),h.graph.add(a);e&&e(a);h.graph.afterChange()}})});new g.ContextMenu(d,{event:c,parentMenu:b},k)}var h=m.active_canvas,k=h.getCanvasWindow(),p=h.graph;if(p)return f("",d),!1};m.onMenuCollapseAll=function(){};m.onMenuNodeEdit=function(){};m.showMenuNodeOptionalInputs=function(a,b,c,d,e){if(e){var f=this;a=m.active_canvas.getCanvasWindow();
b=e.optional_inputs;e.onGetInputs&&(b=e.onGetInputs());var h=[];if(b)for(var k=0;k<b.length;k++){var p=b[k];if(p){var n=p[0];p[2]||(p[2]={});p[2].label&&(n=p[2].label);p[2].removable=!0;n={content:n,value:p};p[1]==g.ACTION&&(n.className="event");h.push(n)}else h.push(null)}e.onMenuNodeInputs&&(b=e.onMenuNodeInputs(h))&&(h=b);if(h.length)return new g.ContextMenu(h,{event:c,callback:function(a,b,c){if(e&&(a.callback&&a.callback.call(f,e,a,b,c),a.value)){e.graph.beforeChange();e.addInput(a.value[0],
a.value[1],a.value[2]);if(e.onNodeInputAdd)e.onNodeInputAdd(a.value);e.setDirtyCanvas(!0,!0);e.graph.afterChange()}},parentMenu:d,node:e},a),!1;console.log("no input entries")}};m.showMenuNodeOptionalOutputs=function(a,b,c,d,e){function f(a,b,c){if(e&&(a.callback&&a.callback.call(h,e,a,b,c),a.value))if(c=a.value[1],!c||c.constructor!==Object&&c.constructor!==Array){e.graph.beforeChange();e.addOutput(a.value[0],a.value[1],a.value[2]);if(e.onNodeOutputAdd)e.onNodeOutputAdd(a.value);e.setDirtyCanvas(!0,
!0);e.graph.afterChange()}else{a=[];for(var k in c)a.push({content:k,value:c[k]});new g.ContextMenu(a,{event:b,callback:f,parentMenu:d,node:e});return!1}}if(e){var h=this;a=m.active_canvas.getCanvasWindow();b=e.optional_outputs;e.onGetOutputs&&(b=e.onGetOutputs());var k=[];if(b)for(var p=0;p<b.length;p++){var n=b[p];if(!n)k.push(null);else if(!e.flags||!e.flags.skip_repeated_outputs||-1==e.findOutputSlot(n[0])){var q=n[0];n[2]||(n[2]={});n[2].label&&(q=n[2].label);n[2].removable=!0;q={content:q,value:n};
n[1]==g.EVENT&&(q.className="event");k.push(q)}}this.onMenuNodeOutputs&&(k=this.onMenuNodeOutputs(k));g.do_add_triggers_slots&&-1==e.findOutputSlot("onExecuted")&&k.push({content:"On Executed",value:["onExecuted",g.EVENT,{nameLocked:!0}],className:"event"});e.onMenuNodeOutputs&&(b=e.onMenuNodeOutputs(k))&&(k=b);if(k.length)return new g.ContextMenu(k,{event:c,callback:f,parentMenu:d,node:e},a),!1}};m.onShowMenuNodeProperties=function(a,b,c,d,e){if(e&&e.properties){var f=m.active_canvas;b=f.getCanvasWindow();
var h=[],k;for(k in e.properties){a=void 0!==e.properties[k]?e.properties[k]:" ";"object"==typeof a&&(a=JSON.stringify(a));var p=e.getPropertyInfo(k);if("enum"==p.type||"combo"==p.type)a=m.getPropertyPrintableValue(a,p.values);a=m.decodeHTML(a);h.push({content:"<span class='property_name'>"+(p.label?p.label:k)+"</span><span class='property_value'>"+a+"</span>",value:k})}if(h.length)return new g.ContextMenu(h,{event:c,callback:function(a,b,c,d){e&&(b=this.getBoundingClientRect(),f.showEditPropertyValue(e,
a.value,{position:[b.left,b.top]}))},parentMenu:d,allow_html:!0,node:e},b),!1}};m.decodeHTML=function(a){var b=document.createElement("div");b.innerText=a;return b.innerHTML};m.onMenuResizeNode=function(a,b,c,d,e){if(e){a=function(a){a.size=a.computeSize();if(a.onResize)a.onResize(a.size)};b=m.active_canvas;if(!b.selected_nodes||1>=Object.keys(b.selected_nodes).length)a(e);else for(var f in b.selected_nodes)a(b.selected_nodes[f]);e.setDirtyCanvas(!0,!0)}};m.prototype.showLinkMenu=function(a,b){var c=
this,d=c.graph.getNodeById(a.origin_id),e=c.graph.getNodeById(a.target_id),f=!1;d&&d.outputs&&d.outputs[a.origin_slot]&&(f=d.outputs[a.origin_slot].type);var h=!1;e&&e.outputs&&e.outputs[a.target_slot]&&(h=e.inputs[a.target_slot].type);var k=new g.ContextMenu(["Add Node",null,"Delete",null],{event:b,title:null!=a.data?a.data.constructor.name:null,callback:function(b,g,q){switch(b){case "Add Node":m.onMenuAdd(null,null,q,k,function(b){b.inputs&&b.inputs.length&&b.outputs&&b.outputs.length&&d.connectByType(a.origin_slot,
b,f)&&(b.connectByType(a.target_slot,e,h),b.pos[0]-=.5*b.size[0])});break;case "Delete":c.graph.removeLink(a.id)}}});return!1};m.prototype.createDefaultNodeForSlot=function(a){a=a||{};a=Object.assign({nodeFrom:null,slotFrom:null,nodeTo:null,slotTo:null,position:[],nodeType:null,posAdd:[0,0],posSizeFix:[0,0]},a);var b=a.nodeFrom&&null!==a.slotFrom,c=!b&&a.nodeTo&&null!==a.slotTo;if(!b&&!c)return console.warn("No data passed to createDefaultNodeForSlot "+a.nodeFrom+" "+a.slotFrom+" "+a.nodeTo+" "+a.slotTo),
!1;if(!a.nodeType)return console.warn("No type to createDefaultNodeForSlot"),!1;var d=b?a.nodeFrom:a.nodeTo,e=b?a.slotFrom:a.slotTo;switch(typeof e){case "string":c=b?d.findOutputSlot(e,!1):d.findInputSlot(e,!1);e=b?d.outputs[e]:d.inputs[e];break;case "object":c=b?d.findOutputSlot(e.name):d.findInputSlot(e.name);break;case "number":c=e;e=b?d.outputs[e]:d.inputs[e];break;default:return console.warn("Cant get slot information "+e),!1}!1!==e&&!1!==c||console.warn("createDefaultNodeForSlot bad slotX "+
...
if(t&&e&&g.registered_slot_out_types[e]&&g.registered_slot_out_types[e].nodes&&(e=g.registered_slot_out_types[e].nodes.includes(a),!1===e))return!1}return!0};var p=0;d=d.toLowerCase();var n=h.filter||h.graph.filter;if(b.do_type_filter&&f.search_box)var l=f.search_box.querySelector(".slot_in_type_filter"),t=f.search_box.querySelector(".slot_out_type_filter");else t=l=!1;for(k in g.searchbox_extras){var q=g.searchbox_extras[k];if(b.show_all_if_empty&&!d||-1!==q.desc.toLowerCase().indexOf(d)){var r=
g.registered_node_types[q.type];if((!r||r.filter==n)&&e(q.type)&&(a(q.desc,"searchbox_extra"),-1!==m.search_limit&&p++>m.search_limit))break}}q=null;if(Array.prototype.filter)q=Object.keys(g.registered_node_types).filter(e);else for(k in q=[],g.registered_node_types)e(k)&&q.push(k);for(k=0;k<q.length&&!(a(q[k]),-1!==m.search_limit&&p++>m.search_limit);k++);if(b.show_general_after_typefiltered&&(l.value||t.value)){filtered_extra=[];for(k in g.registered_node_types)e(k,{inTypeOverride:l&&l.value?"*":
!1,outTypeOverride:t&&t.value?"*":!1})&&filtered_extra.push(k);for(k=0;k<filtered_extra.length&&!(a(filtered_extra[k],"generic_type"),-1!==m.search_limit&&p++>m.search_limit);k++);}if((l.value||t.value)&&0==u.childNodes.length&&b.show_general_if_none_on_typefilter){filtered_extra=[];for(k in g.registered_node_types)e(k,{skipFilter:!0})&&filtered_extra.push(k);for(k=0;k<filtered_extra.length&&!(a(filtered_extra[k],"not_in_filter"),-1!==m.search_limit&&p++>m.search_limit);k++);}}}def_options={slot_from:null,
...
b.do_type_filter&&(n.innerHTML+="<select class='slot_in_type_filter'><option value=''></option></select>",n.innerHTML+="<select class='slot_out_type_filter'><option value=''></option></select>");n.innerHTML+="<div class='helper'></div>";p.fullscreenElement?p.fullscreenElement.appendChild(n):(p.body.appendChild(n),p.body.style.overflow="hidden");if(b.do_type_filter)var q=n.querySelector(".slot_in_type_filter"),t=n.querySelector(".slot_out_type_filter");n.close=function(){f.search_box=null;this.blur();
k.focus();p.body.style.overflow="";setTimeout(function(){f.canvas.focus()},20);n.parentNode&&n.parentNode.removeChild(n)};1<this.ds.scale&&(n.style.transform="scale("+this.ds.scale+")");if(b.hide_on_mouse_leave){var l=!1,r=null;g.pointerListenerAdd(n,"enter",function(a){r&&(clearTimeout(r),r=null)});g.pointerListenerAdd(n,"leave",function(a){l||(r=setTimeout(function(){n.close()},500))});b.do_type_filter&&(q.addEventListener("click",function(a){l++}),q.addEventListener("blur",function(a){l=0}),q.addEventListener("change",
function(a){l=-1}),t.addEventListener("click",function(a){l++}),t.addEventListener("blur",function(a){l=0}),t.addEventListener("change",function(a){l=-1}))}f.search_box&&f.search_box.close();f.search_box=n;var u=n.querySelector(".helper"),v=null,w=null,x=null,z=n.querySelector("input");z&&(z.addEventListener("blur",function(a){this.focus()}),z.addEventListener("keydown",function(a){if(38==a.keyCode)d(!1);else if(40==a.keyCode)d(!0);else if(27==a.keyCode)n.close();else if(13==a.keyCode)x?c(x.innerHTML):
v?c(v):n.close();else{w&&clearInterval(w);w=setTimeout(e,250);return}a.preventDefault();a.stopPropagation();a.stopImmediatePropagation();return!0}));if(b.do_type_filter){if(q){var y=g.slot_types_in,C=y.length;if(b.type_filter_in==g.EVENT||b.type_filter_in==g.ACTION)b.type_filter_in="_event_";for(var A=0;A<C;A++){var B=document.createElement("option");B.value=y[A];B.innerHTML=y[A];q.appendChild(B);!1!==b.type_filter_in&&(b.type_filter_in+"").toLowerCase()==(y[A]+"").toLowerCase()&&(B.selected=!0)}q.addEventListener("change",
function(){e()})}if(t){y=g.slot_types_out;C=y.length;if(b.type_filter_out==g.EVENT||b.type_filter_out==g.ACTION)b.type_filter_out="_event_";for(A=0;A<C;A++)B=document.createElement("option"),B.value=y[A],B.innerHTML=y[A],t.appendChild(B),!1!==b.type_filter_out&&(b.type_filter_out+"").toLowerCase()==(y[A]+"").toLowerCase()&&(B.selected=!0);t.addEventListener("change",function(){e()})}}q=k.getBoundingClientRect();t=(a?a.clientY:q.top+.5*q.height)-20;n.style.left=(a?a.clientX:q.left+.5*q.width)-80+"px";
n.style.top=t+"px";a.layerY>q.height-200&&(u.style.maxHeight=q.height-a.layerY-20+"px");z.focus();b.show_all_on_open&&e();return n};m.prototype.showEditPropertyValue=function(a,b,c){function d(){e(l.value)}function e(d){f&&f.values&&f.values.constructor===Object&&void 0!=f.values[d]&&(d=f.values[d]);"number"==typeof a.properties[b]&&(d=Number(d));if("array"==h||"object"==h)d=JSON.parse(d);a.properties[b]=d;a.graph&&a.graph._version++;if(a.onPropertyChanged)a.onPropertyChanged(b,d);if(c.onclose)c.onclose();
...
p;f.values.constructor===Array&&(n=f.values[p]);g+="<option value='"+n+"' "+(n==a.properties[b]?"selected":"")+">"+f.values[p]+"</option>"}g+="</select>"}var q=this.createDialog("<span class='name'>"+(f.label?f.label:b)+"</span>"+g+"<button>OK</button>",c),l=!1;if("enum"!=h&&"combo"!=h||!f.values)if("boolean"==h||"toggle"==h)(l=q.querySelector("input"))&&l.addEventListener("click",function(a){q.modified();e(!!l.checked)});else{if(l=q.querySelector("input"))l.addEventListener("blur",function(a){this.focus()}),
n=void 0!==a.properties[b]?a.properties[b]:"","string"!==h&&(n=JSON.stringify(n)),l.value=n,l.addEventListener("keydown",function(a){if(27==a.keyCode)q.close();else if(13==a.keyCode)d();else if(13!=a.keyCode){q.modified();return}a.preventDefault();a.stopPropagation()})}else l=q.querySelector("select"),l.addEventListener("change",function(a){q.modified();e(a.target.value)});l&&l.focus();q.querySelector("button").addEventListener("click",d);return q}};m.prototype.createDialog=function(a,b){def_options=
