...
b,null,c)}}};l.prototype.triggerSlot=function(a,b,c,d){d=d||{};if(this.outputs&&(a=this.outputs[a])&&(a=a.links)&&a.length){this.graph&&(this.graph._last_trigger_time=g.getTime());for(var e=0;e<a.length;++e){var f=a[e];if(null==c||c==f){var h=this.graph.links[a[e]];h&&(h._last_time=g.getTime(),f=this.graph.getNodeById(h.target_id))&&(f.mode===g.ON_TRIGGER?(d.action_call||(d.action_call=this.id+"_trigg_"+Math.floor(9999*Math.random())),f.onExecute&&f.doExecute(b,d)):f.onAction&&(d.action_call||(d.action_call=
...
this.properties_info||(this.properties_info=[]);this.properties_info.push(c);this.properties||(this.properties={});this.properties[a]=b;return c};l.prototype.addOutput=function(a,b,c){a={name:a,type:b,links:null};if(c)for(var d in c)a[d]=c[d];this.outputs||(this.outputs=[]);this.outputs.push(a);if(this.onOutputAdded)this.onOutputAdded(a);g.auto_load_slot_types&&g.registerNodeAndSlotType(this,b,!0);this.setSize(this.computeSize());this.setDirtyCanvas(!0,!0);return a};l.prototype.addOutputs=function(a){for(var b=
0;b<a.length;++b){var c=a[b],d={name:c[0],type:c[1],link:null};if(a[2])for(var e in c[2])d[e]=c[2][e];this.outputs||(this.outputs=[]);this.outputs.push(d);if(this.onOutputAdded)this.onOutputAdded(d);g.auto_load_slot_types&&g.registerNodeAndSlotType(this,c[1],!0)}this.setSize(this.computeSize());this.setDirtyCanvas(!0,!0)};l.prototype.removeOutput=function(a){this.disconnectOutput(a);this.outputs.splice(a,1);for(var b=a;b<this.outputs.length;++b)if(this.outputs[b]&&this.outputs[b].links)for(var c=
...
0]);c=Math.max(c,1);var d=g.NODE_TEXT_SIZE,e=b(this.title),f=0,h=0;if(this.inputs)for(var k=0,p=this.inputs.length;k<p;++k){var n=this.inputs[k];n=n.label||n.name||"";n=b(n);f<n&&(f=n)}if(this.outputs)for(k=0,p=this.outputs.length;k<p;++k)n=this.outputs[k],n=n.label||n.name||"",n=b(n),h<n&&(h=n);a[0]=Math.max(f+h+10,e);a[0]=Math.max(a[0],g.NODE_WIDTH);this.widgets&&this.widgets.length&&(a[0]=Math.max(a[0],1.5*g.NODE_WIDTH));a[1]=(this.constructor.slot_start_y||0)+c*g.NODE_SLOT_HEIGHT;c=0;if(this.widgets&&
this.widgets.length){k=0;for(p=this.widgets.length;k<p;++k)c=this.widgets[k].computeSize?c+(this.widgets[k].computeSize(a[0])[1]+4):c+(g.NODE_WIDGET_HEIGHT+4);c+=8}a[1]=this.widgets_up?Math.max(a[1],c):null!=this.widgets_start_y?Math.max(a[1],c+this.widgets_start_y):a[1]+c;this.constructor.min_height&&a[1]<this.constructor.min_height&&(a[1]=this.constructor.min_height);a[1]+=6;return a};l.prototype.getPropertyInfo=function(a){var b=null;if(this.properties_info)for(var c=0;c<this.properties_info.length;++c)if(this.properties_info[c].name==
...
b,c,d){d=d||{};d=Object.assign({createEventInCase:!0,firstFreeIfInputGeneralInCase:!0,generalTypeInCase:!0},d);b&&b.constructor===Number&&(b=this.graph.getNodeById(b));e=b.findOutputSlotByType(c,!1,!0);if(0<=e&&null!==e)return b.connect(e,this,a);if(d.generalTypeInCase){var e=b.findOutputSlotByType(0,!1,!0,!0);if(0<=e)return b.connect(e,this,a)}if(d.createEventInCase&&c==g.EVENT&&g.do_add_triggers_slots)return e=b.addOnExecutedOutput(),b.connect(e,this,a);if(d.firstFreeIfInputGeneralInCase&&(0==c||
"*"==c||""==c)&&(e=b.findOutputSlotFree({typesNotAccepted:[g.EVENT]}),0<=e))return b.connect(e,this,a);console.debug("no way to connect byOUT type: ",c," to sourceNODE ",b);return null};l.prototype.connect=function(a,b,c){c=c||0;if(!this.graph)return console.log("Connect: Error, node doesn't belong to any graph. Nodes must be added first to a graph before connecting them."),null;if(a.constructor===String){if(a=this.findOutputSlot(a),-1==a)return g.debug&&console.log("Connect: Error, no slot of name "+
a),null}else if(!this.outputs||a>=this.outputs.length)return g.debug&&console.log("Connect: Error, slot number not found"),null;b&&b.constructor===Number&&(b=this.graph.getNodeById(b));if(!b)throw"target node is null";if(b==this)return null;if(c.constructor===String){if(c=b.findInputSlot(c),-1==c)return g.debug&&console.log("Connect: Error, no slot of name "+c),null}else if(c===g.EVENT)if(g.do_add_triggers_slots)b.changeMode(g.ON_TRIGGER),c=b.findInputSlot("onTrigger");else return null;else if(!b.inputs||
c>=b.inputs.length)return g.debug&&console.log("Connect: Error, slot number not found"),null;var d=b.inputs[c],e=this.outputs[a];if(!this.outputs[a])return null;b.onBeforeConnectInput&&(c=b.onBeforeConnectInput(c));if(!1===c||null===c||!g.isValidConnection(e.type,d.type))return this.setDirtyCanvas(!1,!0),null;if(b.onConnectInput&&!1===b.onConnectInput(c,e.type,e,this,a)||this.onConnectOutput&&!1===this.onConnectOutput(a,d.type,d,b,c))return null;b.inputs[c]&&null!=b.inputs[c].link&&(this.graph.beforeChange(),
...
c,!0,f,d);this.graph&&this.graph.onNodeConnectionChange&&(this.graph.onNodeConnectionChange(g.INPUT,b,c,this,a),this.graph.onNodeConnectionChange(g.OUTPUT,this,a,b,c));this.setDirtyCanvas(!1,!0);this.graph.afterChange();this.graph.connectionChange(this,f);return f};l.prototype.disconnectOutput=function(a,b){if(a.constructor===String){if(a=this.findOutputSlot(a),-1==a)return g.debug&&console.log("Connect: Error, no slot of name "+a),!1}else if(!this.outputs||a>=this.outputs.length)return g.debug&&
...
enumerable:!0})};z.prototype.configure=function(a){this.title=a.title;this._bounding.set(a.bounding);this.color=a.color;this.font=a.font};z.prototype.serialize=function(){var a=this._bounding;return{title:this.title,bounding:[Math.round(a[0]),Math.round(a[1]),Math.round(a[2]),Math.round(a[3])],color:this.color,font:this.font}};z.prototype.move=function(a,b,c){this._pos[0]+=a;this._pos[1]+=b;if(!c)for(c=0;c<this._nodes.length;++c){var d=this._nodes[c];d.pos[0]+=a;d.pos[1]+=b}};z.prototype.recomputeInsideNodes=
function(){this._nodes.length=0;for(var a=this.graph._nodes,b=new Float32Array(4),c=0;c<a.length;++c){var d=a[c];d.getBounding(b);F(this._bounding,b)&&this._nodes.push(d)}};z.prototype.isPointInside=l.prototype.isPointInside;z.prototype.setDirtyCanvas=l.prototype.setDirtyCanvas;g.DragAndScale=y;y.prototype.bindEvents=function(a){this.last_mouse=new Float32Array(2);this._binded_mouse_callback=this.onMouse.bind(this);g.pointerListenerAdd(a,"down",this._binded_mouse_callback);g.pointerListenerAdd(a,
...
1;this.offset[0]=0;this.offset[1]=0};q.LGraphCanvas=g.LGraphCanvas=m;m.DEFAULT_BACKGROUND_IMAGE="data:image/png;base64,";
...
(d="red");this.drawNodeShape(a,b,B,c,d,a.is_selected,a.mouseOver);b.shadowColor="transparent";if(a.onDrawForeground)a.onDrawForeground(b,this,this.canvas);b.textAlign=k?"center":"left";b.font=this.inner_text_font;d=!e;var n=this.connecting_output;h=this.connecting_input;b.lineWidth=1;p=0;var t=new Float32Array(2);if(!a.flags.collapsed){if(a.inputs)for(c=0;c<a.inputs.length;c++){var r=a.inputs[c],m=r.type,l=r.shape;b.globalAlpha=f;this.connecting_output&&!g.isValidConnection(r.type,n.type)&&(b.globalAlpha=
.4*f);b.fillStyle=null!=r.link?r.color_on||this.default_connection_color_byType[m]||this.default_connection_color.input_on:r.color_off||this.default_connection_color_byTypeOff[m]||this.default_connection_color_byType[m]||this.default_connection_color.input_off;var u=a.getConnectionPos(!0,c,t);u[0]-=a.pos[0];u[1]-=a.pos[1];p<u[1]+.5*g.NODE_SLOT_HEIGHT&&(p=u[1]+.5*g.NODE_SLOT_HEIGHT);b.beginPath();"array"==m&&(l=g.GRID_SHAPE);r.type===g.EVENT||r.shape===g.BOX_SHAPE?k?b.rect(u[0]-5+.5,u[1]-8+.5,10,14):
b.rect(u[0]-6+.5,u[1]-5+.5,14,10):l===g.ARROW_SHAPE?(b.moveTo(u[0]+8,u[1]+.5),b.lineTo(u[0]-4,u[1]+6+.5),b.lineTo(u[0]-4,u[1]-6+.5),b.closePath()):l===g.GRID_SHAPE?(b.rect(u[0]-4,u[1]-4,2,2),b.rect(u[0]-1,u[1]-4,2,2),b.rect(u[0]+2,u[1]-4,2,2),b.rect(u[0]-4,u[1]-1,2,2),b.rect(u[0]-1,u[1]-1,2,2),b.rect(u[0]+2,u[1]-1,2,2),b.rect(u[0]-4,u[1]+2,2,2),b.rect(u[0]-1,u[1]+2,2,2),b.rect(u[0]+2,u[1]+2,2,2)):e?b.rect(u[0]-4,u[1]-4,8,8):b.arc(u[0],u[1],4,0,2*Math.PI);b.fill();d&&(m=null!=r.label?r.label:r.name)&&
(b.fillStyle=g.NODE_TEXT_COLOR,k||r.dir==g.UP?b.fillText(m,u[0],u[1]-10):b.fillText(m,u[0]+10,u[1]+5))}b.textAlign=k?"center":"right";b.strokeStyle="black";if(a.outputs)for(c=0;c<a.outputs.length;c++)if(r=a.outputs[c],m=r.type,l=r.shape,this.connecting_input&&!g.isValidConnection(m,h.type)&&(b.globalAlpha=.4*f),u=a.getConnectionPos(!1,c,t),u[0]-=a.pos[0],u[1]-=a.pos[1],p<u[1]+.5*g.NODE_SLOT_HEIGHT&&(p=u[1]+.5*g.NODE_SLOT_HEIGHT),b.fillStyle=r.links&&r.links.length?r.color_on||this.default_connection_color_byType[m]||
this.default_connection_color.output_on:r.color_off||this.default_connection_color_byTypeOff[m]||this.default_connection_color_byType[m]||this.default_connection_color.output_off,b.beginPath(),"array"==m&&(l=g.GRID_SHAPE),n=!0,m===g.EVENT||l===g.BOX_SHAPE?k?b.rect(u[0]-5+.5,u[1]-8+.5,10,14):b.rect(u[0]-6+.5,u[1]-5+.5,14,10):l===g.ARROW_SHAPE?(b.moveTo(u[0]+8,u[1]+.5),b.lineTo(u[0]-4,u[1]+6+.5),b.lineTo(u[0]-4,u[1]-6+.5),b.closePath()):l===g.GRID_SHAPE?(b.rect(u[0]-4,u[1]-4,2,2),b.rect(u[0]-1,u[1]-
4,2,2),b.rect(u[0]+2,u[1]-4,2,2),b.rect(u[0]-4,u[1]-1,2,2),b.rect(u[0]-1,u[1]-1,2,2),b.rect(u[0]+2,u[1]-1,2,2),b.rect(u[0]-4,u[1]+2,2,2),b.rect(u[0]-1,u[1]+2,2,2),b.rect(u[0]+2,u[1]+2,2,2),n=!1):e?b.rect(u[0]-4,u[1]-4,8,8):b.arc(u[0],u[1],4,0,2*Math.PI),b.fill(),!e&&n&&b.stroke(),d&&(m=null!=r.label?r.label:r.name))b.fillStyle=g.NODE_TEXT_COLOR,k||r.dir==g.DOWN?b.fillText(m,u[0],u[1]-8):b.fillText(m,u[0]-10,u[1]+5);b.textAlign="left";b.globalAlpha=1;if(a.widgets){r=p;if(k||a.widgets_up)r=2;null!=
...
g.ROUND_SHAPE,n=a.constructor.title_mode,t=!0;n==g.TRANSPARENT_TITLE||n==g.NO_TITLE?t=!1:n==g.AUTOHIDE_TITLE&&h&&(t=!0);x[0]=0;x[1]=t?-e:0;x[2]=c[0]+1;x[3]=t?c[1]+e:c[1];h=b.globalAlpha;b.beginPath();p==g.BOX_SHAPE||k?b.fillRect(x[0],x[1],x[2],x[3]):p==g.ROUND_SHAPE||p==g.CARD_SHAPE?b.roundRect(x[0],x[1],x[2],x[3],p==g.CARD_SHAPE?[this.round_radius,this.round_radius,0,0]:[this.round_radius]):p==g.CIRCLE_SHAPE&&b.arc(.5*c[0],.5*c[1],.5*c[0],0,2*Math.PI);b.fill();!a.flags.collapsed&&t&&(b.shadowColor=
"transparent",b.fillStyle="rgba(0,0,0,0.2)",b.fillRect(0,-1,x[2],2));b.shadowColor="transparent";if(a.onDrawBackground)a.onDrawBackground(b,this,this.canvas,this.graph_mouse);if(t||n==g.TRANSPARENT_TITLE){if(a.onDrawTitleBar)a.onDrawTitleBar(b,e,c,this.ds.scale,d);else if(n!=g.TRANSPARENT_TITLE&&(a.constructor.title_color||this.render_title_colored)){t=a.constructor.title_color||d;a.flags.collapsed&&(b.shadowColor=g.DEFAULT_SHADOW_COLOR);if(this.use_gradients){var r=m.gradients[t];r||(r=m.gradients[t]=
b.createLinearGradient(0,0,400,0),r.addColorStop(0,t),r.addColorStop(1,"#000"));b.fillStyle=r}else b.fillStyle=t;b.beginPath();p==g.BOX_SHAPE||k?b.rect(0,-e,c[0]+1,e):(p==g.ROUND_SHAPE||p==g.CARD_SHAPE)&&b.roundRect(0,-e,c[0]+1,e,a.flags.collapsed?[this.round_radius]:[this.round_radius,this.round_radius,0,0]);b.fill();b.shadowColor="transparent"}t=!1;g.node_box_coloured_by_mode&&g.NODE_MODES_COLORS[a.mode]&&(t=g.NODE_MODES_COLORS[a.mode]);g.node_box_coloured_when_on&&(t=a.action_triggered?"#FFF":
a.execute_triggered?"#AAA":t);if(a.onDrawTitleBox)a.onDrawTitleBox(b,e,c,this.ds.scale);else p==g.ROUND_SHAPE||p==g.CIRCLE_SHAPE||p==g.CARD_SHAPE?(k&&(b.fillStyle="black",b.beginPath(),b.arc(.5*e,-.5*e,6,0,2*Math.PI),b.fill()),b.fillStyle=a.boxcolor||t||g.NODE_DEFAULT_BOXCOLOR,k?b.fillRect(.5*e-5,-.5*e-5,10,10):(b.beginPath(),b.arc(.5*e,-.5*e,5,0,2*Math.PI),b.fill())):(k&&(b.fillStyle="black",b.fillRect(.5*(e-10)-1,-.5*(e+10)-1,12,12)),b.fillStyle=a.boxcolor||t||g.NODE_DEFAULT_BOXCOLOR,b.fillRect(.5*
...
(h=g.NODE_TITLE_HEIGHT,t=a.size[0]-h,r=g.isInsideRectangle(this.graph_mouse[0]-a.pos[0],this.graph_mouse[1]-a.pos[1],t+2,-h+2,h-4,h-4),b.fillStyle=r?"#888":"#555",p==g.BOX_SHAPE||k?b.fillRect(t+2,-h+2,h-4,h-4):(b.beginPath(),b.roundRect(t+2,-h+2,h-4,h-4,[4]),b.fill()),b.fillStyle="#333",b.beginPath(),b.moveTo(t+.2*h,.6*-h),b.lineTo(t+.8*h,.6*-h),b.lineTo(t+.5*h,.3*-h),b.fill());if(a.onDrawTitle)a.onDrawTitle(b)}if(f){if(a.onBounding)a.onBounding(x);n==g.TRANSPARENT_TITLE&&(x[1]-=e,x[3]+=e);b.lineWidth=
...
f.inputs[h];if(k&&null!=k.link&&(k=this.graph.links[k.link])){var p=this.graph.getNodeById(k.origin_id);if(null!=p){var n=k.origin_slot;var t=-1==n?[p.pos[0]+10,p.pos[1]+10]:p.getConnectionPos(!1,n,J);var r=f.getConnectionPos(!0,h,K);C[0]=t[0];C[1]=t[1];C[2]=r[0]-t[0];C[3]=r[1]-t[1];0>C[2]&&(C[0]+=C[2],C[2]=Math.abs(C[2]));0>C[3]&&(C[1]+=C[3],C[3]=Math.abs(C[3]));if(F(C,G)){var m=p.outputs[n];n=f.inputs[h];if(m&&n&&(p=m.dir||(p.horizontal?g.DOWN:g.RIGHT),n=n.dir||(f.horizontal?g.UP:g.LEFT),this.renderLink(a,
t,r,k,!1,0,null,p,n),k&&k._last_time&&1E3>b-k._last_time)){m=2-.002*(b-k._last_time);var l=a.globalAlpha;a.globalAlpha=l*m;this.renderLink(a,t,r,k,!0,m,"white",p,n);a.globalAlpha=l}}}}}}a.globalAlpha=1};m.prototype.renderLink=function(a,b,c,d,e,f,h,k,p,n){d&&this.visible_links.push(d);!h&&d&&(h=d.color||m.link_type_colors[d.type]);h||(h=this.default_link_color);null!=d&&this.highlighted_links[d.id]&&(h="#FFF");k=k||g.RIGHT;p=p||g.LEFT;var t=H(b,c);this.render_connections_border&&.6<this.ds.scale&&
(a.lineWidth=this.connections_width+4);a.lineJoin="round";n=n||1;1<n&&(a.lineWidth=.5);a.beginPath();for(var r=0;r<n;r+=1){var l=5*(r-.5*(n-1));if(this.links_render_mode==g.SPLINE_LINK){a.moveTo(b[0],b[1]+l);var q=0,u=0,v=0,w=0;switch(k){case g.LEFT:q=-.25*t;break;case g.RIGHT:q=.25*t;break;case g.UP:u=-.25*t;break;case g.DOWN:u=.25*t}switch(p){case g.LEFT:v=-.25*t;break;case g.RIGHT:v=.25*t;break;case g.UP:w=-.25*t;break;case g.DOWN:w=.25*t}a.bezierCurveTo(b[0]+q,b[1]+u+l,c[0]+v,c[1]+w+l,c[0],c[1]+
l)}else if(this.links_render_mode==g.LINEAR_LINK){a.moveTo(b[0],b[1]+l);w=v=u=q=0;switch(k){case g.LEFT:q=-1;break;case g.RIGHT:q=1;break;case g.UP:u=-1;break;case g.DOWN:u=1}switch(p){case g.LEFT:v=-1;break;case g.RIGHT:v=1;break;case g.UP:w=-1;break;case g.DOWN:w=1}a.lineTo(b[0]+15*q,b[1]+15*u+l);a.lineTo(c[0]+15*v,c[1]+15*w+l);a.lineTo(c[0],c[1]+l)}else if(this.links_render_mode==g.STRAIGHT_LINK)a.moveTo(b[0],b[1]),l=b[0],q=b[1],u=c[0],v=c[1],k==g.RIGHT?l+=10:q+=10,p==g.LEFT?u-=10:v-=10,a.lineTo(l,
q),a.lineTo(.5*(l+u),q),a.lineTo(.5*(l+u),v),a.lineTo(u,v),a.lineTo(c[0],c[1]);else return}this.render_connections_border&&.6<this.ds.scale&&!e&&(a.strokeStyle="rgba(0,0,0,0.5)",a.stroke());a.lineWidth=this.connections_width;a.fillStyle=a.strokeStyle=h;a.stroke();e=this.computeConnectionPoint(b,c,.5,k,p);d&&d._pos&&(d._pos[0]=e[0],d._pos[1]=e[1]);.6<=this.ds.scale&&this.highquality_render&&p!=g.CENTER&&(this.render_connection_arrows&&(r=this.computeConnectionPoint(b,c,.25,k,p),t=this.computeConnectionPoint(b,
c,.26,k,p),d=this.computeConnectionPoint(b,c,.75,k,p),n=this.computeConnectionPoint(b,c,.76,k,p),this.render_curved_connections?(t=-Math.atan2(t[0]-r[0],t[1]-r[1]),n=-Math.atan2(n[0]-d[0],n[1]-d[1])):n=t=c[1]>b[1]?0:Math.PI,a.save(),a.translate(r[0],r[1]),a.rotate(t),a.beginPath(),a.moveTo(-5,-3),a.lineTo(0,7),a.lineTo(5,-3),a.fill(),a.restore(),a.save(),a.translate(d[0],d[1]),a.rotate(n),a.beginPath(),a.moveTo(-5,-3),a.lineTo(0,7),a.lineTo(5,-3),a.fill(),a.restore()),a.beginPath(),a.arc(e[0],e[1],
5,0,2*Math.PI),a.fill());if(f)for(a.fillStyle=h,r=0;5>r;++r)f=(.001*g.getTime()+.2*r)%1,e=this.computeConnectionPoint(b,c,f,k,p),a.beginPath(),a.arc(e[0],e[1],5,0,2*Math.PI),a.fill()};m.prototype.computeConnectionPoint=function(a,b,c,d,e){d=d||g.RIGHT;e=e||g.LEFT;var f=H(a,b),h=[a[0],a[1]],k=[b[0],b[1]];switch(d){case g.LEFT:h[0]+=-.25*f;break;case g.RIGHT:h[0]+=.25*f;break;case g.UP:h[1]+=-.25*f;break;case g.DOWN:h[1]+=.25*f}switch(e){case g.LEFT:k[0]+=-.25*f;break;case g.RIGHT:k[0]+=.25*f;break;
...
value:c[k]});new g.ContextMenu(a,{event:b,callback:f,parentMenu:d,node:e});return!1}}if(e){var h=this;a=m.active_canvas.getCanvasWindow();b=e.optional_outputs;e.onGetOutputs&&(b=e.onGetOutputs());var k=[];if(b)for(var p=0;p<b.length;p++){var n=b[p];if(!n)k.push(null);else if(!e.flags||!e.flags.skip_repeated_outputs||-1==e.findOutputSlot(n[0])){var t=n[0];n[2]||(n[2]={});n[2].label&&(t=n[2].label);n[2].removable=!0;t={content:t,value:n};n[1]==g.EVENT&&(t.className="event");k.push(t)}}this.onMenuNodeOutputs&&
