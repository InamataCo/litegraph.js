...
$jscomp.polyfill("Object.values",function(x){return x?x:function(m){var t=[],l;for(l in m)$jscomp.owns(m,l)&&t.push(m[l]);return t}},"es8","es3");$jscomp.polyfill("String.prototype.startsWith",function(x){return x?x:function(m,t){var l=$jscomp.checkStringArgs(this,m,"startsWith");m+="";var w=l.length,A=m.length;t=Math.max(0,Math.min(t|0,l.length));for(var h=0;h<A&&t<w;)if(l[t++]!=m[h++])return!1;return h>=A}},"es6","es3");
...
[];this.viewport=c.viewport||null;b&&b.attachCanvas(this);this.setCanvas(a,c.skip_events);this.clear();c.skip_render||this.startRendering();this.autoresize=c.autoresize}function E(a,b){return Math.sqrt((b[0]-a[0])*(b[0]-a[0])+(b[1]-a[1])*(b[1]-a[1]))}function D(a,b,c,e,d,g){return c<a&&c+d>a&&e<b&&e+g>b?!0:!1}function B(a,b){var c=a[0]+a[2],e=a[1]+a[3],d=b[1]+b[3];return a[0]>b[0]+b[2]||a[1]>d||c<b[0]||e<b[1]?!1:!0}function F(a,b){function c(a){var c=parseInt(g.style.top);g.style.top=(c+a.deltaY*
...
k.width&&d>k.width-n.width-10&&(d=k.width-n.width-10),k.height&&a>k.height-n.height-10&&(a=k.height-n.height-10));g.style.left=d+"px";g.style.top=a+"px";b.scale&&(g.style.transform="scale("+b.scale+")")}function I(a){this.points=a;this.nearest=this.selected=-1;this.size=null;this.must_update=!0;this.margin=5}var f=x.LiteGraph={VERSION:.4,CANVAS_GRID_SIZE:10,NODE_TITLE_HEIGHT:30,NODE_TITLE_TEXT_Y:20,NODE_SLOT_HEIGHT:20,NODE_WIDGET_HEIGHT:20,NODE_WIDTH:140,NODE_MIN_WIDTH:50,NODE_COLLAPSED_RADIUS:10,
...
b.type=a;f.debug&&console.log("Node registered: "+a);var c=b.name,e=a.lastIndexOf("/");b.category=a.substring(0,e);b.title||(b.title=c);for(var d in l.prototype)b.prototype[d]||(b.prototype[d]=l.prototype[d]);(e=this.registered_node_types[a])&&console.log("replacing node type: "+a);if(!Object.prototype.hasOwnProperty.call(b.prototype,"shape")&&(Object.defineProperty(b.prototype,"shape",{set:function(a){switch(a){case "default":delete this._shape;break;case "box":this._shape=f.BOX_SHAPE;break;case "round":this._shape=
f.ROUND_SHAPE;break;case "circle":this._shape=f.CIRCLE_SHAPE;break;case "card":this._shape=f.CARD_SHAPE;break;default:this._shape=a}},get:function(){return this._shape},enumerable:!0,configurable:!0}),b.supported_extensions))for(var g in b.supported_extensions)(d=b.supported_extensions[g])&&d.constructor===String&&(this.node_types_by_file_extension[d.toLowerCase()]=b);this.registered_node_types[a]=b;b.constructor.name&&(this.Nodes[c]=b);if(f.onNodeTypeRegistered)f.onNodeTypeRegistered(a,b);if(e&&
...
[];this.inputs={};this.outputs={};this.change();this.sendActionToCanvas("clear")};m.prototype.attachCanvas=function(a){if(a.constructor!=h)throw"attachCanvas expects a LGraphCanvas instance";a.graph&&a.graph!=this&&a.graph.detachCanvas(a);a.graph=this;this.list_of_graphcanvas||(this.list_of_graphcanvas=[]);this.list_of_graphcanvas.push(a)};m.prototype.detachCanvas=function(a){if(this.list_of_graphcanvas){var b=this.list_of_graphcanvas.indexOf(a);-1!=b&&(a.graph=null,this.list_of_graphcanvas.splice(b,
1))}};m.prototype.start=function(a){if(this.status!=m.STATUS_RUNNING){this.status=m.STATUS_RUNNING;if(this.onPlayEvent)this.onPlayEvent();this.sendEventToAllNodes("onStart");this.last_update_time=this.starttime=f.getTime();a=a||0;var b=this;if(0==a&&"undefined"!=typeof window&&window.requestAnimationFrame){var c=function(){if(-1==b.execution_timer_id){window.requestAnimationFrame(c);if(b.onBeforeStep)b.onBeforeStep();b.runStep(1,!b.catch_errors);if(b.onAfterStep)b.onAfterStep()}};this.execution_timer_id=
-1;c()}else this.execution_timer_id=setInterval(function(){if(b.onBeforeStep)b.onBeforeStep();b.runStep(1,!b.catch_errors);if(b.onAfterStep)b.onAfterStep()},a)}};m.prototype.stop=function(){if(this.status!=m.STATUS_STOPPED){this.status=m.STATUS_STOPPED;if(this.onStopEvent)this.onStopEvent();null!=this.execution_timer_id&&(-1!=this.execution_timer_id&&clearInterval(this.execution_timer_id),this.execution_timer_id=null);this.sendEventToAllNodes("onStop")}};m.prototype.runStep=function(a,b,c){a=a||1;
var e=f.getTime();this.globaltime=.001*(e-this.starttime);var d=this._nodes_executable?this._nodes_executable:this._nodes;if(d){c=c||d.length;if(b){for(var g=0;g<a;g++){for(var k=0;k<c;++k){var n=d[k];n.mode==f.ALWAYS&&n.onExecute&&n.doExecute()}this.fixedtime+=this.fixedtime_lapse;if(this.onExecuteStep)this.onExecuteStep()}if(this.onAfterExecute)this.onAfterExecute()}else try{for(g=0;g<a;g++){for(k=0;k<c;++k)if(n=d[k],n.mode==f.ALWAYS&&n.onExecute)n.onExecute();this.fixedtime+=this.fixedtime_lapse;
if(this.onExecuteStep)this.onExecuteStep()}if(this.onAfterExecute)this.onAfterExecute();this.errors_in_execution=!1}catch(p){this.errors_in_execution=!0;if(f.throw_errors)throw p;f.debug&&console.log("Error during execution: "+p);this.stop()}a=f.getTime();e=a-e;0==e&&(e=1);this.execution_time=.001*e;this.globaltime+=.001*e;this.iteration+=1;this.elapsed_time=.001*(a-this.last_update_time);this.last_update_time=a;this.nodes_executing=[];this.nodes_actioning=[];this.nodes_executedAction=[]}};m.prototype.updateExecutionOrder=
function(){this._nodes_in_order=this.computeExecutionOrder(!1);this._nodes_executable=[];for(var a=0;a<this._nodes_in_order.length;++a)this._nodes_in_order[a].onExecute&&this._nodes_executable.push(this._nodes_in_order[a])};m.prototype.computeExecutionOrder=function(a,b){for(var c=[],e=[],d={},g={},k={},n=0,p=this._nodes.length;n<p;++n){var q=this._nodes[n];if(!a||q.onExecute){d[q.id]=q;var u=0;if(q.inputs)for(var r=0,y=q.inputs.length;r<y;r++)q.inputs[r]&&null!=q.inputs[r].link&&(u+=1);0==u?(e.push(q),
b&&(q._level=1)):(b&&(q._level=0),k[q.id]=u)}}for(;0!=e.length;)if(q=e.shift(),c.push(q),delete d[q.id],q.outputs)for(n=0;n<q.outputs.length;n++)if(a=q.outputs[n],null!=a&&null!=a.links&&0!=a.links.length)for(r=0;r<a.links.length;r++)(p=this.links[a.links[r]])&&!g[p.id]&&(u=this.getNodeById(p.target_id),null==u?g[p.id]=!0:(b&&(!u._level||u._level<=q._level)&&(u._level=q._level+1),g[p.id]=!0,--k[u.id],0==k[u.id]&&e.push(u)));for(n in d)c.push(d[n]);c.length!=this._nodes.length&&f.debug&&console.warn("something went wrong, nodes missing");
p=c.length;for(n=0;n<p;++n)c[n].order=n;c=c.sort(function(a,b){var c=a.constructor.priority||a.priority||0,d=b.constructor.priority||b.priority||0;return c==d?a.order-b.order:c-d});for(n=0;n<p;++n)c[n].order=n;return c};m.prototype.getAncestors=function(a){for(var b=[],c=[a],e={};c.length;){var d=c.shift();if(d.inputs){e[d.id]||d==a||(e[d.id]=!0,b.push(d));for(var g=0;g<d.inputs.length;++g){var k=d.getInputNode(g);k&&-1==b.indexOf(k)&&c.push(k)}}}b.sort(function(a,b){return a.order-b.order});return b};
m.prototype.arrange=function(a,b){a=a||100;for(var c=this.computeExecutionOrder(!1,!0),e=[],d=0;d<c.length;++d){var g=c[d],k=g._level||1;e[k]||(e[k]=[]);e[k].push(g)}c=a;for(d=0;d<e.length;++d)if(g=e[d]){k=100;for(var n=a+f.NODE_TITLE_HEIGHT,p=0;p<g.length;++p){var q=g[p];q.pos[0]=b==f.VERTICAL_LAYOUT?n:c;q.pos[1]=b==f.VERTICAL_LAYOUT?c:n;var u=b==f.VERTICAL_LAYOUT?1:0;q.size[u]>k&&(k=q.size[u]);n+=q.size[b==f.VERTICAL_LAYOUT?0:1]+a+f.NODE_TITLE_HEIGHT}c+=k+a}this.setDirtyCanvas(!0,!0)};m.prototype.getTime=
...
m.prototype.setDirtyCanvas=function(a,b){this.sendActionToCanvas("setDirty",[a,b])};m.prototype.removeLink=function(a){if(a=this.links[a]){var b=this.getNodeById(a.target_id);b&&b.disconnectInput(a.target_slot)}};m.prototype.serialize=function(){for(var a=[],b=0,c=this._nodes.length;b<c;++b)a.push(this._nodes[b].serialize());c=[];for(b in this.links){var e=this.links[b];if(!e.serialize){console.warn("weird LLink bug, link info is not a LLink but a regular object");var d=new t;for(g in e)d[g]=e[g];
e=this.links[b]=d}c.push(e.serialize())}var g=[];for(b=0;b<this._groups.length;++b)g.push(this._groups[b].serialize());a={last_node_id:this.last_node_id,last_link_id:this.last_link_id,nodes:a,links:c,groups:g,config:this.config,extra:this.extra,version:f.VERSION};if(this.onSerialize)this.onSerialize(a);return a};m.prototype.configure=function(a,b){if(a){b||this.clear();b=a.nodes;if(a.links&&a.links.constructor===Array){for(var c=[],e=0;e<a.links.length;++e){var d=a.links[e];if(d){var g=new t;g.configure(d);
c[g.id]=g}else console.warn("serialized graph link data contains errors, skipping.")}a.links=c}for(e in a)"nodes"!=e&&"groups"!=e&&(this[e]=a[e]);c=!1;this._nodes=[];if(b){e=0;for(d=b.length;e<d;++e){g=b[e];var k=f.createNode(g.type,g.title);k||(f.debug&&console.log("Node not found or has errors: "+g.type),k=new l,k.last_serialization=g,c=k.has_errors=!0);k.id=g.id;this.add(k,!0)}e=0;for(d=b.length;e<d;++e)g=b[e],(k=this.getNodeById(g.id))&&k.configure(g)}this._groups.length=0;if(a.groups)for(e=0;e<
a.groups.length;++e)b=new f.LGraphGroup,b.configure(a.groups[e]),this.add(b);this.updateExecutionOrder();this.extra=a.extra||{};if(this.onConfigure)this.onConfigure(a);this._version++;this.setDirtyCanvas(!0,!0);return c}};m.prototype.load=function(a,b){var c=this;if(a.constructor===File||a.constructor===Blob){var e=new FileReader;e.addEventListener("load",function(a){a=JSON.parse(a.target.result);c.configure(a);b&&b()});e.readAsText(a)}else{var d=new XMLHttpRequest;d.open("GET",a,!0);d.send(null);
...
a[b]);a.title||(this.title=this.constructor.title);if(this.inputs)for(c=0;c<this.inputs.length;++c){b=this.inputs[c];var e=this.graph?this.graph.links[b.link]:null;if(this.onConnectionsChange)this.onConnectionsChange(f.INPUT,c,!0,e,b);if(this.onInputAdded)this.onInputAdded(b)}if(this.outputs)for(c=0;c<this.outputs.length;++c){var d=this.outputs[c];if(d.links){for(b=0;b<d.links.length;++b)if(e=this.graph?this.graph.links[d.links[b]]:null,this.onConnectionsChange)this.onConnectionsChange(f.OUTPUT,c,
...
null:null};l.prototype.getInputOrProperty=function(a){if(!this.inputs||!this.inputs.length)return this.properties?this.properties[a]:null;for(var b=0,c=this.inputs.length;b<c;++b){var e=this.inputs[b];if(a==e.name&&null!=e.link&&(e=this.graph.links[e.link]))return e.data}return this.properties[a]};l.prototype.getOutputData=function(a){return!this.outputs||a>=this.outputs.length?null:this.outputs[a]._data};l.prototype.getOutputInfo=function(a){return this.outputs?a<this.outputs.length?this.outputs[a]:
...
f.auto_load_slot_types&&f.registerNodeAndSlotType(this,b,!0);this.setSize(this.computeSize());this.setDirtyCanvas(!0,!0);return a};l.prototype.addOutputs=function(a){for(var b=0;b<a.length;++b){var c=a[b],e={name:c[0],type:c[1],link:null};if(a[2])for(var d in c[2])e[d]=c[2][d];this.outputs||(this.outputs=[]);this.outputs.push(e);if(this.onOutputAdded)this.onOutputAdded(e);f.auto_load_slot_types&&f.registerNodeAndSlotType(this,c[1],!0)}this.setSize(this.computeSize());this.setDirtyCanvas(!0,!0)};l.prototype.removeOutput=
...
this.setSize(this.computeSize());if(this.onInputAdded)this.onInputAdded(a);f.registerNodeAndSlotType(this,b);this.setDirtyCanvas(!0,!0);return a};l.prototype.addInputs=function(a){for(var b=0;b<a.length;++b){var c=a[b],e={name:c[0],type:c[1],link:null};if(a[2])for(var d in c[2])e[d]=c[2][d];this.inputs||(this.inputs=[]);this.inputs.push(e);if(this.onInputAdded)this.onInputAdded(e);f.registerNodeAndSlotType(this,c[1])}this.setSize(this.computeSize());this.setDirtyCanvas(!0,!0)};l.prototype.removeInput=
function(a){this.disconnectInput(a);for(var b=this.inputs.splice(a,1),c=a;c<this.inputs.length;++c)if(this.inputs[c]){var e=this.graph.links[this.inputs[c].link];e&&--e.target_slot}this.setSize(this.computeSize());if(this.onInputRemoved)this.onInputRemoved(a,b[0]);this.setDirtyCanvas(!0,!0)};l.prototype.addConnection=function(a,b,c,e){a={name:a,type:b,pos:c,direction:e,links:null};this.connections.push(a);return a};l.prototype.computeSize=function(a){function b(a){return a?e*a.length*.6:0}if(this.constructor.size)return this.constructor.size.concat();
var c=Math.max(this.inputs?this.inputs.length:1,this.outputs?this.outputs.length:1);a=a||new Float32Array([0,0]);c=Math.max(c,1);var e=f.NODE_TEXT_SIZE,d=b(this.title),g=0,k=0;if(this.inputs)for(var n=0,p=this.inputs.length;n<p;++n){var q=this.inputs[n];q=q.label||q.name||"";q=b(q);g<q&&(g=q)}if(this.outputs)for(n=0,p=this.outputs.length;n<p;++n)q=this.outputs[n],q=q.label||q.name||"",q=b(q),k<q&&(k=q);a[0]=Math.max(g+k+10,d);a[0]=Math.max(a[0],f.NODE_WIDTH);this.widgets&&this.widgets.length&&(a[0]=
...
function(a,b){var c=new Float32Array(2);if(this.inputs)for(var e=0,d=this.inputs.length;e<d;++e){var g=this.inputs[e];this.getConnectionPos(!0,e,c);if(D(a,b,c[0]-10,c[1]-5,20,10))return{input:g,slot:e,link_pos:c}}if(this.outputs)for(e=0,d=this.outputs.length;e<d;++e)if(g=this.outputs[e],this.getConnectionPos(!1,e,c),D(a,b,c[0]-10,c[1]-5,20,10))return{output:g,slot:e,link_pos:c};return null};l.prototype.findInputSlot=function(a,b){if(!this.inputs)return-1;for(var c=0,e=this.inputs.length;c<e;++c)if(a==
...
function(a,b,c,e){return this.findSlotByType(!0,a,b,c,e)};l.prototype.findOutputSlotByType=function(a,b,c,e){return this.findSlotByType(!1,a,b,c,e)};l.prototype.findSlotByType=function(a,b,c,e,d){c=c||!1;e=e||!1;d=d||!1;a=a?this.inputs:this.outputs;if(!a)return-1;if(""==b||"*"==b)b=0;for(var g=0,k=a.length;g<k;++g){var n=(b+"").toLowerCase().split(","),p="0"==a[g].type||"*"==a[g].type?"0":a[g].type;p=(p+"").toLowerCase().split(",");for(var q=0;q<n.length;q++)for(var u=0;u<p.length;u++)if("_event_"==
n[q]&&(n[q]=f.EVENT),"_event_"==p[q]&&(p[q]=f.EVENT),"*"==n[q]&&(n[q]=0),"*"==p[q]&&(p[q]=0),!(n[q]!=p[u]||e&&a[g].links&&null!==a[g].links))return c?a[g]:g}if(e&&!d)for(g=0,k=a.length;g<k;++g)for(n=(b+"").toLowerCase().split(","),p="0"==a[g].type||"*"==a[g].type?"0":a[g].type,p=(p+"").toLowerCase().split(","),q=0;q<n.length;q++)for(u=0;u<p.length;u++)if("*"==n[q]&&(n[q]=0),"*"==p[q]&&(p[q]=0),n[q]==p[u])return c?a[g]:g;return-1};l.prototype.connectByType=function(a,b,c,e){e=e||{};e=Object.assign({createEventInCase:!0,
firstFreeIfOutputGeneralInCase:!0,generalTypeInCase:!0},e);b&&b.constructor===Number&&(b=this.graph.getNodeById(b));var d=b.findInputSlotByType(c,!1,!0);if(0<=d&&null!==d)return this.connect(a,b,d);if(e.createEventInCase&&c==f.EVENT)return this.connect(a,b,-1);if(e.generalTypeInCase&&(d=b.findInputSlotByType(0,!1,!0,!0),0<=d)||e.firstFreeIfOutputGeneralInCase&&(0==c||"*"==c||""==c)&&(d=b.findInputSlotFree({typesNotAccepted:[f.EVENT]}),0<=d))return this.connect(a,b,d);console.debug("no way to connect type: ",
c," to targetNODE ",b);return null};l.prototype.connectByTypeOutput=function(a,b,c,e){e=e||{};e=Object.assign({createEventInCase:!0,firstFreeIfInputGeneralInCase:!0,generalTypeInCase:!0},e);b&&b.constructor===Number&&(b=this.graph.getNodeById(b));var d=b.findOutputSlotByType(c,!1,!0);if(0<=d&&null!==d||e.generalTypeInCase&&(d=b.findOutputSlotByType(0,!1,!0,!0),0<=d))return b.connect(d,this,a);if(e.createEventInCase&&c==f.EVENT&&f.do_add_triggers_slots)return d=b.addOnExecutedOutput(),b.connect(d,
this,a);if(e.firstFreeIfInputGeneralInCase&&(0==c||"*"==c||""==c)&&(d=b.findOutputSlotFree({typesNotAccepted:[f.EVENT]}),0<=d))return b.connect(d,this,a);console.debug("no way to connect byOUT type: ",c," to sourceNODE ",b);return null};l.prototype.connect=function(a,b,c){c=c||0;if(!this.graph)return console.log("Connect: Error, node doesn't belong to any graph. Nodes must be added first to a graph before connecting them."),null;if(a.constructor===String){if(a=this.findOutputSlot(a),-1==a)return f.debug&&
console.log("Connect: Error, no slot of name "+a),null}else if(!this.outputs||a>=this.outputs.length)return f.debug&&console.log("Connect: Error, slot number not found"),null;b&&b.constructor===Number&&(b=this.graph.getNodeById(b));if(!b)throw"target node is null";if(b==this)return null;if(c.constructor===String){if(c=b.findInputSlot(c),-1==c)return f.debug&&console.log("Connect: Error, no slot of name "+c),null}else if(c===f.EVENT)if(f.do_add_triggers_slots)b.changeMode(f.ON_TRIGGER),c=b.findInputSlot("onTrigger");
else return null;else if(!b.inputs||c>=b.inputs.length)return f.debug&&console.log("Connect: Error, slot number not found"),null;var e=b.inputs[c],d=this.outputs[a];if(!this.outputs[a])return null;b.onBeforeConnectInput&&(c=b.onBeforeConnectInput(c));if(!1===c||null===c||!f.isValidConnection(d.type,e.type))return this.setDirtyCanvas(!1,!0),null;if(b.onConnectInput&&!1===b.onConnectInput(c,d.type,d,this,a)||this.onConnectOutput&&!1===this.onConnectOutput(a,e.type,e,b,c))return null;b.inputs[c]&&null!=
b.inputs[c].link&&(this.graph.beforeChange(),b.disconnectInput(c,{doProcessChange:!1}));if(null!==d.links&&d.links.length)switch(d.type){case f.EVENT:f.allow_multi_output_for_events||(this.graph.beforeChange(),this.disconnectOutput(a,!1,{doProcessChange:!1}))}var g=new t(++this.graph.last_link_id,e.type||d.type,this.id,a,b.id,c);this.graph.links[g.id]=g;null==d.links&&(d.links=[]);d.links.push(g.id);b.inputs[c].link=g.id;this.graph&&this.graph._version++;if(this.onConnectionsChange)this.onConnectionsChange(f.OUTPUT,
a,!0,g,d);if(b.onConnectionsChange)b.onConnectionsChange(f.INPUT,c,!0,g,e);this.graph&&this.graph.onNodeConnectionChange&&(this.graph.onNodeConnectionChange(f.INPUT,b,c,this,a),this.graph.onNodeConnectionChange(f.OUTPUT,this,a,b,c));this.setDirtyCanvas(!1,!0);this.graph.afterChange();this.graph.connectionChange(this,g);return g};l.prototype.disconnectOutput=function(a,b){if(a.constructor===String){if(a=this.findOutputSlot(a),-1==a)return f.debug&&console.log("Connect: Error, no slot of name "+a),
!1}else if(!this.outputs||a>=this.outputs.length)return f.debug&&console.log("Connect: Error, slot number not found"),!1;var c=this.outputs[a];if(!c||!c.links||0==c.links.length)return!1;if(b){b.constructor===Number&&(b=this.graph.getNodeById(b));if(!b)throw"Target Node not found";for(var e=0,d=c.links.length;e<d;e++){var g=c.links[e],k=this.graph.links[g];if(k.target_id==b.id){c.links.splice(e,1);var n=b.inputs[k.target_slot];n.link=null;delete this.graph.links[g];this.graph&&this.graph._version++;
...
this.graph&&this.graph._version++;if(b){n=b.inputs[k.target_slot];n.link=null;if(b.onConnectionsChange)b.onConnectionsChange(f.INPUT,k.target_slot,!1,k,n);if(this.graph&&this.graph.onNodeConnectionChange)this.graph.onNodeConnectionChange(f.INPUT,b,k.target_slot)}delete this.graph.links[g];if(this.onConnectionsChange)this.onConnectionsChange(f.OUTPUT,a,!1,k,c);this.graph&&this.graph.onNodeConnectionChange&&(this.graph.onNodeConnectionChange(f.OUTPUT,this,a),this.graph.onNodeConnectionChange(f.INPUT,
b,k.target_slot))}c.links=null}this.setDirtyCanvas(!1,!0);this.graph.connectionChange(this);return!0};l.prototype.disconnectInput=function(a){if(a.constructor===String){if(a=this.findInputSlot(a),-1==a)return f.debug&&console.log("Connect: Error, no slot of name "+a),!1}else if(!this.inputs||a>=this.inputs.length)return f.debug&&console.log("Connect: Error, slot number not found"),!1;var b=this.inputs[a];if(!b)return!1;var c=this.inputs[a].link;if(null!=c){this.inputs[a].link=null;var e=this.graph.links[c];
if(e){var d=this.graph.getNodeById(e.origin_id);if(!d)return!1;var g=d.outputs[e.origin_slot];if(!g||!g.links||0==g.links.length)return!1;for(var k=0,n=g.links.length;k<n;k++)if(g.links[k]==c){g.links.splice(k,1);break}delete this.graph.links[c];this.graph&&this.graph._version++;if(this.onConnectionsChange)this.onConnectionsChange(f.INPUT,a,!1,e,b);if(d.onConnectionsChange)d.onConnectionsChange(f.OUTPUT,k,!1,e,g);this.graph&&this.graph.onNodeConnectionChange&&(this.graph.onNodeConnectionChange(f.OUTPUT,
d,k),this.graph.onNodeConnectionChange(f.INPUT,this,a))}}this.setDirtyCanvas(!1,!0);this.graph&&this.graph.connectionChange(this);return!0};l.prototype.getConnectionPos=function(a,b,c){c=c||new Float32Array(2);var e=0;a&&this.inputs&&(e=this.inputs.length);!a&&this.outputs&&(e=this.outputs.length);var d=.5*f.NODE_SLOT_HEIGHT;if(this.flags.collapsed)return b=this._collapsed_width||f.NODE_COLLAPSED_WIDTH,this.horizontal?(c[0]=this.pos[0]+.5*b,c[1]=a?this.pos[1]-f.NODE_TITLE_HEIGHT:this.pos[1]):(c[0]=
a?this.pos[0]:this.pos[0]+b,c[1]=this.pos[1]-.5*f.NODE_TITLE_HEIGHT),c;if(a&&-1==b)return c[0]=this.pos[0]+.5*f.NODE_TITLE_HEIGHT,c[1]=this.pos[1]+.5*f.NODE_TITLE_HEIGHT,c;if(a&&e>b&&this.inputs[b].pos)return c[0]=this.pos[0]+this.inputs[b].pos[0],c[1]=this.pos[1]+this.inputs[b].pos[1],c;if(!a&&e>b&&this.outputs[b].pos)return c[0]=this.pos[0]+this.outputs[b].pos[0],c[1]=this.pos[1]+this.outputs[b].pos[1],c;if(this.horizontal)return c[0]=this.pos[0]+this.size[0]/e*(b+.5),c[1]=a?this.pos[1]-f.NODE_TITLE_HEIGHT:
this.pos[1]+this.size[1],c;c[0]=a?this.pos[0]+d:this.pos[0]+this.size[0]+1-d;c[1]=this.pos[1]+(b+.7)*f.NODE_SLOT_HEIGHT+(this.constructor.slot_start_y||0);return c};l.prototype.alignToGrid=function(){this.pos[0]=f.CANVAS_GRID_SIZE*Math.round(this.pos[0]/f.CANVAS_GRID_SIZE);this.pos[1]=f.CANVAS_GRID_SIZE*Math.round(this.pos[1]/f.CANVAS_GRID_SIZE)};l.prototype.trace=function(a){this.console||(this.console=[]);this.console.push(a);this.console.length>l.MAX_CONSOLE&&this.console.shift();if(this.graph.onNodeTrace)this.graph.onNodeTrace(this,
...
enumerable:!0})};w.prototype.configure=function(a){this.title=a.title;this._bounding.set(a.bounding);this.color=a.color;this.font=a.font};w.prototype.serialize=function(){var a=this._bounding;return{title:this.title,bounding:[Math.round(a[0]),Math.round(a[1]),Math.round(a[2]),Math.round(a[3])],color:this.color,font:this.font}};w.prototype.move=function(a,b,c){this._pos[0]+=a;this._pos[1]+=b;if(!c)for(c=0;c<this._nodes.length;++c){var e=this._nodes[c];e.pos[0]+=a;e.pos[1]+=b}};w.prototype.recomputeInsideNodes=
function(){this._nodes.length=0;for(var a=this.graph._nodes,b=new Float32Array(4),c=0;c<a.length;++c){var e=a[c];e.getBounding(b);B(this._bounding,b)&&this._nodes.push(e)}};w.prototype.isPointInside=l.prototype.isPointInside;w.prototype.setDirtyCanvas=l.prototype.setDirtyCanvas;f.DragAndScale=A;A.prototype.bindEvents=function(a){this.last_mouse=new Float32Array(2);this._binded_mouse_callback=this.onMouse.bind(this);f.pointerListenerAdd(a,"down",this._binded_mouse_callback);f.pointerListenerAdd(a,
"move",this._binded_mouse_callback);f.pointerListenerAdd(a,"up",this._binded_mouse_callback);a.addEventListener("mousewheel",this._binded_mouse_callback,!1);a.addEventListener("wheel",this._binded_mouse_callback,!1)};A.prototype.computeVisibleArea=function(a){if(this.element){var b=this.element.width,c=this.element.height,e=-this.offset[0],d=-this.offset[1];a&&(e+=a[0]/this.scale,d+=a[1]/this.scale,b=a[2],c=a[3]);a=e+b/this.scale;c=d+c/this.scale;this.visible_area[0]=e;this.visible_area[1]=d;this.visible_area[2]=
a-e;this.visible_area[3]=c-d}else this.visible_area[0]=this.visible_area[1]=this.visible_area[2]=this.visible_area[3]=0};A.prototype.onMouse=function(a){if(this.enabled){var b=this.element,c=b.getBoundingClientRect(),e=a.clientX-c.left;c=a.clientY-c.top;a.canvasx=e;a.canvasy=c;a.dragging=this.dragging;var d=!this.viewport||this.viewport&&e>=this.viewport[0]&&e<this.viewport[0]+this.viewport[2]&&c>=this.viewport[1]&&c<this.viewport[1]+this.viewport[3],g=!1;this.onmouse&&(g=this.onmouse(a));a.type==
...
"up",this._binded_mouse_callback),f.pointerListenerAdd(b,"move",this._binded_mouse_callback)):!d||"mousewheel"!=a.type&&"wheel"!=a.type&&"DOMMouseScroll"!=a.type||(a.eventType="mousewheel",a.wheel="wheel"==a.type?-a.deltaY:null!=a.wheelDeltaY?a.wheelDeltaY:-60*a.detail,a.delta=a.wheelDelta?a.wheelDelta/40:a.deltaY?-a.deltaY/3:0,this.changeDeltaScale(1+.05*a.delta));this.last_mouse[0]=e;this.last_mouse[1]=c;if(d)return a.preventDefault(),a.stopPropagation(),!1}};A.prototype.toCanvasContext=function(a){a.scale(this.scale,
this.scale);a.translate(this.offset[0],this.offset[1])};A.prototype.convertOffsetToCanvas=function(a){return[(a[0]+this.offset[0])*this.scale,(a[1]+this.offset[1])*this.scale]};A.prototype.convertCanvasToOffset=function(a,b){b=b||[0,0];b[0]=a[0]/this.scale-this.offset[0];b[1]=a[1]/this.scale-this.offset[1];return b};A.prototype.mouseDrag=function(a,b){this.offset[0]+=a/this.scale;this.offset[1]+=b/this.scale;if(this.onredraw)this.onredraw(this)};A.prototype.changeScale=function(a,b){a<this.min_scale?
...
1!=this.onMouse(a)){if(1!=a.which||this.pointer_is_double)if(2==a.which){if(f.middle_click_slot_add_default_node&&g&&this.allow_interaction&&!e&&!this.read_only&&!this.connecting_node&&!g.flags.collapsed&&!this.live_mode){d=e=k=!1;if(g.outputs)for(u=0,n=g.outputs.length;u<n;++u)if(p=g.outputs[u],q=g.getConnectionPos(!1,u),D(a.canvasX,a.canvasY,q[0]-15,q[1]-10,30,20)){k=p;e=u;d=!0;break}if(g.inputs)for(u=0,n=g.inputs.length;u<n;++u)if(p=g.inputs[u],q=g.getConnectionPos(!0,u),D(a.canvasX,a.canvasY,
q[0]-15,q[1]-10,30,20)){k=p;e=u;d=!1;break}k&&!1!==e&&(u=.5-(e+1)/(d?g.outputs.length:g.inputs.length),k=g.getBounding(),this.createDefaultNodeForSlot({nodeFrom:d?g:null,slotFrom:d?e:null,nodeTo:d?null:g,slotTo:d?null:e,position:[d?k[0]+k[2]:k[0],a.canvasY-80],nodeType:"AUTO",posAdd:[d?30:-30,130*-u],posSizeFix:[d?0:-1,0]}))}}else 3!=a.which&&!this.pointer_is_double||!this.allow_interaction||e||this.read_only||(g&&(Object.keys(this.selected_nodes).length&&(this.selected_nodes[g.id]||a.shiftKey||a.ctrlKey||
a.metaKey)?this.selected_nodes[g.id]||this.selectNodes([g],!0):this.selectNodes([g])),this.processContextMenu(g,a));else{a.ctrlKey&&(this.dragging_rectangle=new Float32Array(4),this.dragging_rectangle[0]=a.canvasX,this.dragging_rectangle[1]=a.canvasY,this.dragging_rectangle[2]=1,this.dragging_rectangle[3]=1,e=!0);f.alt_drag_do_clone_nodes&&a.altKey&&g&&this.allow_interaction&&!e&&!this.read_only&&(cloned=g.clone())&&(cloned.pos[0]+=5,cloned.pos[1]+=5,this.graph.add(cloned,!1,{doCalcSize:!1}),g=cloned,
e=!0,u||(this.allow_dragnodes&&(this.graph.beforeChange(),this.node_dragged=g),this.selected_nodes[g.id]||this.processNodeSelected(g,a)));k=!1;if(g&&this.allow_interaction&&!e&&!this.read_only){this.live_mode||g.flags.pinned||this.bringToFront(g);if(!this.connecting_node&&!g.flags.collapsed&&!this.live_mode)if(!e&&!1!==g.resizable&&D(a.canvasX,a.canvasY,g.pos[0]+g.size[0]-5,g.pos[1]+g.size[1]-5,10,10))this.graph.beforeChange(),this.resizing_node=g,this.canvas.style.cursor="se-resize",e=!0;else{if(g.outputs){u=
0;for(var n=g.outputs.length;u<n;++u){var p=g.outputs[u],q=g.getConnectionPos(!1,u);if(D(a.canvasX,a.canvasY,q[0]-15,q[1]-10,30,20)){this.connecting_node=g;this.connecting_output=p;this.connecting_output.slot_index=u;this.connecting_pos=g.getConnectionPos(!1,u);this.connecting_slot=u;f.shift_click_do_break_link_from&&a.shiftKey&&g.disconnectOutput(u);if(d){if(g.onOutputDblClick)g.onOutputDblClick(u,a)}else if(g.onOutputClick)g.onOutputClick(u,a);e=!0;break}}}if(g.inputs)for(u=0,n=g.inputs.length;u<
n;++u)if(p=g.inputs[u],q=g.getConnectionPos(!0,u),D(a.canvasX,a.canvasY,q[0]-15,q[1]-10,30,20)){if(d){if(g.onInputDblClick)g.onInputDblClick(u,a)}else if(g.onInputClick)g.onInputClick(u,a);null!==p.link&&(q=this.graph.links[p.link],f.click_do_break_link_to&&(g.disconnectInput(u),e=this.dirty_bgcanvas=!0),this.allow_reconnect_links||a.shiftKey)&&(f.click_do_break_link_to||g.disconnectInput(u),this.connecting_node=this.graph._nodes_by_id[q.origin_id],this.connecting_slot=q.origin_slot,this.connecting_output=
this.connecting_node.outputs[this.connecting_slot],this.connecting_pos=this.connecting_node.getConnectionPos(!1,this.connecting_slot),e=this.dirty_bgcanvas=!0);e||(this.connecting_node=g,this.connecting_input=p,this.connecting_input.slot_index=u,this.connecting_pos=g.getConnectionPos(!0,u),this.connecting_slot=u,e=this.dirty_bgcanvas=!0)}}if(!e){var u=!1;n=[a.canvasX-g.pos[0],a.canvasY-g.pos[1]];if(q=this.processNodeWidgets(g,this.graph_mouse,a))u=!0,this.node_widget=[g,q];if(d&&this.selected_nodes[g.id]){if(g.onDblClick)g.onDblClick(a,
n,this);this.processNodeDblClicked(g);u=!0}g.onMouseDown&&g.onMouseDown(a,n,this)?u=!0:(g.subgraph&&!g.skip_subgraph_button&&!g.flags.collapsed&&n[0]>g.size[0]-f.NODE_TITLE_HEIGHT&&0>n[1]&&(c=this,setTimeout(function(){c.openSubgraph(g.subgraph)},10)),this.live_mode&&(u=k=!0));u?g.is_selected||this.processNodeSelected(g,a):(this.allow_dragnodes&&(this.graph.beforeChange(),this.node_dragged=g),this.processNodeSelected(g,a));this.dirty_canvas=!0}}else if(!e){if(!this.read_only)for(u=0;u<this.visible_links.length;++u)if(k=
...
-1!=g&&e.outputs[g]?(k=e.outputs[g].type,f.isValidConnection(this.connecting_input.type,k)&&(this._highlight_output=d)):this._highlight_output=null));this.canvas&&(D(a.canvasX,a.canvasY,e.pos[0]+e.size[0]-5,e.pos[1]+e.size[1]-5,5,5)?this.canvas.style.cursor="se-resize":this.canvas.style.cursor="crosshair")}else{d=null;for(b=0;b<this.visible_links.length;++b)if(g=this.visible_links[b],k=g._pos,!(!k||a.canvasX<k[0]-4||a.canvasX>k[0]+4||a.canvasY<k[1]-4||a.canvasY>k[1]+4)){d=g;break}d!=this.over_link_center&&
...
!1;var e=this.graph.getNodeOnPos(a.canvasX,a.canvasY,this.visible_nodes);if(this.dragging_rectangle){if(this.graph){c=this.graph._nodes;var d=new Float32Array(4),g=Math.abs(this.dragging_rectangle[2]),k=Math.abs(this.dragging_rectangle[3]),n=0>this.dragging_rectangle[3]?this.dragging_rectangle[1]-k:this.dragging_rectangle[1];this.dragging_rectangle[0]=0>this.dragging_rectangle[2]?this.dragging_rectangle[0]-g:this.dragging_rectangle[0];this.dragging_rectangle[1]=n;this.dragging_rectangle[2]=g;this.dragging_rectangle[3]=
k;if(!e||10<g&&10<k){e=[];for(g=0;g<c.length;++g)k=c[g],k.getBounding(d),B(this.dragging_rectangle,d)&&e.push(k);e.length&&this.selectNodes(e,a.shiftKey)}else this.selectNodes([e],a.shiftKey||a.ctrlKey)}this.dragging_rectangle=null}else if(this.connecting_node)this.dirty_bgcanvas=this.dirty_canvas=!0,c=(this.connecting_output||this.connecting_input).type,e?this.connecting_output?(d=this.isOverNodeInput(e,a.canvasX,a.canvasY),-1!=d?this.connecting_node.connect(this.connecting_slot,e,d):this.connecting_node.connectByType(this.connecting_slot,
e,c)):this.connecting_input&&(d=this.isOverNodeOutput(e,a.canvasX,a.canvasY),-1!=d?e.connect(d,this.connecting_node,this.connecting_slot):this.connecting_node.connectByTypeOutput(this.connecting_slot,e,c)):f.release_link_on_empty_shows_menu&&(a.shiftKey&&this.allow_searchbox?this.connecting_output?this.showSearchBox(a,{node_from:this.connecting_node,slot_from:this.connecting_output,type_filter_in:this.connecting_output.type}):this.connecting_input&&this.showSearchBox(a,{node_to:this.connecting_node,
...
c>=this.viewport[0]&&c<this.viewport[0]+this.viewport[2]&&e>=this.viewport[1]&&e<this.viewport[1]+this.viewport[3])return c=this.ds.scale,0<b?c*=1.1:0>b&&(c*=1/1.1),this.ds.changeScale(c,[a.clientX,a.clientY]),this.graph.change(),a.preventDefault(),!1}};h.prototype.isOverNodeBox=function(a,b,c){var e=f.NODE_TITLE_HEIGHT;return D(b,c,a.pos[0]+2,a.pos[1]+2-e,e-4,e-4)?!0:!1};h.prototype.isOverNodeInput=function(a,b,c,e){if(a.inputs)for(var d=0,g=a.inputs.length;d<g;++d){var k=a.getConnectionPos(!0,d);
if(a.horizontal?D(b,c,k[0]-5,k[1]-10,10,20):D(b,c,k[0]-10,k[1]-5,40,10))return e&&(e[0]=k[0],e[1]=k[1]),d}return-1};h.prototype.isOverNodeOutput=function(a,b,c,e){if(a.outputs)for(var d=0,g=a.outputs.length;d<g;++d){var k=a.getConnectionPos(!1,d);if(a.horizontal?D(b,c,k[0]-5,k[1]-10,10,20):D(b,c,k[0]-10,k[1]-5,40,10))return e&&(e[0]=k[0],e[1]=k[1]),d}return-1};h.prototype.processKey=function(a){if(this.graph){var b=!1;if("input"!=a.target.localName){if("keydown"==a.type){if(32==a.keyCode&&(b=this.dragging_canvas=
...
d.inputs.length)for(b=0;b<d.inputs.length;++b){var g=d.inputs[b];if(g&&null!=g.link&&(g=this.graph.links[g.link])){var k=this.graph.getNodeById(g.origin_id);k&&a.links.push([k._relative_id,g.origin_slot,d._relative_id,g.target_slot,k.id])}}}else console.warn("node type not found: "+d.type);localStorage.setItem("litegrapheditor_clipboard",JSON.stringify(a))};h.prototype.pasteFromClipboard=function(a){a=void 0===a?!1:a;if(f.ctrl_shift_v_paste_connect_unselected_outputs||!a){var b=localStorage.getItem("litegrapheditor_clipboard");
...
b.links[d];g=c[0];if(null!=g)var n=e[g];else f.ctrl_shift_v_paste_connect_unselected_outputs&&a&&(g=c[4])&&(n=this.graph.getNodeById(g));g=e[c[2]];n&&g?n.connect(c[1],g,c[3]):console.warn("Warning, nodes missing on pasting")}this.selectNodes(e);this.graph.afterChange()}}};h.prototype.processDrop=function(a){a.preventDefault();this.adjustMouseEvent(a);var b=a.clientX,c=a.clientY;if(!this.viewport||this.viewport&&b>=this.viewport[0]&&b<this.viewport[0]+this.viewport[2]&&c>=this.viewport[1]&&c<this.viewport[1]+
this.viewport[3]){b=[a.canvasX,a.canvasY];var e=this.graph?this.graph.getNodeOnPos(b[0],b[1]):null;if(e){if((e.onDropFile||e.onDropData)&&(b=a.dataTransfer.files)&&b.length)for(c=0;c<b.length;c++){var d=a.dataTransfer.files[0],g=d.name;h.getFileExtension(g);if(e.onDropFile)e.onDropFile(d);if(e.onDropData){var k=new FileReader;k.onload=function(a){e.onDropData(a.target.result,g,d)};var f=d.type.split("/")[0];"text"==f||""==f?k.readAsText(d):"image"==f?k.readAsDataURL(d):k.readAsArrayBuffer(d)}}return e.onDropItem&&
...
h.prototype.deselectNode=function(a){if(a.is_selected){if(a.onDeselected)a.onDeselected();a.is_selected=!1;if(this.onNodeDeselected)this.onNodeDeselected(a);if(a.inputs)for(var b=0;b<a.inputs.length;++b)delete this.highlighted_links[a.inputs[b].link];if(a.outputs)for(b=0;b<a.outputs.length;++b){var c=a.outputs[b];if(c.links)for(var e=0;e<c.links.length;++e)delete this.highlighted_links[c.links[e]]}}};h.prototype.deselectAllNodes=function(){if(this.graph){for(var a=this.graph._nodes,b=0,c=a.length;b<
c;++b){var e=a[b];if(e.is_selected){if(e.onDeselected)e.onDeselected();e.is_selected=!1;if(this.onNodeDeselected)this.onNodeDeselected(e)}}this.selected_nodes={};this.current_node=null;this.highlighted_links={};if(this.onSelectionChange)this.onSelectionChange(this.selected_nodes);this.setDirty(!0)}};h.prototype.deleteSelectedNodes=function(){this.graph.beforeChange();for(var a in this.selected_nodes){var b=this.selected_nodes[a];if(!b.block_delete){if(b.inputs&&b.inputs.length&&b.outputs&&b.outputs.length&&
...
"left";e=c.measureText("Graph Outputs").width;c.fillText("Graph Outputs",d-e-20,34);if(this.drawButton(d-200,20,20,20,"X","#151515"))this.closeSubgraph();else{e=50;c.font="14px Arial";if(b.outputs)for(var k=0;k<b.outputs.length;++k){var n=b.outputs[k];if(!n.not_subgraph_input){if(this.drawButton(d-200,e+2,180,g-2)){var p=b.constructor.output_node_type||"graph/output";this.graph.beforeChange();var q=f.createNode(p);q?(a.add(q),this.block_click=!1,this.last_click_position=null,this.selectNodes([q]),
this.node_dragged=q,this.dragging_canvas=!1,q.setProperty("name",n.name),q.setProperty("type",n.type),this.node_dragged.pos[0]=this.graph_mouse[0]-5,this.node_dragged.pos[1]=this.graph_mouse[1]-5,this.graph.afterChange()):console.error("graph input node not found:",p)}c.fillStyle="#9C9";c.beginPath();c.arc(d-200+16,e+.5*g,5,0,2*Math.PI);c.fill();c.fillStyle="#AAA";c.fillText(n.name,d-200+30,e+.75*g);c.fillStyle="#777";c.fillText(n.type,d-200+130,e+.75*g);e+=g}}this.drawButton(d-200,e+2,180,g-2,"+",
"#151515","#222")&&this.showSubgraphPropertiesDialogRight(b)}};h.prototype.drawButton=function(a,b,c,e,d,g,k,n){var p=this.ctx;g=g||f.NODE_DEFAULT_COLOR;k=k||"#555";n=n||f.NODE_TEXT_COLOR;var q=b+f.NODE_TITLE_HEIGHT+2,u=this.mouse,r=f.isInsideRectangle(u[0],u[1],a,q,c,e);q=(u=this.last_click_position)&&f.isInsideRectangle(u[0],u[1],a,q,c,e);p.fillStyle=r?k:g;q&&(p.fillStyle="#AAA");p.beginPath();p.roundRect(a,b,c,e,[4]);p.fill();null!=d&&d.constructor==String&&(p.fillStyle=n,p.textAlign="center",
p.font=(.65*e|0)+"px Arial",p.fillText(d,a+.5*c,b+.75*e),p.textAlign="left");a=q&&!this.block_click;q&&this.blockClick();return a};h.prototype.isAreaClicked=function(a,b,c,e,d){var g=this.mouse;f.isInsideRectangle(g[0],g[1],a,b,c,e);b=(a=(g=this.last_click_position)&&f.isInsideRectangle(g[0],g[1],a,b,c,e))&&!this.block_click;a&&d&&this.blockClick();return b};h.prototype.renderInfo=function(a,b,c){b=b||10;c=c||this.canvas.height-80;a.save();a.translate(b,c);a.font="10px Arial";a.fillStyle="#888";a.textAlign=
...
this._graph_stack[d]._subgraph_node.getTitle()+" >> ";b.fillText(e+c.getTitle(),.5*a.width,40);b.restore()}c=!1;this.onRenderBackground&&(c=this.onRenderBackground(a,b));this.viewport||(b.restore(),b.setTransform(1,0,0,1,0,0));this.visible_links.length=0;if(this.graph){b.save();this.ds.toCanvasContext(b);1.5>this.ds.scale&&!c&&this.clear_background_color&&(b.fillStyle=this.clear_background_color,b.fillRect(this.visible_area[0],this.visible_area[1],this.visible_area[2],this.visible_area[3]));if(this.background_image&&
.5<this.ds.scale&&!c){b.globalAlpha=this.zoom_modify_alpha?(1-.5/this.ds.scale)*this.editor_alpha:this.editor_alpha;b.imageSmoothingEnabled=b.imageSmoothingEnabled=!1;if(!this._bg_img||this._bg_img.name!=this.background_image){this._bg_img=new Image;this._bg_img.name=this.background_image;this._bg_img.src=this.background_image;var g=this;this._bg_img.onload=function(){g.draw(!0,!0)}}c=null;null==this._pattern&&0<this._bg_img.width?(c=b.createPattern(this._bg_img,"repeat"),this._pattern_img=this._bg_img,
this._pattern=c):c=this._pattern;c&&(b.fillStyle=c,b.fillRect(this.visible_area[0],this.visible_area[1],this.visible_area[2],this.visible_area[3]),b.fillStyle="transparent");b.globalAlpha=1;b.imageSmoothingEnabled=b.imageSmoothingEnabled=!0}this.graph._groups.length&&!this.live_mode&&this.drawGroups(a,b);if(this.onDrawBackground)this.onDrawBackground(b,this.visible_area);this.onBackgroundRender&&(console.error("WARNING! onBackgroundRender deprecated, now is named onDrawBackground "),this.onBackgroundRender=
...
this)){var k=a._shape||f.BOX_SHAPE;H.set(a.size);var n=a.horizontal;if(a.flags.collapsed){b.font=this.inner_text_font;var p=a.getTitle?a.getTitle():a.title;null!=p&&(a._collapsed_width=Math.min(a.size[0],b.measureText(p).width+2*f.NODE_TITLE_HEIGHT),H[0]=a._collapsed_width,H[1]=0)}a.clip_area&&(b.save(),b.beginPath(),k==f.BOX_SHAPE?b.rect(0,0,H[0],H[1]):k==f.ROUND_SHAPE?b.roundRect(0,0,H[0],H[1],[10]):k==f.CIRCLE_SHAPE&&b.arc(.5*H[0],.5*H[1],.5*H[0],0,2*Math.PI),b.clip());a.has_errors&&(e="red");
this.drawNodeShape(a,b,H,c,e,a.is_selected,a.mouseOver);b.shadowColor="transparent";if(a.onDrawForeground)a.onDrawForeground(b,this,this.canvas);b.textAlign=n?"center":"left";b.font=this.inner_text_font;e=!d;var q=this.connecting_output;k=this.connecting_input;b.lineWidth=1;p=0;var u=new Float32Array(2);if(!a.flags.collapsed){if(a.inputs)for(c=0;c<a.inputs.length;c++){var r=a.inputs[c],y=r.type,h=r.shape;b.globalAlpha=g;this.connecting_output&&!f.isValidConnection(r.type,q.type)&&(b.globalAlpha=.4*
g);b.fillStyle=null!=r.link?r.color_on||this.default_connection_color_byType[y]||this.default_connection_color.input_on:r.color_off||this.default_connection_color_byTypeOff[y]||this.default_connection_color_byType[y]||this.default_connection_color.input_off;var v=a.getConnectionPos(!0,c,u);v[0]-=a.pos[0];v[1]-=a.pos[1];p<v[1]+.5*f.NODE_SLOT_HEIGHT&&(p=v[1]+.5*f.NODE_SLOT_HEIGHT);b.beginPath();"array"==y&&(h=f.GRID_SHAPE);r.type===f.EVENT||r.shape===f.BOX_SHAPE?n?b.rect(v[0]-5+.5,v[1]-8+.5,10,14):
b.rect(v[0]-6+.5,v[1]-5+.5,14,10):h===f.ARROW_SHAPE?(b.moveTo(v[0]+8,v[1]+.5),b.lineTo(v[0]-4,v[1]+6+.5),b.lineTo(v[0]-4,v[1]-6+.5),b.closePath()):h===f.GRID_SHAPE?(b.rect(v[0]-4,v[1]-4,2,2),b.rect(v[0]-1,v[1]-4,2,2),b.rect(v[0]+2,v[1]-4,2,2),b.rect(v[0]-4,v[1]-1,2,2),b.rect(v[0]-1,v[1]-1,2,2),b.rect(v[0]+2,v[1]-1,2,2),b.rect(v[0]-4,v[1]+2,2,2),b.rect(v[0]-1,v[1]+2,2,2),b.rect(v[0]+2,v[1]+2,2,2)):d?b.rect(v[0]-4,v[1]-4,8,8):b.arc(v[0],v[1],4,0,2*Math.PI);b.fill();e&&(y=null!=r.label?r.label:r.name)&&
(b.fillStyle=f.NODE_TEXT_COLOR,n||r.dir==f.UP?b.fillText(y,v[0],v[1]-10):b.fillText(y,v[0]+10,v[1]+5))}b.textAlign=n?"center":"right";b.strokeStyle="black";if(a.outputs)for(c=0;c<a.outputs.length;c++)if(r=a.outputs[c],y=r.type,h=r.shape,this.connecting_input&&!f.isValidConnection(y,k.type)&&(b.globalAlpha=.4*g),v=a.getConnectionPos(!1,c,u),v[0]-=a.pos[0],v[1]-=a.pos[1],p<v[1]+.5*f.NODE_SLOT_HEIGHT&&(p=v[1]+.5*f.NODE_SLOT_HEIGHT),b.fillStyle=r.links&&r.links.length?r.color_on||this.default_connection_color_byType[y]||
this.default_connection_color.output_on:r.color_off||this.default_connection_color_byTypeOff[y]||this.default_connection_color_byType[y]||this.default_connection_color.output_off,b.beginPath(),"array"==y&&(h=f.GRID_SHAPE),q=!0,y===f.EVENT||h===f.BOX_SHAPE?n?b.rect(v[0]-5+.5,v[1]-8+.5,10,14):b.rect(v[0]-6+.5,v[1]-5+.5,14,10):h===f.ARROW_SHAPE?(b.moveTo(v[0]+8,v[1]+.5),b.lineTo(v[0]-4,v[1]+6+.5),b.lineTo(v[0]-4,v[1]-6+.5),b.closePath()):h===f.GRID_SHAPE?(b.rect(v[0]-4,v[1]-4,2,2),b.rect(v[0]-1,v[1]-
4,2,2),b.rect(v[0]+2,v[1]-4,2,2),b.rect(v[0]-4,v[1]-1,2,2),b.rect(v[0]-1,v[1]-1,2,2),b.rect(v[0]+2,v[1]-1,2,2),b.rect(v[0]-4,v[1]+2,2,2),b.rect(v[0]-1,v[1]+2,2,2),b.rect(v[0]+2,v[1]+2,2,2),q=!1):d?b.rect(v[0]-4,v[1]-4,8,8):b.arc(v[0],v[1],4,0,2*Math.PI),b.fill(),!d&&q&&b.stroke(),e&&(y=null!=r.label?r.label:r.name))b.fillStyle=f.NODE_TEXT_COLOR,n||r.dir==f.DOWN?b.fillText(y,v[0],v[1]-8):b.fillText(y,v[0]-10,v[1]+5);b.textAlign="left";b.globalAlpha=1;if(a.widgets){r=p;if(n||a.widgets_up)r=2;null!=
a.widgets_start_y&&(r=a.widgets_start_y);this.drawNodeWidgets(a,r,b,this.node_widget&&this.node_widget[0]==a?this.node_widget[1]:null)}}else if(this.render_collapsed_slots){d=g=null;if(a.inputs)for(c=0;c<a.inputs.length;c++)if(r=a.inputs[c],null!=r.link){g=r;break}if(a.outputs)for(c=0;c<a.outputs.length;c++)r=a.outputs[c],r.links&&r.links.length&&(d=r);g&&(g=0,c=-.5*f.NODE_TITLE_HEIGHT,n&&(g=.5*a._collapsed_width,c=-f.NODE_TITLE_HEIGHT),b.fillStyle="#686",b.beginPath(),r.type===f.EVENT||r.shape===
f.BOX_SHAPE?b.rect(g-7+.5,c-4,14,8):r.shape===f.ARROW_SHAPE?(b.moveTo(g+8,c),b.lineTo(g+-4,c-4),b.lineTo(g+-4,c+4),b.closePath()):b.arc(g,c,4,0,2*Math.PI),b.fill());d&&(g=a._collapsed_width,c=-.5*f.NODE_TITLE_HEIGHT,n&&(g=.5*a._collapsed_width,c=0),b.fillStyle="#686",b.strokeStyle="black",b.beginPath(),r.type===f.EVENT||r.shape===f.BOX_SHAPE?b.rect(g-7+.5,c-4,14,8):r.shape===f.ARROW_SHAPE?(b.moveTo(g+6,c),b.lineTo(g-6,c-4),b.lineTo(g-6,c+4),b.closePath()):b.arc(g,c,4,0,2*Math.PI),b.fill())}a.clip_area&&
b.restore();b.globalAlpha=1}}};h.prototype.drawLinkTooltip=function(a,b){var c=b._pos;a.fillStyle="black";a.beginPath();a.arc(c[0],c[1],3,0,2*Math.PI);a.fill();if(null!=b.data&&(!this.onDrawLinkTooltip||1!=this.onDrawLinkTooltip(a,b,this))&&(b=b.data,b=b.constructor===Number?b.toFixed(2):b.constructor===String?'"'+b+'"':b.constructor===Boolean?String(b):b.toToolTip?b.toToolTip():"["+b.constructor.name+"]",null!=b)){b=b.substr(0,30);a.font="14px Courier New";var e=a.measureText(b).width+20;a.shadowColor=
"black";a.shadowOffsetX=2;a.shadowOffsetY=2;a.shadowBlur=3;a.fillStyle="#454";a.beginPath();a.roundRect(c[0]-.5*e,c[1]-15-24,e,24,[3]);a.moveTo(c[0]-10,c[1]-15);a.lineTo(c[0]+10,c[1]-15);a.lineTo(c[0],c[1]-5);a.fill();a.shadowColor="transparent";a.textAlign="center";a.fillStyle="#CEC";a.fillText(b,c[0],c[1]-15-24*.3)}};var C=new Float32Array(4);h.prototype.drawNodeShape=function(a,b,c,e,d,g,k){b.strokeStyle=e;b.fillStyle=d;d=f.NODE_TITLE_HEIGHT;var n=.5>this.ds.scale,p=a._shape||a.constructor.shape||
f.ROUND_SHAPE,q=a.constructor.title_mode,u=!0;q==f.TRANSPARENT_TITLE||q==f.NO_TITLE?u=!1:q==f.AUTOHIDE_TITLE&&k&&(u=!0);C[0]=0;C[1]=u?-d:0;C[2]=c[0]+1;C[3]=u?c[1]+d:c[1];k=b.globalAlpha;b.beginPath();p==f.BOX_SHAPE||n?b.fillRect(C[0],C[1],C[2],C[3]):p==f.ROUND_SHAPE||p==f.CARD_SHAPE?b.roundRect(C[0],C[1],C[2],C[3],p==f.CARD_SHAPE?[this.round_radius,this.round_radius,0,0]:[this.round_radius]):p==f.CIRCLE_SHAPE&&b.arc(.5*c[0],.5*c[1],.5*c[0],0,2*Math.PI);b.fill();!a.flags.collapsed&&u&&(b.shadowColor=
"transparent",b.fillStyle="rgba(0,0,0,0.2)",b.fillRect(0,-1,C[2],2));b.shadowColor="transparent";if(a.onDrawBackground)a.onDrawBackground(b,this,this.canvas,this.graph_mouse);if(u||q==f.TRANSPARENT_TITLE){if(a.onDrawTitleBar)a.onDrawTitleBar(b,d,c,this.ds.scale,e);else if(q!=f.TRANSPARENT_TITLE&&(a.constructor.title_color||this.render_title_colored)){u=a.constructor.title_color||e;a.flags.collapsed&&(b.shadowColor=f.DEFAULT_SHADOW_COLOR);if(this.use_gradients){var r=h.gradients[u];r||(r=h.gradients[u]=
b.createLinearGradient(0,0,400,0),r.addColorStop(0,u),r.addColorStop(1,"#000"));b.fillStyle=r}else b.fillStyle=u;b.beginPath();p==f.BOX_SHAPE||n?b.rect(0,-d,c[0]+1,d):(p==f.ROUND_SHAPE||p==f.CARD_SHAPE)&&b.roundRect(0,-d,c[0]+1,d,a.flags.collapsed?[this.round_radius]:[this.round_radius,this.round_radius,0,0]);b.fill();b.shadowColor="transparent"}u=!1;f.node_box_coloured_by_mode&&f.NODE_MODES_COLORS[a.mode]&&(u=f.NODE_MODES_COLORS[a.mode]);f.node_box_coloured_when_on&&(u=a.action_triggered?"#FFF":
a.execute_triggered?"#AAA":u);if(a.onDrawTitleBox)a.onDrawTitleBox(b,d,c,this.ds.scale);else p==f.ROUND_SHAPE||p==f.CIRCLE_SHAPE||p==f.CARD_SHAPE?(n&&(b.fillStyle="black",b.beginPath(),b.arc(.5*d,-.5*d,6,0,2*Math.PI),b.fill()),b.fillStyle=a.boxcolor||u||f.NODE_DEFAULT_BOXCOLOR,n?b.fillRect(.5*d-5,-.5*d-5,10,10):(b.beginPath(),b.arc(.5*d,-.5*d,5,0,2*Math.PI),b.fill())):(n&&(b.fillStyle="black",b.fillRect(.5*(d-10)-1,-.5*(d+10)-1,12,12)),b.fillStyle=a.boxcolor||u||f.NODE_DEFAULT_BOXCOLOR,b.fillRect(.5*
(d-10),-.5*(d+10),10,10));b.globalAlpha=k;if(a.onDrawTitleText)a.onDrawTitleText(b,d,c,this.ds.scale,this.title_text_font,g);!n&&(b.font=this.title_text_font,k=String(a.getTitle()))&&(b.fillStyle=g?f.NODE_SELECTED_TITLE_COLOR:a.constructor.title_text_color||this.node_title_color,a.flags.collapsed?(b.textAlign="left",b.measureText(k),b.fillText(k.substr(0,20),d,f.NODE_TITLE_TEXT_Y-d),b.textAlign="left"):(b.textAlign="left",b.fillText(k,d,f.NODE_TITLE_TEXT_Y-d)));a.flags.collapsed||!a.subgraph||a.skip_subgraph_button||
(k=f.NODE_TITLE_HEIGHT,u=a.size[0]-k,r=f.isInsideRectangle(this.graph_mouse[0]-a.pos[0],this.graph_mouse[1]-a.pos[1],u+2,-k+2,k-4,k-4),b.fillStyle=r?"#888":"#555",p==f.BOX_SHAPE||n?b.fillRect(u+2,-k+2,k-4,k-4):(b.beginPath(),b.roundRect(u+2,-k+2,k-4,k-4,[4]),b.fill()),b.fillStyle="#333",b.beginPath(),b.moveTo(u+.2*k,.6*-k),b.lineTo(u+.8*k,.6*-k),b.lineTo(u+.5*k,.3*-k),b.fill());if(a.onDrawTitle)a.onDrawTitle(b)}if(g){if(a.onBounding)a.onBounding(C);q==f.TRANSPARENT_TITLE&&(C[1]-=d,C[3]+=d);b.lineWidth=
...
0<a.action_triggered&&a.action_triggered--};var L=new Float32Array(4),G=new Float32Array(4),K=new Float32Array(2),J=new Float32Array(2);h.prototype.drawConnections=function(a){var b=f.getTime(),c=this.visible_area;L[0]=c[0]-20;L[1]=c[1]-20;L[2]=c[2]+40;L[3]=c[3]+40;a.lineWidth=this.connections_width;a.fillStyle="#AAA";a.strokeStyle="#AAA";a.globalAlpha=this.editor_alpha;c=this.graph._nodes;for(var e=0,d=c.length;e<d;++e){var g=c[e];if(g.inputs&&g.inputs.length)for(var k=0;k<g.inputs.length;++k){var n=
g.inputs[k];if(n&&null!=n.link&&(n=this.graph.links[n.link])){var p=this.graph.getNodeById(n.origin_id);if(null!=p){var q=n.origin_slot;var u=-1==q?[p.pos[0]+10,p.pos[1]+10]:p.getConnectionPos(!1,q,K);var r=g.getConnectionPos(!0,k,J);G[0]=u[0];G[1]=u[1];G[2]=r[0]-u[0];G[3]=r[1]-u[1];0>G[2]&&(G[0]+=G[2],G[2]=Math.abs(G[2]));0>G[3]&&(G[1]+=G[3],G[3]=Math.abs(G[3]));if(B(G,L)){var y=p.outputs[q];q=g.inputs[k];if(y&&q&&(p=y.dir||(p.horizontal?f.DOWN:f.RIGHT),q=q.dir||(g.horizontal?f.UP:f.LEFT),this.renderLink(a,
u,r,n,!1,0,null,p,q),n&&n._last_time&&1E3>b-n._last_time)){y=2-.002*(b-n._last_time);var h=a.globalAlpha;a.globalAlpha=h*y;this.renderLink(a,u,r,n,!0,y,"white",p,q);a.globalAlpha=h}}}}}}a.globalAlpha=1};h.prototype.renderLink=function(a,b,c,e,d,g,k,n,p,q){e&&this.visible_links.push(e);!k&&e&&(k=e.color||h.link_type_colors[e.type]);k||(k=this.default_link_color);null!=e&&this.highlighted_links[e.id]&&(k="#FFF");n=n||f.RIGHT;p=p||f.LEFT;var u=E(b,c);this.render_connections_border&&.6<this.ds.scale&&
(a.lineWidth=this.connections_width+4);a.lineJoin="round";q=q||1;1<q&&(a.lineWidth=.5);a.beginPath();for(var r=0;r<q;r+=1){var y=5*(r-.5*(q-1));if(this.links_render_mode==f.SPLINE_LINK){a.moveTo(b[0],b[1]+y);var l=0,v=0,m=0,w=0;switch(n){case f.LEFT:l=-.25*u;break;case f.RIGHT:l=.25*u;break;case f.UP:v=-.25*u;break;case f.DOWN:v=.25*u}switch(p){case f.LEFT:m=-.25*u;break;case f.RIGHT:m=.25*u;break;case f.UP:w=-.25*u;break;case f.DOWN:w=.25*u}a.bezierCurveTo(b[0]+l,b[1]+v+y,c[0]+m,c[1]+w+y,c[0],c[1]+
...
l),a.lineTo(.5*(y+v),l),a.lineTo(.5*(y+v),m),a.lineTo(v,m),a.lineTo(c[0],c[1]);else return}this.render_connections_border&&.6<this.ds.scale&&!d&&(a.strokeStyle="rgba(0,0,0,0.5)",a.stroke());a.lineWidth=this.connections_width;a.fillStyle=a.strokeStyle=k;a.stroke();d=this.computeConnectionPoint(b,c,.5,n,p);e&&e._pos&&(e._pos[0]=d[0],e._pos[1]=d[1]);.6<=this.ds.scale&&this.highquality_render&&p!=f.CENTER&&(this.render_connection_arrows&&(r=this.computeConnectionPoint(b,c,.25,n,p),u=this.computeConnectionPoint(b,
c,.26,n,p),e=this.computeConnectionPoint(b,c,.75,n,p),q=this.computeConnectionPoint(b,c,.76,n,p),this.render_curved_connections?(u=-Math.atan2(u[0]-r[0],u[1]-r[1]),q=-Math.atan2(q[0]-e[0],q[1]-e[1])):q=u=c[1]>b[1]?0:Math.PI,a.save(),a.translate(r[0],r[1]),a.rotate(u),a.beginPath(),a.moveTo(-5,-3),a.lineTo(0,7),a.lineTo(5,-3),a.fill(),a.restore(),a.save(),a.translate(e[0],e[1]),a.rotate(q),a.beginPath(),a.moveTo(-5,-3),a.lineTo(0,7),a.lineTo(5,-3),a.fill(),a.restore()),a.beginPath(),a.arc(d[0],d[1],
5,0,2*Math.PI),a.fill());if(g)for(a.fillStyle=k,r=0;5>r;++r)g=(.001*f.getTime()+.2*r)%1,d=this.computeConnectionPoint(b,c,g,n,p),a.beginPath(),a.arc(d[0],d[1],5,0,2*Math.PI),a.fill()};h.prototype.computeConnectionPoint=function(a,b,c,e,d){e=e||f.RIGHT;d=d||f.LEFT;var g=E(a,b),k=[a[0],a[1]],n=[b[0],b[1]];switch(e){case f.LEFT:k[0]+=-.25*g;break;case f.RIGHT:k[0]+=.25*g;break;case f.UP:k[1]+=-.25*g;break;case f.DOWN:k[1]+=.25*g}switch(d){case f.LEFT:n[0]+=-.25*g;break;case f.RIGHT:n[0]+=.25*g;break;
case f.UP:n[1]+=-.25*g;break;case f.DOWN:n[1]+=.25*g}e=(1-c)*(1-c)*(1-c);d=3*(1-c)*(1-c)*c;g=3*(1-c)*c*c;c*=c*c;return[e*a[0]+d*k[0]+g*n[0]+c*b[0],e*a[1]+d*k[1]+g*n[1]+c*b[1]]};h.prototype.drawExecutionOrder=function(a){a.shadowColor="transparent";a.globalAlpha=.25;a.textAlign="center";a.strokeStyle="white";a.globalAlpha=.75;for(var b=this.visible_nodes,c=0;c<b.length;++c){var e=b[c];a.fillStyle="black";a.fillRect(e.pos[0]-f.NODE_TITLE_HEIGHT,e.pos[1]-f.NODE_TITLE_HEIGHT,f.NODE_TITLE_HEIGHT,f.NODE_TITLE_HEIGHT);
0==e.order&&a.strokeRect(e.pos[0]-f.NODE_TITLE_HEIGHT+.5,e.pos[1]-f.NODE_TITLE_HEIGHT+.5,f.NODE_TITLE_HEIGHT,f.NODE_TITLE_HEIGHT);a.fillStyle="#FFF";a.fillText(e.order,e.pos[0]+-.5*f.NODE_TITLE_HEIGHT,e.pos[1]-6)}a.globalAlpha=1};h.prototype.drawNodeWidgets=function(a,b,c,e){if(!a.widgets||!a.widgets.length)return 0;var d=a.size[0],g=a.widgets;b+=2;var k=f.NODE_WIDGET_HEIGHT,n=.5<this.ds.scale;c.save();c.globalAlpha=this.editor_alpha;for(var p=f.WIDGET_OUTLINE_COLOR,q=f.WIDGET_BGCOLOR,u=f.WIDGET_TEXT_COLOR,
r=f.WIDGET_SECONDARY_TEXT_COLOR,y=0;y<g.length;++y){var h=g[y],v=b;h.y&&(v=h.y);h.last_y=v;c.strokeStyle=p;c.fillStyle="#222";c.textAlign="left";h.disabled&&(c.globalAlpha*=.5);var l=h.width||d;switch(h.type){case "button":h.clicked&&(c.fillStyle="#AAA",h.clicked=!1,this.dirty_canvas=!0);c.fillRect(15,v,l-30,k);n&&!h.disabled&&c.strokeRect(15,v,l-30,k);n&&(c.textAlign="center",c.fillStyle=u,c.fillText(h.name,.5*l,v+.7*k));break;case "toggle":c.textAlign="left";c.strokeStyle=p;c.fillStyle=q;c.beginPath();
n?c.roundRect(15,v,l-30,k,[.5*k]):c.rect(15,v,l-30,k);c.fill();n&&!h.disabled&&c.stroke();c.fillStyle=h.value?"#89A":"#333";c.beginPath();c.arc(l-30,v+.5*k,.36*k,0,2*Math.PI);c.fill();n&&(c.fillStyle=r,null!=h.name&&c.fillText(h.name,30,v+.7*k),c.fillStyle=h.value?u:r,c.textAlign="right",c.fillText(h.value?h.options.on||"true":h.options.off||"false",l-40,v+.7*k));break;case "slider":c.fillStyle=q;c.fillRect(15,v,l-30,k);var m=h.options.max-h.options.min,w=(h.value-h.options.min)/m;0>w&&(w=0);1<w&&
(w=1);c.fillStyle=h.options.hasOwnProperty("slider_color")?h.options.slider_color:e==h?"#89A":"#678";c.fillRect(15,v,w*(l-30),k);n&&!h.disabled&&c.strokeRect(15,v,l-30,k);h.marker&&(m=(h.marker-h.options.min)/m,0>m&&(m=0),1<m&&(m=1),c.fillStyle=h.options.hasOwnProperty("marker_color")?h.options.marker_color:"#AA9",c.fillRect(15+m*(l-30),v,2,k));n&&(c.textAlign="center",c.fillStyle=u,c.fillText(h.name+"  "+Number(h.value).toFixed(3),.5*l,v+.7*k));break;case "number":case "combo":c.textAlign="left";
c.strokeStyle=p;c.fillStyle=q;c.beginPath();n?c.roundRect(15,v,l-30,k,[.5*k]):c.rect(15,v,l-30,k);c.fill();n&&(h.disabled||c.stroke(),c.fillStyle=u,h.disabled||(c.beginPath(),c.moveTo(31,v+5),c.lineTo(21,v+.5*k),c.lineTo(31,v+k-5),c.fill(),c.beginPath(),c.moveTo(l-15-16,v+5),c.lineTo(l-15-6,v+.5*k),c.lineTo(l-15-16,v+k-5),c.fill()),c.fillStyle=r,c.fillText(h.name,35,v+.7*k),c.fillStyle=u,c.textAlign="right","number"==h.type?c.fillText(Number(h.value).toFixed(void 0!==h.options.precision?h.options.precision:
3),l-30-20,v+.7*k):(m=h.value,h.options.values&&(w=h.options.values,w.constructor===Function&&(w=w()),w&&w.constructor!==Array&&(m=w[h.value])),c.fillText(m,l-30-20,v+.7*k)));break;case "string":case "text":c.textAlign="left";c.strokeStyle=p;c.fillStyle=q;c.beginPath();n?c.roundRect(15,v,l-30,k,[.5*k]):c.rect(15,v,l-30,k);c.fill();n&&(h.disabled||c.stroke(),c.save(),c.beginPath(),c.rect(15,v,l-30,k),c.clip(),c.fillStyle=r,null!=h.name&&c.fillText(h.name,30,v+.7*k),c.fillStyle=u,c.textAlign="right",
c.fillText(String(h.value).substr(0,30),l-30,v+.7*k),c.restore());break;default:h.draw&&h.draw(c,a,l,v,k)}b+=(h.computeSize?h.computeSize(l)[1]:k)+4;c.globalAlpha=this.editor_alpha}c.restore();c.textAlign="left"};h.prototype.processNodeWidgets=function(a,b,c,e){function d(d,e){"number"==d.type&&(e=Number(e));d.value=e;d.options&&d.options.property&&void 0!==a.properties[d.options.property]&&a.setProperty(d.options.property,e);d.callback&&d.callback(d.value,h,a,b,c)}if(!a.widgets||!a.widgets.length)return null;
for(var g=b[0]-a.pos[0],k=b[1]-a.pos[1],n=a.size[0],h=this,q=this.getCanvasWindow(),u=0;u<a.widgets.length;++u){var r=a.widgets[u];if(r&&!r.disabled){var y=r.computeSize?r.computeSize(n)[1]:f.NODE_WIDGET_HEIGHT,l=r.width||n;if(r==e||!(6>g||g>l-12||k<r.last_y||k>r.last_y+y||void 0===r.last_y)){e=r.value;switch(r.type){case "button":c.type===f.pointerevents_method+"down"&&(r.callback&&setTimeout(function(){r.callback(r,h,a,b,c)},20),this.dirty_canvas=r.clicked=!0);break;case "slider":e=r.value;q=Math.clamp((g-
15)/(l-30),0,1);if(r.options.read_only)break;r.value=r.options.min+(r.options.max-r.options.min)*q;e!=r.value&&setTimeout(function(){d(r,r.value)},20);this.dirty_canvas=!0;break;case "number":case "combo":e=r.value;if(c.type==f.pointerevents_method+"move"&&"number"==r.type)c.deltaX&&(r.value+=.1*c.deltaX*(r.options.step||1)),null!=r.options.min&&r.value<r.options.min&&(r.value=r.options.min),null!=r.options.max&&r.value>r.options.max&&(r.value=r.options.max);else if(c.type==f.pointerevents_method+
"down"){var v=r.options.values;v&&v.constructor===Function&&(v=r.options.values(r,a));var m=null;"number"!=r.type&&(m=v.constructor===Array?v:Object.keys(v));g=40>g?-1:g>l-40?1:0;if("number"==r.type)r.value+=.1*g*(r.options.step||1),null!=r.options.min&&r.value<r.options.min&&(r.value=r.options.min),null!=r.options.max&&r.value>r.options.max&&(r.value=r.options.max);else if(g)q=-1,this.last_mouseclick=0,q=v.constructor===Object?m.indexOf(String(r.value))+g:m.indexOf(r.value)+g,q>=m.length&&(q=m.length-
1),0>q&&(q=0),r.value=v.constructor===Array?v[q]:q;else{var w=v!=m?Object.values(v):v;new f.ContextMenu(w,{scale:Math.max(1,this.ds.scale),event:c,className:"dark",callback:function(a,b,c){v!=m&&(a=w.indexOf(a));this.value=a;d(this,a);h.dirty_canvas=!0;return!1}.bind(r)},q)}}else c.type==f.pointerevents_method+"up"&&"number"==r.type&&(g=40>g?-1:g>l-40?1:0,200>c.click_time&&0==g&&this.prompt("Value",r.value,function(a){if(/^[0-9+\-*/()\s]+$/.test(a))try{a=eval(a)}catch(O){}this.value=Number(a);d(this,
this.value)}.bind(r),c));e!=r.value&&setTimeout(function(){d(this,this.value)}.bind(r),20);this.dirty_canvas=!0;break;case "toggle":c.type==f.pointerevents_method+"down"&&(r.value=!r.value,setTimeout(function(){d(r,r.value)},20));break;case "string":case "text":c.type==f.pointerevents_method+"down"&&this.prompt("Value",r.value,function(a){d(this,a)}.bind(r),c,r.options?r.options.multiline:!1);break;default:r.mouse&&(this.dirty_canvas=r.mouse(c,[g,k],a))}if(e!=r.value){if(a.onWidgetChanged)a.onWidgetChanged(r.name,
r.value,e,r);a.graph._version++}return r}}}return null};h.prototype.drawGroups=function(a,b){if(this.graph){a=this.graph._groups;b.save();b.globalAlpha=.5*this.editor_alpha;for(var c=0;c<a.length;++c){var e=a[c];if(B(this.visible_area,e._bounding)){b.fillStyle=e.color||"#335";b.strokeStyle=e.color||"#335";var d=e._pos,g=e._size;b.globalAlpha=.25*this.editor_alpha;b.beginPath();b.rect(d[0]+.5,d[1]+.5,g[0],g[1]);b.fill();b.globalAlpha=this.editor_alpha;b.stroke();b.beginPath();b.moveTo(d[0]+g[0],d[1]+
...
{event:c,parentMenu:b},n)}var k=h.active_canvas,n=k.getCanvasWindow(),p=k.graph;if(p)return g("",e),!1};h.onMenuCollapseAll=function(){};h.onMenuNodeEdit=function(){};h.showMenuNodeOptionalInputs=function(a,b,c,e,d){if(d){var g=this;a=h.active_canvas.getCanvasWindow();b=d.optional_inputs;d.onGetInputs&&(b=d.onGetInputs());var k=[];if(b)for(var n=0;n<b.length;n++){var p=b[n];if(p){var q=p[0];p[2]||(p[2]={});p[2].label&&(q=p[2].label);p[2].removable=!0;q={content:q,value:p};p[1]==f.ACTION&&(q.className=
"event");k.push(q)}else k.push(null)}d.onMenuNodeInputs&&(b=d.onMenuNodeInputs(k))&&(k=b);if(k.length)return new f.ContextMenu(k,{event:c,callback:function(a,b,c){if(d&&(a.callback&&a.callback.call(g,d,a,b,c),a.value)){d.graph.beforeChange();d.addInput(a.value[0],a.value[1],a.value[2]);if(d.onNodeInputAdd)d.onNodeInputAdd(a.value);d.setDirtyCanvas(!0,!0);d.graph.afterChange()}},parentMenu:e,node:d},a),!1;console.log("no input entries")}};h.showMenuNodeOptionalOutputs=function(a,b,c,e,d){function g(a,
b,c){if(d&&(a.callback&&a.callback.call(k,d,a,b,c),a.value))if(c=a.value[1],!c||c.constructor!==Object&&c.constructor!==Array){d.graph.beforeChange();d.addOutput(a.value[0],a.value[1],a.value[2]);if(d.onNodeOutputAdd)d.onNodeOutputAdd(a.value);d.setDirtyCanvas(!0,!0);d.graph.afterChange()}else{a=[];for(var q in c)a.push({content:q,value:c[q]});new f.ContextMenu(a,{event:b,callback:g,parentMenu:e,node:d});return!1}}if(d){var k=this;a=h.active_canvas.getCanvasWindow();b=d.optional_outputs;d.onGetOutputs&&
(b=d.onGetOutputs());var n=[];if(b)for(var p=0;p<b.length;p++){var q=b[p];if(!q)n.push(null);else if(!d.flags||!d.flags.skip_repeated_outputs||-1==d.findOutputSlot(q[0])){var u=q[0];q[2]||(q[2]={});q[2].label&&(u=q[2].label);q[2].removable=!0;u={content:u,value:q};q[1]==f.EVENT&&(u.className="event");n.push(u)}}this.onMenuNodeOutputs&&(n=this.onMenuNodeOutputs(n));f.do_add_triggers_slots&&-1==d.findOutputSlot("onExecuted")&&n.push({content:"On Executed",value:["onExecuted",f.EVENT,{nameLocked:!0}],
...
var n=new f.ContextMenu(["Add Node",null,"Delete",null],{event:b,title:null!=a.data?a.data.constructor.name:null,callback:function(b,q,f){switch(b){case "Add Node":h.onMenuAdd(null,null,f,n,function(b){b.inputs&&b.inputs.length&&b.outputs&&b.outputs.length&&e.connectByType(a.origin_slot,b,g)&&(b.connectByType(a.target_slot,d,k),b.pos[0]-=.5*b.size[0])});break;case "Delete":c.graph.removeLink(a.id)}}});return!1};h.prototype.createDefaultNodeForSlot=function(a){a=a||{};a=Object.assign({nodeFrom:null,
...
c.allow_searchbox&&(a.push("Search"),a.push(null));var k=d.type==f.EVENT?"_event_":d.type,n=e?f.slot_types_default_out:f.slot_types_default_in;if(n&&n[k])if("object"==typeof n[k]||"array"==typeof n[k])for(var p in n[k])a.push(n[k][p]);else a.push(n[k]);var q=new f.ContextMenu(a,{event:b.e,title:(d&&""!=d.name?d.name+(k?" | ":""):"")+(d&&k?k:""),callback:function(a,f,n){switch(a){case "Add Node":h.onMenuAdd(null,null,n,q,function(a){e?b.nodeFrom.connectByType(g,a,k):b.nodeTo.connectByTypeOutput(g,
...
k.close=function(){g.prompt_box=null;k.parentNode&&k.parentNode.removeChild(k)};d=h.active_canvas.canvas;d.parentNode.appendChild(k);1<this.ds.scale&&(k.style.transform="scale("+this.ds.scale+")");var n=null,p=!1;f.pointerListenerAdd(k,"leave",function(a){p||f.dialog_close_on_mouse_leave&&!k.is_modified&&f.dialog_close_on_mouse_leave&&(n=setTimeout(k.close,f.dialog_close_on_mouse_leave_delay))});f.pointerListenerAdd(k,"enter",function(a){f.dialog_close_on_mouse_leave&&n&&clearTimeout(n)});var q=k.querySelectorAll("select");
q&&q.forEach(function(a){a.addEventListener("click",function(a){p++});a.addEventListener("blur",function(a){p=0});a.addEventListener("change",function(a){p=-1})});g.prompt_box&&g.prompt_box.close();g.prompt_box=k;k.querySelector(".name").innerText=a;var u=k.querySelector(".value");u.value=b;u.addEventListener("keydown",function(a){k.is_modified=!0;if(27==a.keyCode)k.close();else if(13==a.keyCode&&"textarea"!=a.target.localName)c&&c(this.value),k.close();else return;a.preventDefault();a.stopPropagation()});
k.querySelector("button").addEventListener("click",function(a){c&&c(u.value);g.setDirty(!0);k.close()});a=d.getBoundingClientRect();q=b=-20;a&&(b-=a.left,q-=a.top);e?(k.style.left=e.clientX+b+"px",k.style.top=e.clientY+q+"px"):(k.style.left=.5*d.width+b+"px",k.style.top=.5*d.height+q+"px");setTimeout(function(){u.focus()},10);return k};h.search_limit=-1;h.prototype.showSearchBox=function(a,b){function c(c){if(c)if(g.onSearchBoxSelection)g.onSearchBoxSelection(c,a,k);else{var d=f.searchbox_extras[c.toLowerCase()];
...
-1;-1==d&&"undefined"!==typeof b.slot_from.slot_index&&(d=b.slot_from.slot_index);break;case "number":d=b.slot_from;break;default:d=0}!1!==d&&-1<d&&b.node_to.connectByTypeOutput(d,c,b.node_to.inputs[d].type)}k.graph.afterChange()}q.close()}function e(a){var b=t;t&&t.classList.remove("selected");t?(t=a?t.nextSibling:t.previousSibling)||(t=b):t=a?v.childNodes[0]:v.childNodes[v.childNodes.length];t&&(t.classList.add("selected"),t.scrollIntoView({block:"end",behavior:"smooth"}))}function d(){function a(a,
b){var d=document.createElement("div");m||(m=a);d.innerText=a;d.dataset.type=escape(a);d.className="litegraph lite-search-item";b&&(d.className+=" "+b);d.addEventListener("click",function(a){c(unescape(this.dataset.type))});v.appendChild(d)}w=null;var d=A.value;m=null;v.innerHTML="";if(d||b.show_all_if_empty)if(g.onSearchBox){var e=g.onSearchBox(v,d,k);if(e)for(var q=0;q<e.length;++q)a(e[q])}else{e=function(a,c){c=c||{};c=Object.assign({skipFilter:!1,inTypeOverride:!1,outTypeOverride:!1},c);var e=
f.registered_node_types[a];if(u&&e.filter!=u||(!b.show_all_if_empty||d)&&-1===a.toLowerCase().indexOf(d))return!1;if(b.do_type_filter&&!c.skipFilter){e=y.value;!1!==c.inTypeOverride&&(e=c.inTypeOverride);if(y&&e&&f.registered_slot_in_types[e]&&f.registered_slot_in_types[e].nodes&&(e=f.registered_slot_in_types[e].nodes.includes(a),!1===e))return!1;e=r.value;!1!==c.outTypeOverride&&(e=c.outTypeOverride);if(r&&e&&f.registered_slot_out_types[e]&&f.registered_slot_out_types[e].nodes&&(e=f.registered_slot_out_types[e].nodes.includes(a),
!1===e))return!1}return!0};var n=0;d=d.toLowerCase();var u=k.filter||k.graph.filter;if(b.do_type_filter&&g.search_box)var y=g.search_box.querySelector(".slot_in_type_filter"),r=g.search_box.querySelector(".slot_out_type_filter");else r=y=!1;for(q in f.searchbox_extras){var p=f.searchbox_extras[q];if(b.show_all_if_empty&&!d||-1!==p.desc.toLowerCase().indexOf(d)){var l=f.registered_node_types[p.type];if((!l||l.filter==u)&&e(p.type)&&(a(p.desc,"searchbox_extra"),-1!==h.search_limit&&n++>h.search_limit))break}}p=
null;if(Array.prototype.filter)p=Object.keys(f.registered_node_types).filter(e);else for(q in p=[],f.registered_node_types)e(q)&&p.push(q);for(q=0;q<p.length&&!(a(p[q]),-1!==h.search_limit&&n++>h.search_limit);q++);if(b.show_general_after_typefiltered&&(y.value||r.value)){filtered_extra=[];for(q in f.registered_node_types)e(q,{inTypeOverride:y&&y.value?"*":!1,outTypeOverride:r&&r.value?"*":!1})&&filtered_extra.push(q);for(q=0;q<filtered_extra.length&&!(a(filtered_extra[q],"generic_type"),-1!==h.search_limit&&
n++>h.search_limit);q++);}if((y.value||r.value)&&0==v.childNodes.length&&b.show_general_if_none_on_typefilter){filtered_extra=[];for(q in f.registered_node_types)e(q,{skipFilter:!0})&&filtered_extra.push(q);for(q=0;q<filtered_extra.length&&!(a(filtered_extra[q],"not_in_filter"),-1!==h.search_limit&&n++>h.search_limit);q++);}}}b=Object.assign({slot_from:null,node_from:null,node_to:null,do_type_filter:f.search_filter_enabled,type_filter_in:!1,type_filter_out:!1,show_general_if_none_on_typefilter:!0,
show_general_after_typefiltered:!0,hide_on_mouse_leave:f.search_hide_on_mouse_leave,show_all_if_empty:!0,show_all_on_open:f.search_show_all_on_open},b||{});var g=this,k=h.active_canvas,n=k.canvas,p=n.ownerDocument||document,q=document.createElement("div");q.className="litegraph litesearchbox graphdialog rounded";q.innerHTML="<span class='name'>Search</span> <input autofocus type='text' class='value rounded'/>";b.do_type_filter&&(q.innerHTML+="<select class='slot_in_type_filter'><option value=''></option></select>",
q.innerHTML+="<select class='slot_out_type_filter'><option value=''></option></select>");q.innerHTML+="<div class='helper'></div>";p.fullscreenElement?p.fullscreenElement.appendChild(q):(p.body.appendChild(q),p.body.style.overflow="hidden");if(b.do_type_filter)var u=q.querySelector(".slot_in_type_filter"),r=q.querySelector(".slot_out_type_filter");q.close=function(){g.search_box=null;this.blur();n.focus();p.body.style.overflow="";setTimeout(function(){g.canvas.focus()},20);q.parentNode&&q.parentNode.removeChild(q)};
1<this.ds.scale&&(q.style.transform="scale("+this.ds.scale+")");if(b.hide_on_mouse_leave){var y=!1,l=null;f.pointerListenerAdd(q,"enter",function(a){l&&(clearTimeout(l),l=null)});f.pointerListenerAdd(q,"leave",function(a){y||(l=setTimeout(function(){q.close()},500))});b.do_type_filter&&(u.addEventListener("click",function(a){y++}),u.addEventListener("blur",function(a){y=0}),u.addEventListener("change",function(a){y=-1}),r.addEventListener("click",function(a){y++}),r.addEventListener("blur",function(a){y=
0}),r.addEventListener("change",function(a){y=-1}))}g.search_box&&g.search_box.close();g.search_box=q;var v=q.querySelector(".helper"),m=null,w=null,t=null,A=q.querySelector("input");A&&(A.addEventListener("blur",function(a){this.focus()}),A.addEventListener("keydown",function(a){if(38==a.keyCode)e(!1);else if(40==a.keyCode)e(!0);else if(27==a.keyCode)q.close();else if(13==a.keyCode)t?c(t.innerHTML):m?c(m):q.close();else{w&&clearInterval(w);w=setTimeout(d,250);return}a.preventDefault();a.stopPropagation();
a.stopImmediatePropagation();return!0}));if(b.do_type_filter){if(u){var x=f.slot_types_in,H=x.length;if(b.type_filter_in==f.EVENT||b.type_filter_in==f.ACTION)b.type_filter_in="_event_";for(var B=0;B<H;B++){var C=document.createElement("option");C.value=x[B];C.innerHTML=x[B];u.appendChild(C);!1!==b.type_filter_in&&(b.type_filter_in+"").toLowerCase()==(x[B]+"").toLowerCase()&&(C.selected=!0)}u.addEventListener("change",function(){d()})}if(r){x=f.slot_types_out;H=x.length;if(b.type_filter_out==f.EVENT||
b.type_filter_out==f.ACTION)b.type_filter_out="_event_";for(B=0;B<H;B++)C=document.createElement("option"),C.value=x[B],C.innerHTML=x[B],r.appendChild(C),!1!==b.type_filter_out&&(b.type_filter_out+"").toLowerCase()==(x[B]+"").toLowerCase()&&(C.selected=!0);r.addEventListener("change",function(){d()})}}u=n.getBoundingClientRect();r=(a?a.clientY:u.top+.5*u.height)-20;q.style.left=(a?a.clientX:u.left+.5*u.width)-80+"px";q.style.top=r+"px";a.layerY>u.height-200&&(v.style.maxHeight=u.height-a.layerY-20+
"px");A.focus();b.show_all_on_open&&d();return q};h.prototype.showEditPropertyValue=function(a,b,c){function e(){d(r.value)}function d(d){g&&g.values&&g.values.constructor===Object&&void 0!=g.values[d]&&(d=g.values[d]);"number"==typeof a.properties[b]&&(d=Number(d));if("array"==k||"object"==k)d=JSON.parse(d);a.properties[b]=d;a.graph&&a.graph._version++;if(a.onPropertyChanged)a.onPropertyChanged(b,d);if(c.onclose)c.onclose();u.close();a.setDirtyCanvas(!0,!0)}if(a&&void 0!==a.properties[b]){c=c||{};
var g=a.getPropertyInfo(b),k=g.type,f="";if("string"==k||"number"==k||"array"==k||"object"==k)f="<input autofocus type='text' class='value'/>";else if("enum"!=k&&"combo"!=k||!g.values)if("boolean"==k||"toggle"==k)f="<input autofocus type='checkbox' class='value' "+(a.properties[b]?"checked":"")+"/>";else{console.warn("unknown type: "+k);return}else{f="<select autofocus type='text' class='value'>";for(var h in g.values){var q=h;g.values.constructor===Array&&(q=g.values[h]);f+="<option value='"+q+"' "+
(q==a.properties[b]?"selected":"")+">"+g.values[h]+"</option>"}f+="</select>"}var u=this.createDialog("<span class='name'>"+(g.label?g.label:b)+"</span>"+f+"<button>OK</button>",c),r=!1;if("enum"!=k&&"combo"!=k||!g.values)if("boolean"==k||"toggle"==k)(r=u.querySelector("input"))&&r.addEventListener("click",function(a){u.modified();d(!!r.checked)});else{if(r=u.querySelector("input"))r.addEventListener("blur",function(a){this.focus()}),q=void 0!==a.properties[b]?a.properties[b]:"","string"!==k&&(q=
JSON.stringify(q)),r.value=q,r.addEventListener("keydown",function(a){if(27==a.keyCode)u.close();else if(13==a.keyCode)e();else if(13!=a.keyCode){u.modified();return}a.preventDefault();a.stopPropagation()})}else r=u.querySelector("select"),r.addEventListener("change",function(a){u.modified();d(a.target.value)});r&&r.focus();u.querySelector("button").addEventListener("click",e);return u}};h.prototype.createDialog=function(a,b){b=Object.assign({checkForInput:!1,closeOnLeave:!0,closeOnLeave_checkModified:!0},
b||{});var c=document.createElement("div");c.className="graphdialog";c.innerHTML=a;c.is_modified=!1;a=this.canvas.getBoundingClientRect();var e=-20,d=-20;a&&(e-=a.left,d-=a.top);b.position?(e+=b.position[0],d+=b.position[1]):b.event?(e+=b.event.clientX,d+=b.event.clientY):(e+=.5*this.canvas.width,d+=.5*this.canvas.height);c.style.left=e+"px";c.style.top=d+"px";this.canvas.parentNode.appendChild(c);b.checkForInput&&(a=[],(a=c.querySelectorAll("input"))&&a.forEach(function(a){a.addEventListener("keydown",
...
e.addSeparator=function(){var a=document.createElement("div");a.className="separator";e.content.appendChild(a)};e.addWidget=function(a,b,k,n,p){function d(a,b){n.callback&&n.callback(a,b,n);p&&p(a,b,n)}n=n||{};var g=String(k);a=a.toLowerCase();"number"==a&&(g=k.toFixed(3));var r=document.createElement("div");r.className="property";r.innerHTML="<span class='property_name'></span><span class='property_value'></span>";r.querySelector(".property_name").innerText=n.label||b;var y=r.querySelector(".property_value");
y.innerText=g;r.dataset.property=b;r.dataset.type=n.type||a;r.options=n;r.value=k;if("code"==a)r.addEventListener("click",function(a){e.inner_showCodePad(this.dataset.property)});else if("boolean"==a)r.classList.add("boolean"),k&&r.classList.add("bool-on"),r.addEventListener("click",function(){var a=this.dataset.property;this.value=!this.value;this.classList.toggle("bool-on");this.querySelector(".property_value").innerText=this.value?"true":"false";d(a,this.value)});else if("string"==a||"number"==
a)y.setAttribute("contenteditable",!0),y.addEventListener("keydown",function(b){"Enter"!=b.code||"string"==a&&b.shiftKey||(b.preventDefault(),this.blur())}),y.addEventListener("blur",function(){var a=this.innerText,b=this.parentNode.dataset.property;"number"==this.parentNode.dataset.type&&(a=Number(a));d(b,a)});else if("enum"==a||"combo"==a)g=h.getPropertyPrintableValue(k,n.values),y.innerText=g,y.addEventListener("click",function(a){var b=this.parentNode.dataset.property,e=this;new f.ContextMenu(n.values||
[],{event:a,className:"dark",callback:function(a,c,g){e.innerText=a;d(b,a);return!1}},c)});e.content.appendChild(r);return r};if(e.onOpen&&"function"==typeof e.onOpen)e.onOpen();return e};h.getPropertyPrintableValue=function(a,b){if(!b||b.constructor===Array)return String(a);if(b.constructor===Object){var c="",e;for(e in b)if(b[e]==a){c=e;break}return String(a)+" ("+c+")"}};h.prototype.closePanels=function(){var a=document.querySelector("#node-panel");a&&a.close();(a=document.querySelector("#option-panel"))&&
a.close()};h.prototype.showShowGraphOptionsPanel=function(a,b,c,e){if(this.constructor&&"HTMLDivElement"==this.constructor.name){if(!(b&&b.event&&b.event.target&&b.event.target.lgraphcanvas)){console.warn("Canvas not found");return}var d=b.event.target.lgraphcanvas}else d=this;d.closePanels();a=d.getCanvasWindow();panel=d.createPanel("Options",{closable:!0,window:a,onOpen:function(){d.OPTIONPANEL_IS_OPEN=!0},onClose:function(){d.OPTIONPANEL_IS_OPEN=!1;d.options_panel=null}});d.options_panel=panel;
panel.id="option-panel";panel.classList.add("settings");(function(){panel.content.innerHTML="";var a=function(a,b,c){c&&c.key&&(a=c.key);c.values&&(b=Object.values(c.values).indexOf(b));d[a]=b},b=f.availableCanvasOptions;b.sort();for(var c in b){var e=b[c];panel.addWidget("boolean",e,d[e],{key:e,on:"True",off:"False"},a)}panel.addWidget("combo","Render mode",f.LINK_RENDER_MODES[d.links_render_mode],{key:"links_render_mode",values:f.LINK_RENDER_MODES},a);panel.addSeparator();panel.footer.innerHTML=
""})();d.canvas.parentNode.appendChild(panel)};h.prototype.showShowNodePanel=function(a){function b(){d.content.innerHTML="";d.addHTML("<span class='node_type'>"+a.type+"</span><span class='node_desc'>"+(a.constructor.desc||"")+"</span><span class='separator'></span>");d.addHTML("<h3>Properties</h3>");var b=function(b,c){e.graph.beforeChange(a);switch(b){case "Title":a.title=c;break;case "Mode":b=Object.values(f.NODE_MODES).indexOf(c);0<=b&&f.NODE_MODES[b]?a.changeMode(b):console.warn("unexpected mode: "+
c);break;case "Color":h.node_colors[c]?(a.color=h.node_colors[c].color,a.bgcolor=h.node_colors[c].bgcolor):console.warn("unexpected color: "+c);break;default:a.setProperty(b,c)}e.graph.afterChange();e.dirty_canvas=!0};d.addWidget("string","Title",a.title,{},b);d.addWidget("combo","Mode",f.NODE_MODES[a.mode],{values:f.NODE_MODES},b);var c="";void 0!==a.color&&(c=Object.keys(h.node_colors).filter(function(b){return h.node_colors[b].color==a.color}));d.addWidget("combo","Color",c,{values:Object.keys(h.node_colors)},
b);for(var n in a.properties){c=a.properties[n];var p=a.getPropertyInfo(n);a.onAddPropertyToPanel&&a.onAddPropertyToPanel(n,d)||d.addWidget(p.widget||p.type,n,c,p,b)}d.addSeparator();if(a.onShowCustomPanelInfo)a.onShowCustomPanelInfo(d);d.footer.innerHTML="";d.addButton("Delete",function(){a.block_delete||(a.graph.remove(a),d.close())}).classList.add("delete")}this.SELECTED_NODE=a;this.closePanels();var c=this.getCanvasWindow(),e=this,d=this.createPanel(a.title||"",{closable:!0,window:c,onOpen:function(){e.NODEPANEL_IS_OPEN=
!0},onClose:function(){e.NODEPANEL_IS_OPEN=!1;e.node_panel=null}});e.node_panel=d;d.id="node-panel";d.node=a;d.classList.add("settings");d.inner_showCodePad=function(c){d.classList.remove("settings");d.classList.add("centered");d.alt_content.innerHTML="<textarea class='code'></textarea>";var e=d.alt_content.querySelector("textarea"),g=function(){d.toggleAltContent(!1);d.toggleFooterVisibility(!0);e.parentNode.removeChild(e);d.classList.add("settings");d.classList.remove("centered");b()};e.value=a.properties[c];
e.addEventListener("keydown",function(b){"Enter"==b.code&&b.ctrlKey&&(a.setProperty(c,e.value),g())});d.toggleAltContent(!0);d.toggleFooterVisibility(!1);e.style.height="calc(100% - 40px)";var f=d.addButton("Assign",function(){a.setProperty(c,e.value);g()});d.alt_content.appendChild(f);f=d.addButton("Close",g);f.style.float="right";d.alt_content.appendChild(f)};b();this.canvas.parentNode.appendChild(d)};h.prototype.showSubgraphPropertiesDialog=function(a){function b(){e.clear();if(a.inputs)for(var c=
0;c<a.inputs.length;++c){var g=a.inputs[c];if(!g.not_subgraph_input){var f=e.addHTML("<button>&#10005;</button> <span class='bullet_icon'></span><span class='name'></span><span class='type'></span>","subgraph_property");f.dataset.name=g.name;f.dataset.slot=c;f.querySelector(".name").innerText=g.name;f.querySelector(".type").innerText=g.type;f.querySelector("button").addEventListener("click",function(c){a.removeInput(Number(this.parentNode.dataset.slot));b()})}}}console.log("showing subgraph properties dialog");
...
d&&-1==a.findInputSlot(d)&&(a.addInput(d,e),c.querySelector(".name").value="",c.querySelector(".type").value="",b())});b();this.canvas.parentNode.appendChild(e);return e};h.prototype.showSubgraphPropertiesDialogRight=function(a){function b(){d.clear();if(a.outputs)for(var c=0;c<a.outputs.length;++c){var e=a.outputs[c];if(!e.not_subgraph_output){var f=d.addHTML("<button>&#10005;</button> <span class='bullet_icon'></span><span class='name'></span><span class='type'></span>","subgraph_property");f.dataset.name=
e.name;f.dataset.slot=c;f.querySelector(".name").innerText=e.name;f.querySelector(".type").innerText=e.type;f.querySelector("button").addEventListener("click",function(c){a.removeOutput(Number(this.parentNode.dataset.slot));b()})}}}function c(){var c=this.parentNode,d=c.querySelector(".name").value,e=c.querySelector(".type").value;d&&-1==a.findOutputSlot(d)&&(a.addOutput(d,e),c.querySelector(".name").value="",c.querySelector(".type").value="",b())}var e=this.canvas.parentNode.querySelector(".subgraph_dialog");
...
else if("Rename Slot"==b.content){b=b.slot;var g=b.input?a.getInputInfo(b.slot):a.getOutputInfo(b.slot),q=c.createDialog("<span class='name'>Name</span><input autofocus type='text'/><button>OK</button>",d),f=q.querySelector("input");f&&g&&(f.value=g.label||"");var k=function(){a.graph.beforeChange();f.value&&(g&&(g.label=f.value),c.setDirty(!0));q.close();a.graph.afterChange()};q.querySelector("button").addEventListener("click",k);f.addEventListener("keydown",function(a){q.is_modified=!0;if(27==a.keyCode)q.close();
...
!window.CanvasRenderingContext2D.prototype.roundRect&&(window.CanvasRenderingContext2D.prototype.roundRect=function(a,b,c,e,d,g){var f,h;if(0===d)this.rect(a,b,c,e);else{void 0===g&&(g=d);if(null!=d&&d.constructor===Array)if(1==d.length)var l=f=h=g=d[0];else if(2==d.length)l=g=d[0],f=h=d[1];else if(4==d.length)l=d[0],f=d[1],h=d[2],g=d[3];else return;else l=d||0,f=d||0,h=g||0,g=g||0;this.moveTo(a+l,b);this.lineTo(a+c-f,b);this.quadraticCurveTo(a+c,b,a+c,b+f);this.lineTo(a+c,b+e-g);this.quadraticCurveTo(a+
c,b+e,a+c-g,b+e);this.lineTo(a+g,b+e);this.quadraticCurveTo(a,b+e,a,b+e-h);this.lineTo(a,b+h);this.quadraticCurveTo(a,b,a+l,b)}});f.compareObjects=function(a,b){for(var c in a)if(a[c]!=b[c])return!1;return!0};f.distance=E;f.colorToString=function(a){return"rgba("+Math.round(255*a[0]).toFixed()+","+Math.round(255*a[1]).toFixed()+","+Math.round(255*a[2]).toFixed()+","+(4==a.length?a[3].toFixed(2):"1.0")+")"};f.isInsideRectangle=D;f.growBounding=function(a,b,c){b<a[0]?a[0]=b:b>a[2]&&(a[2]=b);c<a[1]?
a[1]=c:c>a[3]&&(a[3]=c)};f.isInsideBounding=function(a,b){return a[0]<b[0][0]||a[1]<b[0][1]||a[0]>b[1][0]||a[1]>b[1][1]?!1:!0};f.overlapBounding=B;f.hex2num=function(a){"#"==a.charAt(0)&&(a=a.slice(1));a=a.toUpperCase();for(var b=Array(3),c=0,e,d,g=0;6>g;g+=2)e="0123456789ABCDEF".indexOf(a.charAt(g)),d="0123456789ABCDEF".indexOf(a.charAt(g+1)),b[c]=16*e+d,c++;return b};f.num2hex=function(a){for(var b="#",c,e,d=0;3>d;d++)c=a[d]/16,e=a[d]%16,b+="0123456789ABCDEF".charAt(c)+"0123456789ABCDEF".charAt(e);
...
this.options.parentMenu.getFirstEvent():this.options.event};F.isCursorOverElement=function(a,b){var c=a.clientX;a=a.clientY;return(b=b.getBoundingClientRect())?a>b.top&&a<b.top+b.height&&c>b.left&&c<b.left+b.width?!0:!1:!1};f.ContextMenu=F;f.closeAllContextMenus=function(a){a=a||window;a=a.document.querySelectorAll(".litecontextmenu");if(a.length){for(var b=[],c=0;c<a.length;c++)b.push(a[c]);for(c=0;c<b.length;c++)b[c].close?b[c].close():b[c].parentNode&&b[c].parentNode.removeChild(b[c])}};f.extendClass=
function(a,b){for(var c in b)a.hasOwnProperty(c)||(a[c]=b[c]);if(b.prototype)for(c in b.prototype)b.prototype.hasOwnProperty(c)&&!a.prototype.hasOwnProperty(c)&&(b.prototype.__lookupGetter__(c)?a.prototype.__defineGetter__(c,b.prototype.__lookupGetter__(c)):a.prototype[c]=b.prototype[c],b.prototype.__lookupSetter__(c)&&a.prototype.__defineSetter__(c,b.prototype.__lookupSetter__(c)))};I.sampleCurve=function(a,b){if(b){for(var c=0;c<b.length-1;++c){var e=b[c],d=b[c+1];if(!(d[0]<a)){b=d[0]-e[0];if(1E-5>
Math.abs(b))return e[1];a=(a-e[0])/b;return e[1]*(1-a)+d[1]*a}}return 0}};I.prototype.draw=function(a,b,c,e,d,g){if(c=this.points){this.size=b;var f=b[0]-2*this.margin;b=b[1]-2*this.margin;d=d||"#666";a.save();a.translate(this.margin,this.margin);e&&(a.fillStyle="#111",a.fillRect(0,0,f,b),a.fillStyle="#222",a.fillRect(.5*f,0,1,b),a.strokeStyle="#333",a.strokeRect(0,0,f,b));a.strokeStyle=d;g&&(a.globalAlpha=.5);a.beginPath();for(e=0;e<c.length;++e)d=c[e],a.lineTo(d[0]*f,(1-d[1])*b);a.stroke();a.globalAlpha=
1;if(!g)for(e=0;e<c.length;++e)d=c[e],a.fillStyle=this.selected==e?"#FFF":this.nearest==e?"#DDD":"#AAA",a.beginPath(),a.arc(d[0]*f,(1-d[1])*b,2,0,2*Math.PI),a.fill();a.restore()}};I.prototype.onMouseDown=function(a,b){var c=this.points;if(c&&!(0>a[1])){var e=this.size[0]-2*this.margin,d=this.size[1]-2*this.margin,g=a[0]-this.margin;a=a[1]-this.margin;this.selected=this.getCloserPoint([g,a],30/b.ds.scale);-1==this.selected&&(b=[g/e,1-a/d],c.push(b),c.sort(function(a,b){return a[0]-b[0]}),this.selected=
c.indexOf(b),this.must_update=!0);if(-1!=this.selected)return!0}};I.prototype.onMouseMove=function(a,b){var c=this.points;if(c){var e=this.selected;if(!(0>e)){var d=(a[0]-this.margin)/(this.size[0]-2*this.margin),g=(a[1]-this.margin)/(this.size[1]-2*this.margin);this._nearest=this.getCloserPoint([a[0]-this.margin,a[1]-this.margin],30/b.ds.scale);if(b=c[e]){var f=0==e||e==c.length-1;!f&&(-10>a[0]||a[0]>this.size[0]+10||-10>a[1]||a[1]>this.size[1]+10)?(c.splice(e,1),this.selected=-1):(b[0]=f?0==e?0:
1:Math.clamp(d,0,1),b[1]=1-Math.clamp(g,0,1),c.sort(function(a,b){return a[0]-b[0]}),this.selected=c.indexOf(b),this.must_update=!0)}}}};I.prototype.onMouseUp=function(a,b){this.selected=-1;return!1};I.prototype.getCloserPoint=function(a,b){var c=this.points;if(!c)return-1;b=b||30;for(var e=this.size[0]-2*this.margin,d=this.size[1]-2*this.margin,g=c.length,f=[0,0],h=1E6,l=-1,q=0;q<g;++q){var u=c[q];f[0]=u[0]*e;f[1]=(1-u[1])*d;u=vec2.distance(a,f);u>h||u>b||(l=q,h=u)}return l};f.CurveEditor=I;f.getParameterNames=
...
0);this.addInput("assign",z.ACTION);this.addOutput("data",0);this._last_value=null;this.properties={data:null,serialize:!0};var f=this;this.addWidget("button","store","",function(){f.properties.data=f._last_value})}var z=x.LiteGraph;m.title="Log Event";m.desc="Log event in console";m.prototype.onAction=function(f,h,l){console.log(f,h)};z.registerNodeType("events/log",m);t.title="TriggerEvent";t.desc="Triggers event if input evaluates to true";t.prototype.onExecute=function(f,h){var l=this.getInputData(0),
m=l!=this.prev;0===this.prev&&(m=!1);var w=m&&this.properties.only_on_change||!m&&!this.properties.only_on_change;l&&w&&this.triggerSlot(0,f,null,h);!l&&w&&this.triggerSlot(2,f,null,h);m&&this.triggerSlot(1,f,null,h);this.prev=l};z.registerNodeType("events/trigger",t);l.title="Sequence";l.desc="Triggers a sequence of events when an event arrives";l.prototype.getTitle=function(){return""};l.prototype.onAction=function(f,h,l){if(this.outputs){l=l||{};for(var m=0;m<this.outputs.length;++m)l.action_call=
l.action_call?l.action_call+"_seq_"+m:this.id+"_"+(f?f:"action")+"_seq_"+m+"_"+Math.floor(9999*Math.random()),this.triggerSlot(m,h,null,l)}};z.registerNodeType("events/sequence",l);w.title="Stepper";w.desc="Trigger events sequentially when an tick arrives";w.prototype.onDrawBackground=function(f){if(!this.flags.collapsed){var h=this.properties.index||0;f.fillStyle="#AFB";var l=this.size[0];h=(h+1)*z.NODE_SLOT_HEIGHT+4;f.beginPath();f.moveTo(l-30,h);f.lineTo(l-30,h+z.NODE_SLOT_HEIGHT);f.lineTo(l-15,
h+.5*z.NODE_SLOT_HEIGHT);f.fill()}};w.prototype.onExecute=function(){var f=this.getInputData(0);null!=f&&(f=Math.floor(f),f=Math.clamp(f,0,this.outputs?this.outputs.length-2:0),f!=this.properties.index&&(this.properties.index=f,this.triggerSlot(f+1)));this.setOutputData(0,this.properties.index)};w.prototype.onAction=function(f,h){"reset"==f?this.properties.index=0:"step"==f&&(this.triggerSlot(this.properties.index+1,h),this.properties.index=(this.properties.index+1)%(this.outputs?this.outputs.length-
1:0))};z.registerNodeType("events/stepper",w);A.title="Filter Event";A.desc="Blocks events that do not match the filter";A.prototype.onAction=function(f,h,l){if(null!=h&&(!this.properties.equal_to||this.properties.equal_to==h)){if(this.properties.has_property&&(f=h[this.properties.has_property],null==f||this.properties.property_equal_to&&this.properties.property_equal_to!=f))return;this.triggerSlot(0,h,null,l)}};z.registerNodeType("events/filter",A);h.title="Branch";h.desc="If condition is true, outputs triggers true, otherwise false";
h.prototype.onExecute=function(){this._value=this.getInputData(1)};h.prototype.onAction=function(f,h,l){this._value=this.getInputData(1);this.triggerSlot(this._value?0:1,h,null,l)};z.registerNodeType("events/branch",h);E.title="Counter";E.desc="Counts events";E.prototype.getTitle=function(){return this.flags.collapsed?String(this.num):this.title};E.prototype.onAction=function(f,h,l){h=this.num;"inc"==f?this.num+=1:"dec"==f?--this.num:"reset"==f&&(this.num=0);this.num!=h&&this.trigger("change",this.num)};
E.prototype.onDrawBackground=function(f){this.flags.collapsed||(f.fillStyle="#AAA",f.font="20px Arial",f.textAlign="center",f.fillText(this.num,.5*this.size[0],.5*this.size[1]))};E.prototype.onExecute=function(){this.properties.doCountExecution&&(this.num+=1);this.setOutputData(1,this.num)};z.registerNodeType("events/counter",E);D.title="Delay";D.desc="Delays one event";D.prototype.onAction=function(f,h,l){f=this.properties.time_in_ms;0>=f?this.trigger(null,h,l):this._pending.push([f,h])};D.prototype.onExecute=
function(f,h){f=1E3*this.graph.elapsed_time;this.isInputConnected(1)&&(this.properties.time_in_ms=this.getInputData(1));for(var l=0;l<this._pending.length;++l){var m=this._pending[l];m[0]-=f;0<m[0]||(this._pending.splice(l,1),--l,this.trigger(null,m[1],h))}};D.prototype.onGetInputs=function(){return[["event",z.ACTION],["time_in_ms","number"]]};z.registerNodeType("events/delay",D);B.title="Timer";B.desc="Sends an event every N milliseconds";B.prototype.onStart=function(){this.time=0};B.prototype.getTitle=
...
this.boxcolor=this._ready?"#9F9":"#FA5"};F.prototype.onAction=function(f,h){"go"==f?this.triggerSlot(this._ready?0:1):"green"==f?this._ready=!0:"red"==f&&(this._ready=!1)};z.registerNodeType("events/semaphore",F);I.title="Once";I.desc="Only passes an event once, then gets locked";I.prototype.onAction=function(f,h){"in"!=f||this._once?"reset"==f&&(this._once=!1):(this._once=!0,this.triggerSlot(0,h))};z.registerNodeType("events/once",I);f.title="Data Store";f.desc="Stores data and only changes when event is received";
f.prototype.onExecute=function(){this._last_value=this.getInputData(0);this.setOutputData(0,this.properties.data)};f.prototype.onAction=function(f,h,l){this.properties.data=this._last_value};f.prototype.onSerialize=function(f){null!=f.data&&(0==this.properties.serialize||f.data.constructor!==String&&f.data.constructor!==Number&&f.data.constructor!==Boolean&&f.data.constructor!==Array&&f.data.constructor!==Object)&&(f.data=null)};z.registerNodeType("basic/data_store",f)})(this);
...
1);this.size=[120,50]}function A(){this.addOutput("value","number");this.addProperty("min",0);this.addProperty("max",1);this.size=[80,30]}function h(){this.addInput("in","number");this.addOutput("out","number");this.addProperty("min",0);this.addProperty("max",1);this.addProperty("smooth",!0);this.addProperty("seed",0);this.addProperty("octaves",1);this.addProperty("persistence",.8);this.addProperty("speed",1);this.size=[90,30]}function E(){this.addOutput("out","number");this.addProperty("min_time",
...
function d(){this.addInput("x","number");this.addInput("y","number");this.addOutput("","number");this.properties={x:1,y:1,formula:"x+y"};this.code_widget=this.addWidget("text","F(x,y)",this.properties.formula,function(a,b,c){c.properties.formula=a});this.addWidget("toggle","allow",r.allow_scripts,function(a){r.allow_scripts=a});this._func=null}function g(){this.addInput("vec2","vec2");this.addOutput("x","number");this.addOutput("y","number")}function k(){this.addInputs([["x","number"],["y","number"]]);
...
function(){return[["in_min","number"],["in_max","number"],["out_min","number"],["out_max","number"]]};r.registerNodeType("math/range",w);A.title="Rand";A.desc="Random number";A.prototype.onExecute=function(){if(this.inputs)for(var a=0;a<this.inputs.length;a++){var b=this.inputs[a],c=this.getInputData(a);void 0!==c&&(this.properties[b.name]=c)}a=this.properties.min;this._last_v=Math.random()*(this.properties.max-a)+a;this.setOutputData(0,this._last_v)};A.prototype.onDrawBackground=function(a){this.outputs[0].label=
(this._last_v||0).toFixed(3)};A.prototype.onGetInputs=function(){return[["min","number"],["max","number"]]};r.registerNodeType("math/rand",A);h.title="Noise";h.desc="Random number with temporal continuity";h.data=null;h.getValue=function(a,b){if(!h.data){h.data=new Float32Array(1024);for(var c=0;c<h.data.length;++c)h.data[c]=Math.random()}a%=1024;0>a&&(a+=1024);var d=Math.floor(a);a-=d;c=h.data[d];d=h.data[1023==d?0:d+1];b&&(a=a*a*a*(a*(6*a-15)+10));return c*(1-a)+d*a};h.prototype.onExecute=function(){var a=
this.getInputData(0)||0,b=this.properties.octaves||1,c=0,d=1;a+=this.properties.seed||0;for(var e=this.properties.speed||1,f=0,g=0;g<b&&!(c+=h.getValue(a*(1+g)*e,this.properties.smooth)*d,f+=d,d*=this.properties.persistence,.001>d);++g);a=this.properties.min;this._last_v=c/f*(this.properties.max-a)+a;this.setOutputData(0,this._last_v)};h.prototype.onDrawBackground=function(a){this.outputs[0].label=(this._last_v||0).toFixed(3)};r.registerNodeType("math/noise",h);E.title="Spikes";E.desc="spike every random time";
E.prototype.onExecute=function(){var a=this.graph.elapsed_time;this._remaining_time-=a;this._blink_time-=a;a=0;0<this._blink_time&&(a=1/(Math.pow(this._blink_time/this.properties.duration*8-4,4)+1));0>this._remaining_time?(this._remaining_time=Math.random()*(this.properties.max_time-this.properties.min_time)+this.properties.min_time,this._blink_time=this.properties.duration,this.boxcolor="#FFF"):this.boxcolor="#000";this.setOutputData(0,a)};r.registerNodeType("math/spikes",E);D.title="Clamp";D.desc=
"Clamp number between min and max";D.prototype.onExecute=function(){var a=this.getInputData(0);null!=a&&(a=Math.max(this.properties.min,a),a=Math.min(this.properties.max,a),this.setOutputData(0,a))};D.prototype.getCode=function(a){a="";this.isInputConnected(0)&&(a+="clamp({{0}},"+this.properties.min+","+this.properties.max+")");return a};r.registerNodeType("math/clamp",D);B.title="Lerp";B.desc="Linear Interpolation";B.prototype.onExecute=function(){var a=this.getInputData(0);null==a&&(a=0);var b=
this.getInputData(1);null==b&&(b=0);var c=this.properties.f,d=this.getInputData(2);void 0!==d&&(c=d);this.setOutputData(0,a*(1-c)+b*c)};B.prototype.onGetInputs=function(){return[["f","number"]]};r.registerNodeType("math/lerp",B);F.title="Abs";F.desc="Absolute";F.prototype.onExecute=function(){var a=this.getInputData(0);null!=a&&this.setOutputData(0,Math.abs(a))};r.registerNodeType("math/abs",F);I.title="Floor";I.desc="Floor number to remove fractional part";I.prototype.onExecute=function(){var a=
this.getInputData(0);null!=a&&this.setOutputData(0,Math.floor(a))};r.registerNodeType("math/floor",I);f.title="Frac";f.desc="Returns fractional part";f.prototype.onExecute=function(){var a=this.getInputData(0);null!=a&&this.setOutputData(0,a%1)};r.registerNodeType("math/frac",f);z.title="Smoothstep";z.desc="Smoothstep";z.prototype.onExecute=function(){var a=this.getInputData(0);if(void 0!==a){var b=this.properties.A;a=Math.clamp((a-b)/(this.properties.B-b),0,1);this.setOutputData(0,a*a*(3-2*a))}};
r.registerNodeType("math/smoothstep",z);H.title="Scale";H.desc="v * factor";H.prototype.onExecute=function(){var a=this.getInputData(0);null!=a&&this.setOutputData(0,a*this.properties.factor)};r.registerNodeType("math/scale",H);C.title="Gate";C.desc="if v is true, then outputs A, otherwise B";C.prototype.onExecute=function(){var a=this.getInputData(0);this.setOutputData(0,this.getInputData(a?1:2))};r.registerNodeType("math/gate",C);L.title="Average";L.desc="Average Filter";L.prototype.onExecute=function(){var a=
this.getInputData(0);null==a&&(a=0);var b=this._values.length;this._values[this._current%b]=a;this._current+=1;this._current>b&&(this._current=0);for(var c=a=0;c<b;++c)a+=this._values[c];this.setOutputData(0,a/b)};L.prototype.onPropertyChanged=function(a,b){1>b&&(b=1);this.properties.samples=Math.round(b);a=this._values;this._values=new Float32Array(this.properties.samples);a.length<=this._values.length?this._values.set(a):this._values.set(a.subarray(0,this._values.length))};r.registerNodeType("math/average",
L);G.title="TendTo";G.desc="moves the output value always closer to the input";G.prototype.onExecute=function(){var a=this.getInputData(0);null==a&&(a=0);var b=this.properties.factor;this._value=null==this._value?a:this._value*(1-b)+a*b;this.setOutputData(0,this._value)};r.registerNodeType("math/tendTo",G);K.values="+ - * / % ^ max min".split(" ");K.title="Operation";K.desc="Easy math operators";K["@OP"]={type:"enum",title:"operation",values:K.values};K.size=[100,60];K.prototype.getTitle=function(){return"max"==
...
case ">=":c=a>=b;break;case "||":c=a||b;break;case "&&":c=a&&b}this.setOutputData(0,c);this.setOutputData(1,!c)};r.registerNodeType("math/condition",a);b.title="Branch";b.desc="If condition is true, outputs IN in true, otherwise in false";b.prototype.onExecute=function(){var a=this.getInputData(0);this.getInputData(1)?(this.setOutputData(0,a),this.setOutputData(1,null)):(this.setOutputData(0,null),this.setOutputData(1,a))};r.registerNodeType("math/branch",b);c.title="Accumulate";c.desc="Increments a value every time";
c.prototype.onExecute=function(){null===this.properties.value&&(this.properties.value=0);var a=this.getInputData(0);this.properties.value=null!==a?this.properties.value+a:this.properties.value+this.properties.increment;this.setOutputData(0,this.properties.value)};r.registerNodeType("math/accumulate",c);e.title="Trigonometry";e.desc="Sin Cos Tan";e.prototype.onExecute=function(){var a=this.getInputData(0);null==a&&(a=0);var b=this.properties.amplitude,c=this.findInputSlot("amplitude");-1!=c&&(b=this.getInputData(c));
...
function(){return[["sin","number"],["cos","number"],["tan","number"],["asin","number"],["acos","number"],["atan","number"]]};r.registerNodeType("math/trigonometry",e);r.registerSearchboxExtra("math/trigonometry","SIN()",{outputs:[["sin","number"]],title:"SIN()"});r.registerSearchboxExtra("math/trigonometry","COS()",{outputs:[["cos","number"]],title:"COS()"});r.registerSearchboxExtra("math/trigonometry","TAN()",{outputs:[["tan","number"]],title:"TAN()"});d.title="Formula";d.desc="Compute formula";
d.size=[160,100];L.prototype.onPropertyChanged=function(a,b){"formula"==a&&(this.code_widget.value=b)};d.prototype.onExecute=function(){if(r.allow_scripts){var a=this.getInputData(0),b=this.getInputData(1);null!=a?this.properties.x=a:a=this.properties.x;null!=b?this.properties.y=b:b=this.properties.y;try{this._func&&this._func_code==this.properties.formula||(this._func=new Function("x","y","TIME","return "+this.properties.formula),this._func_code=this.properties.formula);var c=this._func(a,b,this.graph.globaltime);
this.boxcolor=null}catch(N){this.boxcolor="red"}this.setOutputData(0,c)}};d.prototype.getTitle=function(){return this._func_code||"Formula"};d.prototype.onDrawBackground=function(){var a=this.properties.formula;this.outputs&&this.outputs.length&&(this.outputs[0].label=a)};r.registerNodeType("math/formula",d);g.title="Vec2->XY";g.desc="vector 2 to components";g.prototype.onExecute=function(){var a=this.getInputData(0);null!=a&&(this.setOutputData(0,a[0]),this.setOutputData(1,a[1]))};r.registerNodeType("math3d/vec2-to-xy",
g);k.title="XY->Vec2";k.desc="components to vector2";k.prototype.onExecute=function(){var a=this.getInputData(0);null==a&&(a=this.properties.x);var b=this.getInputData(1);null==b&&(b=this.properties.y);var c=this._data;c[0]=a;c[1]=b;this.setOutputData(0,c)};r.registerNodeType("math3d/xy-to-vec2",k);n.title="Vec3->XYZ";n.desc="vector 3 to components";n.prototype.onExecute=function(){var a=this.getInputData(0);null!=a&&(this.setOutputData(0,a[0]),this.setOutputData(1,a[1]),this.setOutputData(2,a[2]))};
r.registerNodeType("math3d/vec3-to-xyz",n);p.title="XYZ->Vec3";p.desc="components to vector3";p.prototype.onExecute=function(){var a=this.getInputData(0);null==a&&(a=this.properties.x);var b=this.getInputData(1);null==b&&(b=this.properties.y);var c=this.getInputData(2);null==c&&(c=this.properties.z);var d=this._data;d[0]=a;d[1]=b;d[2]=c;this.setOutputData(0,d)};r.registerNodeType("math3d/xyz-to-vec3",p);q.title="Vec4->XYZW";q.desc="vector 4 to components";q.prototype.onExecute=function(){var a=this.getInputData(0);
null!=a&&(this.setOutputData(0,a[0]),this.setOutputData(1,a[1]),this.setOutputData(2,a[2]),this.setOutputData(3,a[3]))};r.registerNodeType("math3d/vec4-to-xyzw",q);u.title="XYZW->Vec4";u.desc="components to vector4";u.prototype.onExecute=function(){var a=this.getInputData(0);null==a&&(a=this.properties.x);var b=this.getInputData(1);null==b&&(b=this.properties.y);var c=this.getInputData(2);null==c&&(c=this.properties.z);var d=this.getInputData(3);null==d&&(d=this.properties.w);var e=this._data;e[0]=
a;e[1]=b;e[2]=c;e[3]=d;this.setOutputData(0,e)};r.registerNodeType("math3d/xyzw-to-vec4",u)})(this);
...
x.LiteGraph;m.title="Selector";m.desc="selects an output";m.prototype.onDrawBackground=function(h){if(!this.flags.collapsed){h.fillStyle="#AFB";var l=(this.selected+1)*D.NODE_SLOT_HEIGHT+6;h.beginPath();h.moveTo(50,l);h.lineTo(50,l+D.NODE_SLOT_HEIGHT);h.lineTo(34,l+.5*D.NODE_SLOT_HEIGHT);h.fill()}};m.prototype.onExecute=function(){var h=this.getInputData(0);if(null==h||h.constructor!==Number)h=0;this.selected=h=Math.round(h)%(this.inputs.length-1);h=this.getInputData(h+1);void 0!==h&&this.setOutputData(0,
h)};m.prototype.onGetInputs=function(){return[["E",0],["F",0],["G",0],["H",0]]};D.registerNodeType("logic/selector",m);t.title="Sequence";t.desc="select one element from a sequence from a string";t.prototype.onPropertyChanged=function(h,l){"sequence"==h&&(this.values=l.split(","))};t.prototype.onExecute=function(){var h=this.getInputData(1);h&&h!=this.current_sequence&&(this.values=h.split(","),this.current_sequence=h);h=this.getInputData(0);null==h&&(h=0);this.index=h=Math.round(h)%this.values.length;
this.setOutputData(0,this.values[h])};D.registerNodeType("logic/sequence",t);l.title="AND";l.desc="Return true if all inputs are true";l.prototype.onExecute=function(){var h=!0,l;for(l in this.inputs)if(!this.getInputData(l)){h=!1;break}this.setOutputData(0,h)};l.prototype.onGetInputs=function(){return[["and","boolean"]]};D.registerNodeType("logic/AND",l);w.title="OR";w.desc="Return true if at least one input is true";w.prototype.onExecute=function(){var h=!1,l;for(l in this.inputs)if(this.getInputData(l)){h=
!0;break}this.setOutputData(0,h)};w.prototype.onGetInputs=function(){return[["or","boolean"]]};D.registerNodeType("logic/OR",w);A.title="NOT";A.desc="Return the logical negation";A.prototype.onExecute=function(){var h=!this.getInputData(0);this.setOutputData(0,h)};D.registerNodeType("logic/NOT",A);h.title="bool == bool";h.desc="Compare for logical equality";h.prototype.onExecute=function(){var h=null,l=!0,m;for(m in this.inputs)if(null===h)h=this.getInputData(m);else if(h!=this.getInputData(m)){l=
!1;break}this.setOutputData(0,l)};h.prototype.onGetInputs=function(){return[["bool","boolean"]]};D.registerNodeType("logic/CompareBool",h);E.title="Branch";E.desc="Branch execution on condition";E.prototype.onExecute=function(h,l){this.getInputData(1)?this.triggerSlot(0):this.triggerSlot(1)};D.registerNodeType("logic/IF",E)})(this);
...
l&&this.connectSocket()};m.prototype.onExecute=function(){!this._ws&&this.properties.url&&this.connectSocket();if(this._ws&&this._ws.readyState==WebSocket.OPEN){for(var l=this.properties.room,m=this.properties.only_send_changes,h=1;h<this.inputs.length;++h){var t=this.getInputData(h);if(null!=t){try{var x=JSON.stringify({type:0,room:l,channel:h,data:t})}catch(B){continue}m&&this._last_sent_data[h]==x||(this._last_sent_data[h]=x,this._ws.send(x))}}for(h=1;h<this.outputs.length;++h)this.setOutputData(h,
...
1;l<this.inputs.length;++l){var m=this.inputs[l];m.label="in_"+l}for(l=1;l<this.outputs.length;++l)m=this.outputs[l],m.label="out_"+l};t.prototype.onExecute=function(){if(this._server&&this._server.is_connected){for(var l=this.properties.only_send_changes,m=1;m<this.inputs.length;++m){var h=this.getInputData(m),t=this._last_sent_data[m];if(null!=h){if(l){var x=!0;if(h&&h.length&&t&&t.length==h.length&&h.constructor!==String)for(var B=0;B<h.length;++B){if(t[B]!=h[B]){x=!1;break}}else this._last_sent_data[m]!=
