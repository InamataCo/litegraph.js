...
c[g.id]=g}else console.warn("serialized graph link data contains errors, skipping.")}a.links=c}for(d in a)"nodes"!=d&&"groups"!=d&&(this[d]=a[d]);c=!1;this._nodes=[];if(b){d=0;for(e=b.length;d<e;++d){g=b[d];var k=f.createNode(g.type,g.title);k||(f.debug&&console.log("Node not found or has errors: "+g.type),k=new l,k.last_serialization=g,c=k.has_errors=!0);k.id=g.id;this.add(k,!0)}d=0;for(e=b.length;d<e;++d)g=b[d],(k=this.getNodeById(g.id))&&k.configure(g)}this._groups.length=0;if(a.groups)for(d=0;d<
...
this.properties_info||(this.properties_info=[]);this.properties_info.push(c);this.properties||(this.properties={});this.properties[a]=b;return c};l.prototype.addOutput=function(a,b,c){a={name:a,type:b,links:null};if(c)for(var d in c)a[d]=c[d];this.outputs||(this.outputs=[]);this.outputs.push(a);if(this.onOutputAdded)this.onOutputAdded(a);f.auto_load_slot_types&&f.registerNodeAndSlotType(this,b,!0);this.setSize(this.computeSize());this.setDirtyCanvas(!0,!0);return a};l.prototype.addOutputs=function(a){for(var b=
0;b<a.length;++b){var c=a[b],d={name:c[0],type:c[1],link:null};if(a[2])for(var e in c[2])d[e]=c[2][e];this.outputs||(this.outputs=[]);this.outputs.push(d);if(this.onOutputAdded)this.onOutputAdded(d);f.auto_load_slot_types&&f.registerNodeAndSlotType(this,c[1],!0)}this.setSize(this.computeSize());this.setDirtyCanvas(!0,!0)};l.prototype.removeOutput=function(a){this.disconnectOutput(a);this.outputs.splice(a,1);for(var b=a;b<this.outputs.length;++b)if(this.outputs[b]&&this.outputs[b].links)for(var c=
this.outputs[b].links,d=0;d<c.length;++d){var e=this.graph.links[c[d]];e&&--e.origin_slot}this.setSize(this.computeSize());if(this.onOutputRemoved)this.onOutputRemoved(a);this.setDirtyCanvas(!0,!0)};l.prototype.addInput=function(a,b,c){b=b||0;a={name:a,type:b,link:null};if(c)for(var d in c)a[d]=c[d];this.inputs||(this.inputs=[]);this.inputs.push(a);this.setSize(this.computeSize());if(this.onInputAdded)this.onInputAdded(a);f.registerNodeAndSlotType(this,b);this.setDirtyCanvas(!0,!0);return a};l.prototype.addInputs=
function(a){for(var b=0;b<a.length;++b){var c=a[b],d={name:c[0],type:c[1],link:null};if(a[2])for(var e in c[2])d[e]=c[2][e];this.inputs||(this.inputs=[]);this.inputs.push(d);if(this.onInputAdded)this.onInputAdded(d);f.registerNodeAndSlotType(this,c[1])}this.setSize(this.computeSize());this.setDirtyCanvas(!0,!0)};l.prototype.removeInput=function(a){this.disconnectInput(a);for(var b=this.inputs.splice(a,1),c=a;c<this.inputs.length;++c)if(this.inputs[c]){var d=this.graph.links[this.inputs[c].link];d&&
--d.target_slot}this.setSize(this.computeSize());if(this.onInputRemoved)this.onInputRemoved(a,b[0]);this.setDirtyCanvas(!0,!0)};l.prototype.addConnection=function(a,b,c,d){a={name:a,type:b,pos:c,direction:d,links:null};this.connections.push(a);return a};l.prototype.computeSize=function(a){function b(a){return a?d*a.length*.6:0}if(this.constructor.size)return this.constructor.size.concat();var c=Math.max(this.inputs?this.inputs.length:1,this.outputs?this.outputs.length:1);a=a||new Float32Array([0,
...
if(!e)return!1;var g=e.outputs[d.origin_slot];if(!g||!g.links||0==g.links.length)return!1;for(var k=0,n=g.links.length;k<n;k++)if(g.links[k]==c){g.links.splice(k,1);break}delete this.graph.links[c];this.graph&&this.graph._version++;if(this.onConnectionsChange)this.onConnectionsChange(f.INPUT,a,!1,d,b);if(e.onConnectionsChange)e.onConnectionsChange(f.OUTPUT,k,!1,d,g);this.graph&&this.graph.onNodeConnectionChange&&(this.graph.onNodeConnectionChange(f.OUTPUT,e,k),this.graph.onNodeConnectionChange(f.INPUT,
...
enumerable:!0})};w.prototype.configure=function(a){this.title=a.title;this._bounding.set(a.bounding);this.color=a.color;this.font=a.font};w.prototype.serialize=function(){var a=this._bounding;return{title:this.title,bounding:[Math.round(a[0]),Math.round(a[1]),Math.round(a[2]),Math.round(a[3])],color:this.color,font:this.font}};w.prototype.move=function(a,b,c){this._pos[0]+=a;this._pos[1]+=b;if(!c)for(c=0;c<this._nodes.length;++c){var d=this._nodes[c];d.pos[0]+=a;d.pos[1]+=b}};w.prototype.recomputeInsideNodes=
function(){this._nodes.length=0;for(var a=this.graph._nodes,b=new Float32Array(4),c=0;c<a.length;++c){var d=a[c];d.getBounding(b);A(this._bounding,b)&&this._nodes.push(d)}};w.prototype.isPointInside=l.prototype.isPointInside;w.prototype.setDirtyCanvas=l.prototype.setDirtyCanvas;f.DragAndScale=z;z.prototype.bindEvents=function(a){this.last_mouse=new Float32Array(2);this._binded_mouse_callback=this.onMouse.bind(this);f.pointerListenerAdd(a,"down",this._binded_mouse_callback);f.pointerListenerAdd(a,
"move",this._binded_mouse_callback);f.pointerListenerAdd(a,"up",this._binded_mouse_callback);a.addEventListener("mousewheel",this._binded_mouse_callback,!1);a.addEventListener("wheel",this._binded_mouse_callback,!1)};z.prototype.computeVisibleArea=function(a){if(this.element){var b=this.element.width,c=this.element.height,d=-this.offset[0],e=-this.offset[1];a&&(d+=a[0]/this.scale,e+=a[1]/this.scale,b=a[2],c=a[3]);a=d+b/this.scale;c=e+c/this.scale;this.visible_area[0]=d;this.visible_area[1]=e;this.visible_area[2]=
...
a.restore();a.finish2D&&a.finish2D()}};h.prototype.drawSubgraphPanel=function(a){var b=this.graph,c=b._subgraph_node;c?(this.drawSubgraphPanelLeft(b,c,a),this.drawSubgraphPanelRight(b,c,a)):console.warn("subgraph without subnode")};h.prototype.drawSubgraphPanelLeft=function(a,b,c){var d=b.inputs?b.inputs.length:0,e=Math.floor(1.6*f.NODE_SLOT_HEIGHT);c.fillStyle="#111";c.globalAlpha=.8;c.beginPath();c.roundRect(10,10,200,(d+1)*e+50,[8]);c.fill();c.globalAlpha=1;c.fillStyle="#888";c.font="14px Arial";
...
!1,p.setProperty("name",k.name),p.setProperty("type",k.type),this.node_dragged.pos[0]=this.graph_mouse[0]-5,this.node_dragged.pos[1]=this.graph_mouse[1]-5,this.graph.afterChange()):console.error("graph input node not found:",n)}c.fillStyle="#9C9";c.beginPath();c.arc(184,d+.5*e,5,0,2*Math.PI);c.fill();c.fillStyle="#AAA";c.fillText(k.name,30,d+.75*e);c.fillStyle="#777";c.fillText(k.type,130,d+.75*e);d+=e}}this.drawButton(20,d+2,180,e-2,"+","#151515","#222")&&this.showSubgraphPropertiesDialog(b)}};h.prototype.drawSubgraphPanelRight=
...
1!=a.onDrawCollapsed(b,this)){var k=a._shape||f.BOX_SHAPE;I.set(a.size);var n=a.horizontal;if(a.flags.collapsed){b.font=this.inner_text_font;var p=a.getTitle?a.getTitle():a.title;null!=p&&(a._collapsed_width=Math.min(a.size[0],b.measureText(p).width+2*f.NODE_TITLE_HEIGHT),I[0]=a._collapsed_width,I[1]=0)}a.clip_area&&(b.save(),b.beginPath(),k==f.BOX_SHAPE?b.rect(0,0,I[0],I[1]):k==f.ROUND_SHAPE?b.roundRect(0,0,I[0],I[1],[10]):k==f.CIRCLE_SHAPE&&b.arc(.5*I[0],.5*I[1],.5*I[0],0,2*Math.PI),b.clip());a.has_errors&&
...
b.rect(v[0]-6+.5,v[1]-5+.5,14,10):h===f.ARROW_SHAPE?(b.moveTo(v[0]+8,v[1]+.5),b.lineTo(v[0]-4,v[1]+6+.5),b.lineTo(v[0]-4,v[1]-6+.5),b.closePath()):h===f.GRID_SHAPE?(b.rect(v[0]-4,v[1]-4,2,2),b.rect(v[0]-1,v[1]-4,2,2),b.rect(v[0]+2,v[1]-4,2,2),b.rect(v[0]-4,v[1]-1,2,2),b.rect(v[0]-1,v[1]-1,2,2),b.rect(v[0]+2,v[1]-1,2,2),b.rect(v[0]-4,v[1]+2,2,2),b.rect(v[0]-1,v[1]+2,2,2),b.rect(v[0]+2,v[1]+2,2,2)):e?b.rect(v[0]-4,v[1]-4,8,8):b.arc(v[0],v[1],4,0,2*Math.PI);b.fill();d&&(y=null!=q.label?q.label:q.name)&&
...
(e-10),-.5*(e+10),10,10));b.globalAlpha=k;if(a.onDrawTitleText)a.onDrawTitleText(b,e,c,this.ds.scale,this.title_text_font,g);!n&&(b.font=this.title_text_font,k=String(a.getTitle()))&&(b.fillStyle=g?f.NODE_SELECTED_TITLE_COLOR:a.constructor.title_text_color||this.node_title_color,a.flags.collapsed?(b.textAlign="left",b.measureText(k),b.fillText(k.substr(0,20),e,f.NODE_TITLE_TEXT_Y-e),b.textAlign="left"):(b.textAlign="left",b.fillText(k,e,f.NODE_TITLE_TEXT_Y-e)));a.flags.collapsed||!a.subgraph||a.skip_subgraph_button||
(k=f.NODE_TITLE_HEIGHT,u=a.size[0]-k,q=f.isInsideRectangle(this.graph_mouse[0]-a.pos[0],this.graph_mouse[1]-a.pos[1],u+2,-k+2,k-4,k-4),b.fillStyle=q?"#888":"#555",p==f.BOX_SHAPE||n?b.fillRect(u+2,-k+2,k-4,k-4):(b.beginPath(),b.roundRect(u+2,-k+2,k-4,k-4,[4]),b.fill()),b.fillStyle="#333",b.beginPath(),b.moveTo(u+.2*k,.6*-k),b.lineTo(u+.8*k,.6*-k),b.lineTo(u+.5*k,.3*-k),b.fill());if(a.onDrawTitle)a.onDrawTitle(b)}if(g){if(a.onBounding)a.onBounding(B);r==f.TRANSPARENT_TITLE&&(B[1]-=e,B[3]+=e);b.lineWidth=
1;b.globalAlpha=.8;b.beginPath();p==f.BOX_SHAPE?b.rect(-6+B[0],-6+B[1],12+B[2],12+B[3]):p==f.ROUND_SHAPE||p==f.CARD_SHAPE&&a.flags.collapsed?b.roundRect(-6+B[0],-6+B[1],12+B[2],12+B[3],[2*this.round_radius]):p==f.CARD_SHAPE?b.roundRect(-6+B[0],-6+B[1],12+B[2],12+B[3],[2*this.round_radius,2,2*this.round_radius,2]):p==f.CIRCLE_SHAPE&&b.arc(.5*c[0],.5*c[1],.5*c[0]+6,0,2*Math.PI);b.strokeStyle=f.NODE_BOX_OUTLINE_COLOR;b.stroke();b.strokeStyle=d;b.globalAlpha=1}0<a.execute_triggered&&a.execute_triggered--;
0<a.action_triggered&&a.action_triggered--};var K=new Float32Array(4),F=new Float32Array(4),J=new Float32Array(2),H=new Float32Array(2);h.prototype.drawConnections=function(a){var b=f.getTime(),c=this.visible_area;K[0]=c[0]-20;K[1]=c[1]-20;K[2]=c[2]+40;K[3]=c[3]+40;a.lineWidth=this.connections_width;a.fillStyle="#AAA";a.strokeStyle="#AAA";a.globalAlpha=this.editor_alpha;c=this.graph._nodes;for(var d=0,e=c.length;d<e;++d){var g=c[d];if(g.inputs&&g.inputs.length)for(var k=0;k<g.inputs.length;++k){var n=
g.inputs[k];if(n&&null!=n.link&&(n=this.graph.links[n.link])){var p=this.graph.getNodeById(n.origin_id);if(null!=p){var r=n.origin_slot;var u=-1==r?[p.pos[0]+10,p.pos[1]+10]:p.getConnectionPos(!1,r,J);var q=g.getConnectionPos(!0,k,H);F[0]=u[0];F[1]=u[1];F[2]=q[0]-u[0];F[3]=q[1]-u[1];0>F[2]&&(F[0]+=F[2],F[2]=Math.abs(F[2]));0>F[3]&&(F[1]+=F[3],F[3]=Math.abs(F[3]));if(A(F,K)){var y=p.outputs[r];r=g.inputs[k];if(y&&r&&(p=y.dir||(p.horizontal?f.DOWN:f.RIGHT),r=r.dir||(g.horizontal?f.UP:f.LEFT),this.renderLink(a,
u,q,n,!1,0,null,p,r),n&&n._last_time&&1E3>b-n._last_time)){y=2-.002*(b-n._last_time);var h=a.globalAlpha;a.globalAlpha=h*y;this.renderLink(a,u,q,n,!0,y,"white",p,r);a.globalAlpha=h}}}}}}a.globalAlpha=1};h.prototype.renderLink=function(a,b,c,d,e,g,k,n,p,r){d&&this.visible_links.push(d);!k&&d&&(k=d.color||h.link_type_colors[d.type]);k||(k=this.default_link_color);null!=d&&this.highlighted_links[d.id]&&(k="#FFF");n=n||f.RIGHT;p=p||f.LEFT;var u=D(b,c);this.render_connections_border&&.6<this.ds.scale&&
...
5,0,2*Math.PI),a.fill());if(g)for(a.fillStyle=k,q=0;5>q;++q)g=(.001*f.getTime()+.2*q)%1,e=this.computeConnectionPoint(b,c,g,n,p),a.beginPath(),a.arc(e[0],e[1],5,0,2*Math.PI),a.fill()};h.prototype.computeConnectionPoint=function(a,b,c,d,e){d=d||f.RIGHT;e=e||f.LEFT;var g=D(a,b),k=[a[0],a[1]],n=[b[0],b[1]];switch(d){case f.LEFT:k[0]+=-.25*g;break;case f.RIGHT:k[0]+=.25*g;break;case f.UP:k[1]+=-.25*g;break;case f.DOWN:k[1]+=.25*g}switch(e){case f.LEFT:n[0]+=-.25*g;break;case f.RIGHT:n[0]+=.25*g;break;
...
this.editor_alpha}c.restore();c.textAlign="left"};h.prototype.processNodeWidgets=function(a,b,c,d){function e(d,e){d.value=e;d.options&&d.options.property&&void 0!==a.properties[d.options.property]&&a.setProperty(d.options.property,e);d.callback&&d.callback(d.value,h,a,b,c)}if(!a.widgets||!a.widgets.length)return null;for(var g=b[0]-a.pos[0],k=b[1]-a.pos[1],n=a.size[0],h=this,r=this.getCanvasWindow(),u=0;u<a.widgets.length;++u){var q=a.widgets[u];if(q&&!q.disabled){var y=q.computeSize?q.computeSize(n)[1]:
...
value:c[r]});new f.ContextMenu(a,{event:b,callback:g,parentMenu:d,node:e});return!1}}if(e){var k=this;a=h.active_canvas.getCanvasWindow();b=e.optional_outputs;e.onGetOutputs&&(b=e.onGetOutputs());var n=[];if(b)for(var p=0;p<b.length;p++){var r=b[p];if(!r)n.push(null);else if(!e.flags||!e.flags.skip_repeated_outputs||-1==e.findOutputSlot(r[0])){var u=r[0];r[2]||(r[2]={});r[2].label&&(u=r[2].label);r[2].removable=!0;u={content:u,value:r};r[1]==f.EVENT&&(u.className="event");n.push(u)}}this.onMenuNodeOutputs&&
(n=this.onMenuNodeOutputs(n));f.do_add_triggers_slots&&-1==e.findOutputSlot("onExecuted")&&n.push({content:"On Executed",value:["onExecuted",f.EVENT,{nameLocked:!0}],className:"event"});e.onMenuNodeOutputs&&(b=e.onMenuNodeOutputs(n))&&(n=b);if(n.length)return new f.ContextMenu(n,{event:c,callback:g,parentMenu:d,node:e},a),!1}};h.onShowMenuNodeProperties=function(a,b,c,d,e){if(e&&e.properties){var g=h.active_canvas;b=g.getCanvasWindow();var k=[],n;for(n in e.properties){a=void 0!==e.properties[n]?
e.properties[n]:" ";"object"==typeof a&&(a=JSON.stringify(a));var p=e.getPropertyInfo(n);if("enum"==p.type||"combo"==p.type)a=h.getPropertyPrintableValue(a,p.values);a=h.decodeHTML(a);k.push({content:"<span class='property_name'>"+(p.label?p.label:n)+"</span><span class='property_value'>"+a+"</span>",value:n})}if(k.length)return new f.ContextMenu(k,{event:c,callback:function(a,b,c,d){e&&(b=this.getBoundingClientRect(),g.showEditPropertyValue(e,a.value,{position:[b.left,b.top]}))},parentMenu:d,allow_html:!0,
node:e},b),!1}};h.decodeHTML=function(a){var b=document.createElement("div");b.innerText=a;return b.innerHTML};h.onMenuResizeNode=function(a,b,c,d,e){if(e){a=function(a){a.size=a.computeSize();if(a.onResize)a.onResize(a.size)};b=h.active_canvas;if(!b.selected_nodes||1>=Object.keys(b.selected_nodes).length)a(e);else for(var g in b.selected_nodes)a(b.selected_nodes[g]);e.setDirtyCanvas(!0,!0)}};h.prototype.showLinkMenu=function(a,b){var c=this,d=c.graph.getNodeById(a.origin_id),e=c.graph.getNodeById(a.target_id),
g=!1;d&&d.outputs&&d.outputs[a.origin_slot]&&(g=d.outputs[a.origin_slot].type);var k=!1;e&&e.outputs&&e.outputs[a.target_slot]&&(k=e.inputs[a.target_slot].type);var n=new f.ContextMenu(["Add Node",null,"Delete",null],{event:b,title:null!=a.data?a.data.constructor.name:null,callback:function(b,f,u){switch(b){case "Add Node":h.onMenuAdd(null,null,u,n,function(b){b.inputs&&b.inputs.length&&b.outputs&&b.outputs.length&&d.connectByType(a.origin_slot,b,g)&&(b.connectByType(a.target_slot,e,k),b.pos[0]-=
.5*b.size[0])});break;case "Delete":c.graph.removeLink(a.id)}}});return!1};h.prototype.createDefaultNodeForSlot=function(a){a=a||{};a=Object.assign({nodeFrom:null,slotFrom:null,nodeTo:null,slotTo:null,position:[],nodeType:null,posAdd:[0,0],posSizeFix:[0,0]},a);var b=a.nodeFrom&&null!==a.slotFrom,c=!b&&a.nodeTo&&null!==a.slotTo;if(!b&&!c)return console.warn("No data passed to createDefaultNodeForSlot "+a.nodeFrom+" "+a.slotFrom+" "+a.nodeTo+" "+a.slotTo),!1;if(!a.nodeType)return console.warn("No type to createDefaultNodeForSlot"),
...
break;case "number":g=e;e=d?a.outputs[e]:a.inputs[e];break;default:return console.warn("Cant get slot information "+e),!1}a=["Add Node",null];c.allow_searchbox&&(a.push("Search"),a.push(null));var k=e.type==f.EVENT?"_event_":e.type,n=d?f.slot_types_default_out:f.slot_types_default_in;if(n&&n[k])if("object"==typeof n[k]||"array"==typeof n[k])for(var p in n[k])a.push(n[k][p]);else a.push(n[k]);var r=new f.ContextMenu(a,{event:b.e,title:(e&&""!=e.name?e.name+(k?" | ":""):"")+(e&&k?k:""),callback:function(a,
f,n){switch(a){case "Add Node":h.onMenuAdd(null,null,n,r,function(a){d?b.nodeFrom.connectByType(g,a,k):b.nodeTo.connectByTypeOutput(g,a,k)});break;case "Search":d?c.showSearchBox(n,{node_from:b.nodeFrom,slot_from:e,type_filter_in:k}):c.showSearchBox(n,{node_to:b.nodeTo,slot_from:e,type_filter_out:k});break;default:c.createDefaultNodeForSlot(Object.assign(b,{position:[b.e.canvasX,b.e.canvasY],nodeType:a}))}}});return!1};h.onShowPropertyEditor=function(a,b,c,d,e){function g(){if(p){var b=p.value;"Number"==
a.type?b=Number(b):"Boolean"==a.type&&(b=!!b);e[k]=b;n.parentNode&&n.parentNode.removeChild(n);e.setDirtyCanvas(!0,!0)}}var k=a.property||"title";b=e[k];var n=document.createElement("div");n.is_modified=!1;n.className="graphdialog";n.innerHTML="<span class='name'></span><input autofocus type='text' class='value'/><button>OK</button>";n.close=function(){n.parentNode&&n.parentNode.removeChild(n)};n.querySelector(".name").innerText=k;var p=n.querySelector(".value");p&&(p.value=b,p.addEventListener("blur",
...
b.properties.type)return c.readAsBinaryString(a)};p.registerNodeType("basic/file",A);E.title="Const Data";E.desc="Constant Data";E.prototype.onPropertyChanged=function(a,b){this.widget.value=b;if(null!=b&&""!=b)try{this._value=JSON.parse(b),this.boxcolor="#AEA"}catch(q){this.boxcolor="red"}};E.prototype.onExecute=function(){this.setOutputData(0,this._value)};E.prototype.setValue=z.prototype.setValue;p.registerNodeType("basic/data",E);G.title="Const Array";G.desc="Constant Array";G.prototype.onPropertyChanged=
function(a,b){this.widget.value=b;if(null!=b&&""!=b)try{this._value="["!=b[0]?JSON.parse("["+b+"]"):JSON.parse(b),this.boxcolor="#AEA"}catch(q){this.boxcolor="red"}};G.prototype.onExecute=function(){var a=this.getInputData(0);if(a&&a.length){this._value||(this._value=[]);this._value.length=a.length;for(var b=0;b<a.length;++b)this._value[b]=a[b]}this.setOutputData(0,this._value);this.setOutputData(1,this._value?this._value.length||0:0)};G.prototype.setValue=z.prototype.setValue;p.registerNodeType("basic/array",
G);f.title="Set Array";f.desc="Sets index of array";f.prototype.onExecute=function(){var a=this.getInputData(0);if(a){var b=this.getInputData(1);void 0!==b&&(this.properties.index&&(a[Math.floor(this.properties.index)]=b),this.setOutputData(0,a))}};p.registerNodeType("basic/set_array",f);L.title="Array[i]";L.desc="Returns an element from an array";L.prototype.onExecute=function(){var a=this.getInputData(0),b=this.getInputData(1);null==b&&(b=this.properties.index);null!=a&&null!=b&&this.setOutputData(0,
a[Math.floor(Number(b))])};p.registerNodeType("basic/array[]",L);I.title="Table[row][col]";I.desc="Returns an element from a table";I.prototype.onExecute=function(){var a=this.getInputData(0),b=this.getInputData(1),c=this.getInputData(2);null==b&&(b=this.properties.row);null==c&&(c=this.properties.column);null!=a&&null!=b&&null!=c&&((b=a[Math.floor(Number(b))])?this.setOutputData(0,b[Math.floor(Number(c))]):this.setOutputData(0,null))};p.registerNodeType("basic/table[][]",I);B.title="Object property";
B.desc="Outputs the property of an object";B.prototype.setValue=function(a){this.properties.value=a;this.widget.value=a};B.prototype.getTitle=function(){return this.flags.collapsed?"in."+this.properties.value:this.title};B.prototype.onPropertyChanged=function(a,b){this.widget.value=b};B.prototype.onExecute=function(){var a=this.getInputData(0);null!=a&&this.setOutputData(0,a[this.properties.value])};p.registerNodeType("basic/object_property",B);K.title="Object keys";K.desc="Outputs an array with the keys of an object";
K.prototype.onExecute=function(){var a=this.getInputData(0);null!=a&&this.setOutputData(0,Object.keys(a))};p.registerNodeType("basic/object_keys",K);F.title="Set Object";F.desc="Adds propertiesrty to object";F.prototype.onExecute=function(){var a=this.getInputData(0);if(a){var b=this.getInputData(1);void 0!==b&&(this.properties.property&&(a[this.properties.property]=b),this.setOutputData(0,a))}};p.registerNodeType("basic/set_object",F);J.title="Merge Objects";J.desc="Creates an object copying properties from others";
J.prototype.onExecute=function(){var a=this.getInputData(0),b=this.getInputData(1),c=this._result;if(a)for(var d in a)c[d]=a[d];if(b)for(d in b)c[d]=b[d];this.setOutputData(0,c)};p.registerNodeType("basic/merge_objects",J);H.title="Variable";H.desc="store/read variable value";H.LITEGRAPH=0;H.GRAPH=1;H.GLOBALSCOPE=2;H["@container"]={type:"enum",values:{litegraph:H.LITEGRAPH,graph:H.GRAPH,global:H.GLOBALSCOPE}};H.prototype.onExecute=function(){var a=this.getContainer();this.isInputConnected(0)?(this.value=
this.getInputData(0),a[this.properties.varname]=this.value,this.setOutputData(0,this.value)):this.setOutputData(0,a[this.properties.varname])};H.prototype.getContainer=function(){switch(this.properties.container){case H.GRAPH:return this.graph?this.graph.vars:{};case H.GLOBALSCOPE:return x;default:return p.Globals}};H.prototype.getTitle=function(){return this.properties.varname};p.registerNodeType("basic/variable",H);p.wrapFunctionAsNode("basic/length",a,[""],"number");p.wrapFunctionAsNode("basic/not",
