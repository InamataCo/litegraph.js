...
a||100;for(var c=this.computeExecutionOrder(!1,!0),d=[],e=0;e<c.length;++e){var g=c[e],k=g._level||1;d[k]||(d[k]=[]);d[k].push(g)}c=a;for(e=0;e<d.length;++e)if(g=d[e]){k=100;for(var n=a+f.NODE_TITLE_HEIGHT,p=0;p<g.length;++p){var r=g[p];r.pos[0]=b==f.VERTICAL_LAYOUT?n:c;r.pos[1]=b==f.VERTICAL_LAYOUT?c:n;var u=b==f.VERTICAL_LAYOUT?1:0;r.size[u]>k&&(k=r.size[u]);n+=r.size[b==f.VERTICAL_LAYOUT?0:1]+a+f.NODE_TITLE_HEIGHT}c+=k+a}this.setDirtyCanvas(!0,!0)};m.prototype.getTime=function(){return this.globaltime};
...
d.style.height="calc(100% - 40px)";var f=panel.addButton("Assign",function(){a.setProperty(c,d.value);e()});panel.alt_content.appendChild(f);f=panel.addButton("Close",e);f.style.float="right";panel.alt_content.appendChild(f)};b();this.canvas.parentNode.appendChild(panel)};h.prototype.showSubgraphPropertiesDialog=function(a){function b(){d.clear();if(a.inputs)for(var c=0;c<a.inputs.length;++c){var f=a.inputs[c];if(!f.not_subgraph_input){var k=d.addHTML("<button>&#10005;</button> <span class='bullet_icon'></span><span class='name'></span><span class='type'></span>",
"subgraph_property");k.dataset.name=f.name;k.dataset.slot=c;k.querySelector(".name").innerText=f.name;k.querySelector(".type").innerText=f.type;k.querySelector("button").addEventListener("click",function(c){a.removeInput(Number(this.parentNode.dataset.slot));b()})}}}console.log("showing subgraph properties dialog");var c=this.canvas.parentNode.querySelector(".subgraph_dialog");c&&c.close();var d=this.createPanel("Subgraph Inputs",{closable:!0,width:500});d.node=a;d.classList.add("subgraph_dialog");
